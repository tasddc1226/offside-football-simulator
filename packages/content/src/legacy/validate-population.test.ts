...
import { LEGACY_POLICY } from '@offside/domain';
import {
  populationChecksum,
...
describe('legacy reference population validator', () => {
  ...
  it('rejects wrong count and unsorted score groups', () => {
    ...
    expect(() => validateLegacyPopulation(unsorted, manifestFor(unsorted), registered)).toThrow(/sorted/);
  });
  ...
  it('rejects duplicate groups and malformed population checksum', () => {
    ...
    expect(() => validateLegacyPopulation(raw, duplicate, registered)).toThrow(/checksum\/provenance/);

    const tampered = manifestFor(raw);
    tampered.populationChecksum = 'd'.repeat(64);
    expect(() => validateLegacyPopulation(raw, tampered, registered)).toThrow(/checksum\/provenance/);
  });
  ...
  it('rejects policy or registered artifact checksum drift', () => {
    ...
    expect(() => validateLegacyPopulation(raw, wrongPolicy, registered)).toThrow(/checksum\/provenance/);

    const wrongRegistered = { ...registered, contentPackChecksum: 'e'.repeat(64) };
    expect(() => validateLegacyPopulation(raw, manifestFor(raw), wrongRegistered)).toThrow(/checksum\/provenance/);
  });
});
