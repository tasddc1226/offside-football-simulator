...
type PackSource = { manifest: unknown; events: readonly unknown[]; narrativeTokens: unknown };
...
/**
 * 번들에 포함된 팩 JSON(manifest·이벤트 10개·narrative 사전)을 스키마로 검증해 동기로 돌려준다.
 * `loadRuleset`과 같은 방식(정적 JSON import)이라 Node `fs` 없이 브라우저에서도 쓸 수 있다.
 * CLI 전용 `cli/load-pack.ts`(디렉터리를 `readdirSync`로 스캔)와는 별개다.
 */
export function loadContentPack(version: string): ContentPack {
  ...
  return { manifest, events, eventsById, narrativeTokens };
}
