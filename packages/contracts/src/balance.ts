...
export const BalanceOverridesSchema = z.strictObject({
  ...(Object.fromEntries(
    BALANCE_KEYS.map((k) => [k, z.number().min(BALANCE_SPEC[k].min).max(BALANCE_SPEC[k].max).optional()]),
  ) as Record<BalanceKey, z.ZodOptional<z.ZodNumber>>),
  eventWeight: z.record(z.string().regex(EVENT_ID_PATTERN), z.number().min(EVENT_WEIGHT_RANGE.min).max(EVENT_WEIGHT_RANGE.max)).optional(),
  choiceBonus: z.record(z.string().regex(CHOICE_KEY_PATTERN), z.number().min(CHOICE_BONUS_RANGE.min).max(CHOICE_BONUS_RANGE.max)).optional(),
});
