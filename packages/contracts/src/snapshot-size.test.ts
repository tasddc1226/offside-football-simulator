  ...
  const commandsPart = steps
    ...
    .map(
      (
        ...
        CommandLogEntry,
        'revision' | 'commandId' | 'commandType' | 'payload' | 'resultHash'
      > => ({
        ...
        resultHash:
          step.snapshot.revision === lastRevision ? last.stateHash : step.snapshot.stateHash,
      }),
    );
  return {
    baseRevision: from,
...
function withAtLeastThreeOffersForSize(step: Step): Step {
  ...
  if (template === undefined)
    throw new Error('제안 시장이 비어 있어 3개 제안 크기를 합성할 수 없다.');
...
describe('Snapshot·PUT 본문 크기(D-33)', () => {
  ...
  it.each(['FAST', 'CHAPTER'] as const)(
    ...
    (mode) => {
      ...
      const peak = seasonSteps.reduce(
        (max, step) => (stateBytes(step.snapshot) > stateBytes(max) ? step.snapshot : max),
        seasonSteps[0]!.snapshot,
      );
  ...
  it.each([
    {
      label: 'career-07-df',
      engineCommands: career07DfEngineCommands,
      versions: career07Df,
      prefix: 'size-c7',
    },
    {
      label: 'career-08-mf',
      engineCommands: career08MfEngineCommands,
      versions: career08Mf,
      prefix: 'size-c8',
    },
    {
      label: 'career-09-fw',
      engineCommands: career09FwEngineCommands,
      versions: career09Fw,
      prefix: 'size-c9',
    },
  ])(
    '$label(SETTLE_SEASON 직후, season null): 상태·PUT 본문 크기가 상한 안에 든다',
    ({ label, engineCommands, versions, prefix }) => {
      let counter = 0;
  ...
  it.each([
    ...
    '$label: 시장·협상·계약 전환·최종 Snapshot과 전체 PUT 본문이 예산 안에 든다',
    ({ label, engineCommands, versions, prefix }) => {
      ...
      const fixtureMarket = offersSteps.reduce<Step | undefined>(
        (largest, step) =>
          ...
          (step.snapshot.state.pending?.kind === 'OFFERS' &&
            ...
            step.snapshot.state.pending.offers.length >
              largest.snapshot.state.pending.offers.length)
            ? step
            : largest,
        undefined,
      );
      ...
      const negotiated =
        label === 'career-10-transfer'
          ? findStep((step) => step.command.type === 'NEGOTIATE', '협상 직후')
          : undefined;
      const transfer =
        label === 'career-10-transfer'
          ? findStep(
              (step) =>
                step.command.type === 'ACCEPT_OFFER' && step.command.payload.offerId === 'OFR-16-1',
              '이적 직후',
            )
          : undefined;
      const loan =
        label === 'career-11-loan'
          ? findStep(
              (step) => step.snapshot.state.contract?.kind === 'LOAN',
              '임대 중(parentContract 포함)',
            )
          : undefined;
      const final = steps[steps.length - 1]!;
      const offerCount =
        market.snapshot.state.pending?.kind === 'OFFERS'
          ? market.snapshot.state.pending.offers.length
          : 0;
      ...
      for (const point of measured) {
        expect(point.stateBytes, `${label} ${point.checkpoint}`).toBeLessThan(
          SNAPSHOT_STATE_RECOMMENDED_BYTES,
        );
        expect(point.bodyBytes, `${label} ${point.checkpoint} PUT`).toBeLessThan(
  ...
  it('career-13-integration: 시즌별 결산 상태·PUT 본문 크기와 timeline/relationshipLog/health.episodes 시즌당 증가량', () => {
    const steps = replayIndependentFixture(
      career13IntegrationEngineCommands,
      career13Integration,
      'size-c13',
    );
    ...
    function fieldBytes(
      ...
      field: 'timeline' | 'relationshipLog' | 'health',
    ): number {
    ...
    for (const season of perSeason) {
      ...
      expect(
        season.stateBytes,
        `career-13-integration season${season.seasonIndex} state`,
      ).toBeLessThanOrEqual(SNAPSHOT_STATE_RECOMMENDED_BYTES);
      if (season.stateBytes > SNAPSHOT_STATE_RECOMMENDED_BYTES / 2) {
        console.warn(
          `career-13-integration season${season.seasonIndex} state가 128KB(경고 기준)를 넘었다: ${season.stateBytes} bytes`,
        );
      }
      expect(
        season.bodyBytes,
        `career-13-integration season${season.seasonIndex} PUT`,
      ).toBeLessThanOrEqual(REQUEST_BODY_MAX_BYTES);
    }
    expect(bodyBytesFull, 'career-13-integration full PUT').toBeLessThanOrEqual(
      REQUEST_BODY_MAX_BYTES,
    );
    ...
    const activeTemplate = [...steps]
      .reverse()
      .find((step) => step.snapshot.state.season !== null)!;
    ...
    const templateLeague = rulesetProto.leagues.find(
      (league) => league.id === activeTeam.leagueId,
    )!;
    const maxLeague = { ...templateLeague, teamCount: 16 };
    const maxRuleset: Ruleset = {
      ...rulesetProto,
      version: '1.7.0',
      leagues: rulesetProto.leagues.map((league) =>
        league.id === maxLeague.id ? maxLeague : league,
      ),
      leagueLedgerRules: {
        policyVersion: '1.0.0',
        maxTeamCount: 16,
    ...
    const leagueSchedule = fixtures
      .filter(
        (fixture) => fixture.homeTeamId === activeTeam.id || fixture.awayTeamId === activeTeam.id,
      )
      .map((fixture) => ({
        step: fixture.step,
        order: 0,
    ...
    const activeSchedule = [
      ...leagueSchedule,
      ...activeTemplateSeason.schedule.filter((entry) => entry.kind === 'CUP'),
    ]
    ...
    const activeLeagueMatches = activeSchedule
      .filter((entry) => entry.kind === 'LEAGUE')
      .map((entry) => {
    ...
    const activeState = {
      ...
      season: {
        ...
        competitions: projectLeagueCompetition(
          maxRuleset,
          ledger,
          activeTemplateSeason.competitions,
        ),
      },
    };
    ...
    const combinedTarget: Step = {
      ...
      snapshot: {
        ...
        state: combinedState,
        stateHash: hashState(combinedState),
      },
    };
