...
import { BALANCE_KEYS, BALANCE_SPEC, BalanceDraftInputSchema, BalanceOverridesSchema } from './balance.js';
...
describe('밸런스 스펙 · 스키마', () => {
  ...
  it('범위 안 값과 이벤트 맵은 받고, 범위 밖·모르는 키·잘못된 키는 거절한다', () => {
    expect(BalanceOverridesSchema.parse({ growthScale: 1.2, eventWeight: { knock: 0 }, choiceBonus: { 'knock:1': -0.2 } })).toEqual({
      growthScale: 1.2,
      eventWeight: { knock: 0 },
      choiceBonus: { 'knock:1': -0.2 },
    });
    for (const bad of [{ growthScale: 2 }, { mystery: 1 }, { eventWeight: { knock: 6 } }, { choiceBonus: { knock: 0.1 } }]) {
  ...
  it('초안 입력: 메모는 다듬고 기본값은 빈 문자열', () => {
    ...
    expect(BalanceDraftInputSchema.safeParse({ note: 'x'.repeat(201), values: {} }).success).toBe(false);
  });
});
