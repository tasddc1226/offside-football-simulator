...
export function computeSettlementPopularityDelta(
  ...
  const rawDelta =
    ...
    (result.selectionSummary.squadRoleAtEnd === 'STARTER' ? rules.settlement.starterSeasonCenti : 0) +
...
export function applySettlementReputation(state: CareerState, result: SeasonResult, ruleset: Ruleset): CareerState {
  const delta = computeSettlementPopularityDelta(result, ruleset.reputationRules);
  return {
    ...state,
    reputation: {
      ...state.reputation,
      popularityCenti: clamp(state.reputation.popularityCenti + delta, 0, ruleset.reputationRules.clampMax),
    },
  };
}
