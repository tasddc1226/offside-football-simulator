...
function currentSquadPerformance(state: CareerState, contract: Contract): SquadPerformanceSnapshot {
  ...
  if (lastSeason !== undefined) {
    return {
      squadRole: lastSeason.result.selectionSummary.squadRoleAtEnd,
      playerStats: lastSeason.result.playerStats,
    };
  }
 ...
 */
export function judgeMarketReason(
  state: CareerState,
  ruleset: Ruleset,
): 'EXPIRED' | 'INTEREST' | null {
  ...
  if (
    state.timeline.some(
      (entry) => entry.kind === 'CONTRACT_RENEWED' && entry.revision > lastSeasonStart,
    )
  )
    return null;
  ...
  if (performance.squadRole === 'STARTER') {
    ...
    const avgRatingTenths =
      stats !== null && stats.ratedMatches > 0
        ? Math.round(stats.ratingSumTenths / stats.ratedMatches)
        : 0;
  ...
  const indexCenti = computeMarketValueIndex(
    buildMarketValueInput(state, ruleset),
    ruleset.marketValueRules,
  ).indexCenti;
...
function hasConsecutiveZeroMinuteSeasons(state: CareerState, count: number): boolean {
  return (
    state.seasonHistory.length >= count &&
    state.seasonHistory.slice(-count).every((season) => season.result.playerStats.minutes === 0)
  );
}
...
function needsRecoveryOpportunity(state: CareerState, ruleset: Ruleset): boolean {
  ...
  return (
    policy !== undefined &&
    hasConsecutiveZeroMinuteSeasons(state, policy.zeroMinutesConsecutiveSeasons)
  );
}
...
export function isYouthExitRequired(state: CareerState, ruleset: Ruleset): boolean {
  ...
  return (
    policy !== undefined &&
    contract !== null &&
    contract.leagueTier === 'YOUTH' &&
    state.age > policy.youthMaxAge
  );
}
...
function buildRecoveryOpportunity(
  ...
  if (policy === undefined || contract === null || profile === null)
    throw new RangeError('buildRecoveryOpportunity: recovery context is incomplete.');
  const team = ruleset.teams
    .filter(
      (candidate) =>
        candidate.leagueTier === policy.opportunityTier && candidate.id !== contract.teamId,
    )
    .sort((a, b) => a.squadStrength - b.squadStrength || compareCodePoints(a.id, b.id))[0];
  if (team === undefined)
    throw new RangeError(
      `buildRecoveryOpportunity: tier ${policy.opportunityTier} team is missing.`,
    );
  ...
  return {
    ...
    lengthSeasons: 1,
    wageMinorPerWeek: lookupBandAmount(
      ruleset.contractRules.wageBands,
      team.wageBandId,
      band.id,
      'wageBands',
    ),
    signingBonusMinor: lookupBandAmount(
    ...
    rolePromise: policy.opportunityRole,
    appearancePromise: {
      minutesShareBp: ruleset.contractRules.promiseMinutesShareBp[policy.opportunityRole],
    },
    ...
    competitorSummary:
      ...
      computeCompetitorSummary(
        ...
        profile.baseOvr,
        `recovery:${state.careerId}:${revision}:${team.id}`,
      ),
    ...
    loan:
      kind === 'LOAN'
        ? {
            parentTeamId: contract.teamId,
            seasons: 1,
            wageShareBp: ruleset.transferRules.loan.wageShareBp,
            buyOptionMinor: null,
          }
        : null,
  };
 ...
 * 확보한다.
 */
function computeCandidateTeams(
  ruleset: Ruleset,
...
function findFeeMinor(bands: TransferRules['feeByIndexBand'], indexCenti: number): number {
  ...
  if (band === undefined) {
    throw new RangeError(
      `market.ts: feeByIndexBand에 indexCenti ${indexCenti}에 맞는 구간이 없다.`,
    );
  }
...
function computeCompetitorSummary(
  ...
  const positionCompetitors = generated.competitors.filter(
    (competitor) => competitor.position === position,
  );
  const topOvr = positionCompetitors.reduce(
    (max, competitor) => Math.max(max, competitor.baseOvr),
    0,
  );
...
function buildSafeOffer(args: BuildSafeOfferArgs): Offer {
  ...
  > = {
    ...
    ...(contract.leagueName === undefined ? {} : { leagueName: contract.leagueName }),
  ...
  const band = findOvrBand(ruleset.contractRules, profile.baseOvr);
  const wageBase = lookupBandAmount(
    ruleset.contractRules.wageBands,
    team.wageBandId,
    band.id,
    'wageBands',
  ...
  return {
    ...
    rolePromise: degradedRole,
    appearancePromise: {
      minutesShareBp: ruleset.contractRules.promiseMinutesShareBp[degradedRole],
    },
    tacticalFitEstimate: projection?.tacticalFit ?? state.context.tacticalFit,
    competitorSummary: projection?.competitorSummary ?? null,
    validUntilRevision: null,
...
export function generateMarket(args: GenerateMarketArgs): GeneratedMarket {
  ...
  const candidateTeams = computeCandidateTeams(ruleset, contract.teamId, indexCenti, state);
  ...
  for (let index = 0; index < lotteryCount; index++) {
    ...
    if (reason === 'EXPIRED') {
      ...
      const rolledKind =
        kindRoll.value < transferRules.kindWeightsByRole[squadRole].TRANSFER ? 'TRANSFER' : 'LOAN';
      const preference =
        state.clubMeeting?.preferenceStatus === 'PENDING' &&
        state.clubMeeting.contractId === contract.id &&
        state.clubMeeting.seasonIndex === state.seasonHistory.length
    ...
    if (kind === 'LOAN') {
      ...
      loan = {
        parentTeamId: contract.teamId,
        seasons: 1,
        wageShareBp: transferRules.loan.wageShareBp,
        buyOptionMinor,
      };
      lengthSeasons = 1;
    } else {
      const lengthRoll = rollRange(
        rngState,
        ruleset.offerRules.lengthSeasons.min,
    ...
    const shirtRoll = rollRange(
      rngState,
      ruleset.offerRules.shirtNumber.min,
      ruleset.offerRules.shirtNumber.max,
    );
    ...
    const fitRoll =
      ruleset.offerProjection === undefined
        ? rollRange(
            rngState,
            ruleset.offerRules.tacticalFitEstimate.min,
            ruleset.offerRules.tacticalFitEstimate.max,
          )
        : null;
    ...
    drawnOffers.push({
      ...
      ...(team.countryCode === undefined
        ? {}
        : { leagueName: ruleset.leagues.find((l) => l.id === team.leagueId)!.name }),
      leagueTier: team.leagueTier,
      lengthSeasons,
      wageMinorPerWeek: wage,
  ...
  const offers =
    needsRecoveryOpportunity(state, ruleset) && !youthExit
...
export function countContractMatchesPlayed(state: CareerState, contract: Contract): number {
  const settled = state.seasonHistory
    .filter(
      (summary) =>
        summary.index >= contract.signedSeasonIndex && summary.teamId === contract.teamId,
    )
    .reduce((sum, summary) => sum + matchesWithMinutes(summary.result.playerStats), 0);
  const season = state.season;
  const current =
    season !== null && season.teamId === contract.teamId
      ? matchesWithMinutes(season.playerStats)
      : 0;
...
export function buildRenewalOffer(state: CareerState, ruleset: Ruleset, revision: number): Offer {
  ...
  const wage = Math.floor(
    (wageBase * transferRules.renewal.wageBpByRole[season.squadRole]) / 10000,
  );
  const bonusBase = lookupBandAmount(
    ruleset.contractRules.signingBonus,
    team.wageBandId,
    band.id,
    'signingBonus',
  ...
  return {
    ...
    appearancePromise: {
      minutesShareBp: ruleset.contractRules.promiseMinutesShareBp[season.squadRole],
    },
    positionPlan: contract.positionPlan,
    shirtNumber: contract.shirtNumber,
 ...
 */
export function openMarketAfterSettlement(
  settledState: CareerState,
  ruleset: Ruleset,
  revision: number,
): OpenMarketResult {
  if (
    ruleset.retirementRules?.maxCareerSeasons !== undefined &&
    settledState.seasonHistory.length >= ruleset.retirementRules.maxCareerSeasons
  )
  ...
  const consumePreference =
    reason === 'INTEREST' &&
    meeting?.preferenceStatus === 'PENDING' &&
    meeting.contractId === settledState.contract?.id &&
    meeting.seasonIndex === settledState.seasonHistory.length;
  const requestedOfferExists =
    consumePreference &&
    generated.pending.offers.some(
      (offer) =>
        offer.id !== generated.pending.market.safeOfferId &&
        offer.kind === meeting.preferredOfferKind,
    );
  return {
    state: {
      ...settledState,
      ...
      ...(meeting?.preferenceStatus === 'PENDING' && reason === 'EXPIRED'
        ? { clubMeeting: { ...meeting, preferenceStatus: 'CANCELLED' as const } }
        : consumePreference
          ? {
              clubMeeting: {
                ...meeting,
                preferenceStatus: requestedOfferExists
                  ? ('OFFERED' as const)
