...
Vite + React SPA가 브라우저 Web Worker에서 게임을 실행하고 IndexedDB에 저장한다. Cloudflare Workers(Hono) + D1이 프로필·Google 로그인·복구·checkpoint 동기화·보관함·서비스 시즌을 담당하고, 필요할 때만 명령 로그를 재생해 결과를 검증한다. 콘텐츠 팩은 JSON 정적 번들이다. 상세는 [`docs/adr/`](docs/adr/README.md).
