...
const season = (over: Partial<FirstSeason> = {}): FirstSeason => ({
  year: 2030 + t, age: 22, club: 'A FC', league: 'K리그1', apps: 30, goals: 10, assists: 5, cs: 0, caps: 0, rating: 7,
  ovr: 70, honors: [], mil: false, createdAt: `2026-09-26T00:00:${String(t++).padStart(2, '0')}.000Z`, ...over,
});
const career = (seasons: FirstSeason[], over: Partial<FirstCareer> = {}): FirstCareer => ({ id: 'c1', legendScore: null, retiredAt: null, seasons, ...over });
...
describe('서버 최초 기록 규칙 (T-10-027)', () => {
  ...
  it('시즌·나이·한 클럽·리그 수 조건', () => {
    expect(ids(career([season({ goals: 41, age: 20 })]))).toEqual(expect.arrayContaining(['sgoals30', 'sgoals40', 'teen20']));
    expect(ids(career([season({ rating: 8.6, apps: 10 })]))).not.toContain('srating80'); // 15경기 미만은 평점 기록 제외
    expect(ids(career([season({ rating: 8.6, apps: 15 })]))).toEqual(expect.arrayContaining(['srating80', 'srating85']));
    ...
    const leagues = ['K리그1', '프리미어리그', '라리가', '세리에 A', '분데스리가'].map((league) => season({ league }));
  ...
  it('레전드 점수 기록은 은퇴 시각으로 잡는다', () => {
    ...
    expect(evaluateCareer(c).find((g) => g.id === 'legend840')).toEqual({ id: 'legend840', at: c.retiredAt, year: null });
