...
const toVersion = ({ valuesJson, createdBy: _createdBy, ...r }: Row): BalanceVersion => ({ ...r, values: sanitizeBalance(JSON.parse(valuesJson)) });

export async function getActiveBalance(db: Db): Promise<BalanceVersion | undefined> {
  const [row] = await db.select().from(balanceVersions).where(eq(balanceVersions.status, 'active')).limit(1);
  return row && toVersion(row);
}

export async function getBalanceVersion(db: Db, version: number): Promise<BalanceVersion | undefined> {
...
export async function listBalanceVersions(db: Db): Promise<BalanceVersion[]> {
  const rows = await db.select().from(balanceVersions).orderBy(desc(balanceVersions.version)).limit(HISTORY_MAX);
...
export async function activateBalance(db: Db, version: number, previous: number | null, profileId: string, now: string) {
  await runBatch(db, [
    db.update(balanceVersions).set({ status: 'archived', updatedAt: now }).where(eq(balanceVersions.status, 'active')),
    db.update(balanceVersions).set({ status: 'active', activatedAt: now, updatedAt: now }).where(eq(balanceVersions.version, version)),
