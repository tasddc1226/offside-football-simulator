...
describe('GET /v1/profile', () => {
  ...
  it('같은 쿠키로 재요청하면 같은 프로필을 돌려주고 새 세션을 만들지 않는다', async () => {
    ...
    const res = await app.request(
      '/v1/profile',
      { headers: { Cookie: `offside_session=${token}` } },
      ctx.env,
    );
  ...
  it('위조된 쿠키는 새 프로필을 발급한다', async () => {
    ...
    const res = await app.request(
      '/v1/profile',
      { headers: { Cookie: 'offside_session=not-a-real-token' } },
      ctx.env,
    );
  ...
  it('만료된 세션은 새 프로필을 발급한다', async () => {
    ...
    const res = await app.request(
      '/v1/profile',
      { headers: { Cookie: `offside_session=${rawToken}` } },
      ctx.env,
    );
  ...
  it('Bearer 세션(toss)이 있으면 같은 프로필을 돌려준다', async () => {
    const profile = await createProfile(ctx.db);
    const { token } = await issueSession(ctx.db, {
      profileId: profile.id,
      channel: 'toss',
  ...
  it('무효한 Bearer는 유효한 쿠키가 있어도 401이고 폴백하지 않는다', async () => {
    ...
    const res = await app.request(
      ...
      {
        headers: {
          Authorization: 'Bearer invalid-token-000000',
...
describe('PATCH /v1/profile/settings', () => {
  ...
  function patchInit(overrides: {
    ...
    if (overrides.idempotencyKey !== null)
      headers['Idempotency-Key'] = overrides.idempotencyKey ?? 'idem-key-0001';
  ...
  it('본문이 1MB를 넘으면 400 BODY_TOO_LARGE다', async () => {
    ...
    const res = await app.request(
      '/v1/profile/settings',
      patchInit({
        rawBody: 'a'.repeat(REQUEST_BODY_MAX_BYTES + 1),
  ...
  it('textScale에 없는 값을 보내면 400 VALIDATION_FAILED, issues path가 textScale이다', async () => {
    ...
    expect(body.error.code).toBe('VALIDATION_FAILED');
    const issues =
      (body.error.details as { issues?: { path?: unknown[] }[] } | undefined)?.issues ?? [];
    expect(
      issues.some((issue) => JSON.stringify(issue.path) === JSON.stringify(['textScale'])),
    ).toBe(true);
  });
  ...
  it('같은 키·같은 본문 재요청은 저장된 응답을 그대로 재생한다', async () => {
    ...
    const idempotencyRows = await ctx.db
      .select()
      .from(idempotency)
      .where(eq(idempotency.key, 'idem-replay-1'));
...
describe('PUT /v1/profile/nickname', () => {
  ...
  const errorOf = async (res: Response) => {
    ...
    return {
      ...
      reason: (body.error.details as { reason?: string } | undefined)?.reason,
    };
  };

  it('구글 로그인한 프로필만 닉네임을 정하고, GET /v1/profile에 실린다', async () => {
    const anon = await issueCookie(ctx);
    expect(await errorOf(await put(anon.token, '루키'))).toEqual({
      status: 403,
      code: 'FORBIDDEN',
    ...
    const me = await app.request(
      '/v1/profile',
      { headers: { Cookie: `offside_session=${user.token}` } },
      ctx.env,
    );
  ...
  it('대소문자만 달라도 겹치면 409, 금칙어·형식 오류는 400', async () => {
    ...
    expect(await errorOf(await put(b.token, 'rookie'))).toEqual({
      status: 409,
      code: 'VALIDATION_FAILED',
      reason: 'NICKNAME_TAKEN',
    });
    expect(await errorOf(await put(b.token, 'www.spam.com'))).toMatchObject({
      status: 400,
      reason: 'BLOCKED_WORD',
    });
  ...
  it("'운영자' 같은 운영진 닉네임은 막고, 관리자 계정은 언제나 '운영자'다", async () => {
    ...
    for (const name of ['운영자', '운 영 자', '진짜운영자', '관리자', '운영진']) {
      expect(await errorOf(await put(user.token, name)), name).toEqual({
        status: 400,
        code: 'VALIDATION_FAILED',
        reason: 'RESERVED_NICKNAME',
      });
    }
    const admin = await googleUser(ADMIN_EMAIL);
    const me = await app.request(
      '/v1/profile',
      { headers: { Cookie: `offside_session=${admin.token}` } },
      env(),
    );
    expect(successEnvelope(ProfileSchema).parse(await me.json()).data.nickname).toBe('운영자');
    expect(await errorOf(await put(admin.token, '루키'))).toEqual({
      status: 403,
      code: 'FORBIDDEN',
      reason: 'ADMIN_NICKNAME_FIXED',
    });
  });
});
