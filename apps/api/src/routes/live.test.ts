...
const summary = { retireAge: 34, peak: 88, legendScore: 612, apps: 300, goals: 120, assists: 60, trophies: 1, awards: 0, caps: 30, ballon: 0, lastClub: '테스트 FC', publicName: null };
...
describe('홈 라이브 현황 /v1/live (T-10-030)', () => {
  ...
  it('시즌·은퇴 업로드가 최신순 피드와 오늘 숫자로 보인다 — 선수당 한 줄', async () => {
    ...
    expect(data.feed[0]).toMatchObject({ kind: 'retire', careerId: A, name: null, pos: 'FW', score: 612, lastClub: '테스트 FC' });
    expect(data.feed[1]).toMatchObject({ kind: 'season', club: '테스트 고교', first: true, honor: '고교리그 우승' });
    expect(data.feed[2]).toMatchObject({ kind: 'season', club: '테스트 FC', goals: 21, first: false, honor: null });
