...
export async function buildForeignDeviceSnapshot(careerId: string): Promise<CareerSnapshot> {
  ...
  if (!created.ok) {
    throw new Error(
      `e2e: 다른 기기 스냅샷 시드 실패 ${created.error.code} ${created.error.message}`,
    );
  }
...
export async function stubRevisionConflict(
  page: Page,
  careerId: string,
  serverSnapshot: CareerSnapshot,
): Promise<void> {
 ...
 */
export async function triggerConflictAndOpenDialog(
  page: Page,
): Promise<{ careerId: string; serverSnapshot: CareerSnapshot }> {
  ...
  await page.route('**/v1/careers/**', async (route) => {
    if (route.request().method() === 'PUT') {
      await fulfillJson(route, 200, {
        data: { revision: 1, syncedAt: '2026-01-01T00:00:00Z' },
        meta: E2E_META,
      });
      return;
    }
  ...
  const [getResponse] = await Promise.all([
    page.waitForResponse(
      (response) =>
        response.url().includes(`/v1/careers/${careerId}`) && response.request().method() === 'GET',
    ),
    page.getByRole('button', { name: '지금 동기화' }).click(),
  ]);
  if (getResponse.status() !== 200)
    throw new Error(`e2e: 충돌 GET 스텁 실패(${getResponse.status()})`);
