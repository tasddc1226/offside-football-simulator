...
test('복구 코드 재발급: 확인 → 코드 대화상자 → 복사 → 닫힘, 발급일이 갱신된다', async ({ page, context }) => {
  ...
  await expect(page.getByText('이전 코드는 즉시 쓸 수 없게 됩니다. 새 코드를 적어 두세요.')).toBeVisible();
...
test('프로필 복구: 형식이 잘못된 코드는 필드 옆에 오류를 보여준다(네트워크 요청 없음)', async ({ page }) => {
...
test('프로필 복구: 코드가 맞지 않으면 안내하고 입력·포커스를 보존한다', async ({ page }) => {
  await page.route('**/v1/profile/recover', (route) =>
    fulfillJson(route, 400, {
      error: { code: 'RECOVERY_CODE_INVALID', message: '복구 코드가 올바르지 않습니다.', retryable: false },
      meta: E2E_META,
    }),
  );
...
test('프로필 복구: 시도 횟수를 넘으면 잠시 뒤 다시 시도하라고 안내한다', async ({ page }) => {
  await page.route('**/v1/profile/recover', (route) =>
    fulfillJson(route, 429, { error: { code: 'RATE_LIMITED', message: '너무 많이 시도했습니다.', retryable: true }, meta: E2E_META }),
  );
...
test('프로필 복구: RECOVERY_CONFLICT면 선택 대화상자가 뜨고 mergeChoice를 붙여 재전송한다', async ({ page }) => {
...
test('프로필 삭제: 1단계 → 확인 대화상자 → 2단계 → 온보딩으로 이동한다', async ({ page }) => {
  await page.route('**/v1/profile/delete', async (route) => {
    ...
    if (Object.keys(body).length === 0) {
      await fulfillJson(route, 200, { data: { confirmToken: 'tok_e2e', expiresAt: '2026-09-03T00:10:00Z' }, meta: E2E_META });
      return;
    }
  ...
  await expect(
    page.getByText('모든 커리어, 복구 코드, 서버 저장 데이터가 즉시 삭제됩니다. 되돌릴 수 없습니다.'),
  ).toBeVisible();
...
test('이 기기 데이터 삭제: 확인 → 온보딩 → 허브가 빈 상태로 돌아온다', async ({ page }) => {
  await page.route('**/v1/careers/*', async (route) => {
    if (route.request().method() === 'PUT') {
      await fulfillJson(route, 200, { data: { revision: 1, syncedAt: '2026-09-03T00:00:00Z' }, meta: E2E_META });
      return;
    }
    await route.continue();
  });
