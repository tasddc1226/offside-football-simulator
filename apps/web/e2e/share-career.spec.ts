...
test('은퇴 기록이 아직 서버에 없으면 연결 오류가 아니라 "아직 올리지 못했다"고 알린다', async ({
  page,
  context,
}) => {
  await context.grantPermissions(['clipboard-read', 'clipboard-write']);
  ...
  await page.route(/\/v1\/careers\/[^/]+\/retirement$/, (r) =>
    r.fulfill({
      status: 503,
      json: { error: { code: 'UNAVAILABLE', message: '점검 중', retryable: true } },
    }),
  );
  await retireNow(page);
  await page.locator('[data-act="share-career"]').click();
  await expect(
    page.getByText('기록을 아직 서버에 올리지 못했어요. 잠시 후 다시 시도해 주세요.'),
  ).toBeVisible();
