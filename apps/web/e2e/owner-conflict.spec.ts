...
const cidOf = (page: Page) => page.evaluate(() => (JSON.parse(localStorage.getItem('ft_save')!) as { cid: string }).cid);

async function raiseConflict(page: Page) {
  const cid = await cidOf(page);
  await page.evaluate((careerId) => {
    window.dispatchEvent(new CustomEvent('offside:owner-conflict', { detail: [{ kind: 'season', careerId, year: 2026, body: { events: [] } }] }));
  }, cid);
...
test('다른 계정 소유 커리어 → 이 기기에만 두면 다시 묻지 않는다', async ({ page }) => {
  ...
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('ft_conflict_skip')!))).toBe(cid);
