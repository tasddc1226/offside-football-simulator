...
async function stub(page: Page, body: unknown = live, status = 200) {
  await page.route(`${API}/v1/live`, (r) =>
    r.fulfill({
      ...
      body: JSON.stringify(status === 200 ? { data: body } : {}),
    }),
  );
}
...
test('서버에 연결하지 못하면 같은 자리에 안내를 띄우고, 아무 활동이 없으면 카드를 숨긴다', async ({
  page,
}) => {
  ...
  await stub(page, {
    ...
    stats: { playing: 0, seasonsToday: 0, newToday: 0, retiredToday: 0 },
    feed: [],
  });
  await page.reload();
  await expect(page.locator('[data-home-news="notice"]')).toBeVisible();
