...
test('cid 없는 옛 은퇴 저장본은 시즌을 먼저 올려 커리어를 만든 뒤 은퇴를 올리고, 새 cid를 저장한다', async ({
  page,
}) => {
  ...
  expect(
    await page.evaluate(
      () => (JSON.parse(localStorage.getItem('ft_hof')!) as { id?: string }[])[0]?.id,
    ),
  ).toBe(cid);
  await page.reload();
...
test('옛 저장 키(sl_save)는 한 번만 ft_save로 옮기고 지운다 — 이후 ft_save가 비어도 되살아나지 않는다', async ({
  page,
}) => {
  ...
  expect(
    await page.evaluate(() => [localStorage.getItem('sl_save'), !!localStorage.getItem('ft_save')]),
  ).toEqual([null, true]);
