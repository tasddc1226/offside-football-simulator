...
import { PlayerHeader, RadioGroup, RadioGroupItem, StatusStrip, buttonClassName, buttonStyle } from '@offside/ui';
...
function PreseasonScreen() {
  ...
  useEffect(() => {
    platform.analytics.track('screen_viewed', { screenId: 'SCR-005', careerPhase: query.data?.state.seasonPhase ?? 'NONE' });
  ...
  return (
    <div className="flex flex-col gap-os-6">
      ...
      <p className="font-os text-os-text-2" style={CAPTION_STYLE}>
        감독 전술: {style ? `${style.name} · ${style.formation}` : '—'} · 주전 경쟁: 시즌 시작 시 정해집니다
      </p>
      ...
      <section className="flex flex-col gap-os-3">
        ...
        <RadioGroup aria-label="시뮬레이션 모드" value={mode} onValueChange={(value) => setMode(value as SimulationMode)}>
          {(['FAST', 'CHAPTER'] as const).map((candidate) => (
            <RadioGroupItem key={candidate} value={candidate} className="flex flex-col gap-os-1 p-os-3 text-left">
      ...
      <section className="flex flex-col gap-os-3">
        ...
        <RadioGroup aria-label="훈련 계획" value={focus} onValueChange={(value) => setFocus(value as TrainingFocus)}>
