...
describe('MLS (T-10-016)', () => {
  it('상위 클럽은 컵 두 개와 CONCACAF 챔피언스컵에 나가고, 챔피언스컵은 1라운드 녹아웃부터 한다', () => {
    ...
    expect(comps.filter((c) => c.type === 'cup').map((c) => c.name)).toEqual(['US 오픈컵', '리그스컵']);
  ...
  it('고교·대학 선수에게는 MLS 오퍼가 오지 않고, MLS 오퍼는 주로 30대에게 온다', () => {
    const count = (patch: Partial<GameState>) => {
      let n = 0;
      for (let seed = 1; seed <= 300; seed++) n += makeOffers(player(seed, patch)).filter((o) => o.leagueId === 'mls').length;
      return n;
    };
