...
async function createCareerWithSeed(engine: AppEngine, careerId: string, seed: string): Promise<void> {
 ...
 * 정책과 같다). */
async function stepOnceTowards(engine: AppEngine, careerId: string, targetEventId: string, maxSeasons: number, startedSeasonsRef: { count: number }): Promise<StepResult> {
  const load = await engine.client.loadCareer(careerId);
  ...
  if (pending === null) {
    if (state.contract !== null && state.season === null) {
      if (startedSeasonsRef.count >= maxSeasons) return { matched: false, blocked: 'SEASON_BUDGET_EXCEEDED' };
  ...
  if (pending.kind === 'EVENT' || pending.kind === 'INJURY' || pending.kind === 'NATIONAL_TEAM') {
    ...
    const definition = engine.pack.eventsById.get(pending.eventId);
    if (definition === undefined) return { matched: false, blocked: `팩에 이벤트 정의 없음: ${pending.eventId}` };
    const choice = definition.choices[0];
    if (choice === undefined) return { matched: false, blocked: `이벤트에 선택지 없음: ${pending.eventId}` };
    const result = await resolveEvent(engine, careerId, choice.id);
    if (!result.ok) return { matched: false, blocked: `RESOLVE_EVENT(${pending.eventId}): ${result.error.code} ${result.error.message}` };
    return { matched: false };
  }

  if (pending.kind === 'CHAPTER') {
    const definition = engine.pack.chaptersById.get(pending.chapterId);
    if (definition === undefined) return { matched: false, blocked: `팩에 챕터 정의 없음: ${pending.chapterId}` };
    ...
    if (option === undefined) return { matched: false, blocked: `판단에 선택지 없음: ${decision.id}` };
    const result = await resolveChapter(engine, careerId, decision.id, option.id);
    if (!result.ok) return { matched: false, blocked: `RESOLVE_CHAPTER: ${result.error.code} ${result.error.message}` };
    return { matched: false };
  }
  ...
  if (pending.kind === 'OFFERS' || pending.kind === 'CONTRACT') {
    ...
    if (!result.ok) return { matched: false, blocked: `ACCEPT_OFFER: ${result.error.code} ${result.error.message}` };
    return { matched: false };
  }
  ...
  if (!result.ok) return { matched: false, blocked: `SETTLE_SEASON: ${result.error.code} ${result.error.message}` };
  return { matched: false };
}
...
describe('T-4-009 §3: phase4-seeds.ts가 실제 engine 경로(career-actions.ts)에서도 같은 pending을 여는지', () => {
  it.each(TARGETS)(
    ...
    async (target) => {
      ...
      const careerId = 'car_phase4_reachability';
      ...
      for (let i = 0; i < stepBudget && !matched; i += 1) {
        const outcome = await stepOnceTowards(engine, careerId, target.eventId, target.seasonIndex, startedSeasonsRef);
      ...
      expect(matched, `${target.eventId}에 도달하지 못했다(블록 사유: ${blockedReason ?? '스텝 예산 소진'})`).toBe(true);
