...
describe('careerShareMeta', () => {
  it('이름·등급·기록으로 제목과 설명을 만든다', () => {
    ...
    expect(m.description).toContain('36세 은퇴 · 540경기 231골 98도움 · 트로피 6개 · 마지막 소속 토트넘.');
  ...
  it('이름이 없으면 익명 표기, 소속이 없으면 소속을 뺀다', () => {
    const m = careerShareMeta({ ...entry, name: null, number: null, lastClub: '', legendScore: 10 }, 'https://x.test');
...
describe('injectShareMeta', () => {
  ...
  it('제목·설명·og 메타를 바꾸고 og:url을 붙인다(이스케이프 포함)', () => {
    ...
    expect(html).toContain('<meta property="og:title" content="손흥&lt;민&gt; · 월드클래스 레전드 (레전드 612점)"');
    expect(html).toMatch(/<meta name="description" content="36세 은퇴/);
    expect(html).toMatch(/<meta property="og:description" content="36세 은퇴/);
    expect(html).toContain(`<meta property="og:image" content="https://offside-lab.com/og-career-lg_world-${BRAND_VERSION}.png"`);
    expect(html).toContain(`<meta property="og:url" content="https://offside-lab.com/career/${entry.id}" />`);
