// ───────── 모션 공통 유틸 (T-10-003) ─────────
// prefers-reduced-motion 판정을 한 곳에 모은다. sheetState.svelte.ts의 playSteps/playBlock/
// playJudge와 새 화면·탭·시트 전환 애니메이션이 모두 이 값을 공유해, "감속 모션" 설정이 앱 전체에서
// 일관되게 적용되게 한다(매체 쿼리를 매번 새로 만들지 않는다).
