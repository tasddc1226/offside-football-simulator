...
describe('서버 최초 기록 화면 계산 (T-10-027)', () => {
  ...
  it('달성된 기록만 최근 순으로, 같은 한국 날짜끼리 묶는다', () => {
    const list = achievedList([f('a', '2026-09-24T10:00:00.000Z'), f('b', null), f('c', '2026-09-24T16:00:00.000Z'), f('d', '2026-09-25T02:00:00.000Z')]);
    expect(list.map((x) => x.id)).toEqual(['d', 'c', 'a']);
    expect(byDay(list).map((g) => [g.day, g.items.map((x) => x.id)])).toEqual([['26.09.25', ['d', 'c']], ['26.09.24', ['a']]]);
  });
  ...
  it('내 선수는 이 기기의 이름, 아니면 공개 이름 또는 익명', () => {
    ...
    expect(holderLabel({ ...h, name: '김오프' }, new Map())).toEqual({ name: '김오프', mine: false });
