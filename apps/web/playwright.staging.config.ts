...
// T-2-016: staging 리허설 전용 config. 실제 서비스(https://offside-web-staging.tasddc1569.workers.dev)에
// 직접 붙는다 — webServer 없음, `pnpm --filter @offside/web e2e:staging`으로만 실행한다. 기본
// `pnpm e2e`·CI에는 포함되지 않는다(testMatch로 staging-rehearsal.spec.ts만 선택하고, 기본
// playwright.config.ts는 이 파일을 testIgnore로 제외한다). staging에 실제 데이터(커리어·복구 코드·
// 분석 이벤트)가 생기므로 실행 절차는 apps/web/e2e/README.md를 따른다.
const BASE_URL = process.env.E2E_STAGING_URL ?? 'https://offside-web-staging.tasddc1569.workers.dev';
