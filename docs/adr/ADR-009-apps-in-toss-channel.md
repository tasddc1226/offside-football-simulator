# ADR-009. 앱인토스 미니앱 동시 출시

- 상태: 확정 (2026-09-02). 사용자 액션 U-007~U-010 완료 전까지 appName·등급분류는 미확정
...
일반 웹(자체 도메인) 출시와 함께 토스 앱 안의 미니앱(앱인토스)으로도 같은 시점에 출시한다. 앱인토스는 WebView SDK와 React Native SDK를 제공하며, 웹 번들을 토스가 호스팅하고 콘솔 검토를 거쳐 노출한다. 플랫폼 이용자는 만 19세 이상이다.
...
- 사용자 액션: U-007 앱인토스 콘솔 가입·워크스페이스·앱 등록(appName 확정), U-008 서버 mTLS 인증서 발급, U-009 게임 등급분류 신청, U-010 콘솔 MCP 인증, U-011(조건부) 개인사업자 등록.
- Phase 0 백로그에 T-0-012 `platform` 골격, T-0-013 `.ait` 빌드·devtools, T-0-014 `POST /auth/toss/session`을 추가한다.
