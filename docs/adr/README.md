...
브라우저가 게임을 실행하고 저장하며, 서버는 프로필·로그인·동기화 스냅샷·보관함·서비스 시즌만 다룬다. 시뮬레이션은 결정론적이므로 서버는 필요할 때 명령 로그를 재생해 결과를 검증할 수 있다. 모든 인프라는 Cloudflare 한 곳에 두고 월 고정비를 도메인 외 약 5달러로 잡는다.
