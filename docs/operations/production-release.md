...
## 2026-09-23 연간 서버 진행 승격 — 배포 전 계획

PR #276의 목표는 신규 생성 버전을 `3.4.0/0.13.0`에서 `3.5.0/0.14.0`으로 승격하는 것이다.
2026-09-23 공개 current API는 아직 `svc_season_1`, ACTIVE/non-test, `3.4.0/0.13.0`이다.
아래 과거 배포 기록의 버전은 현재 값이나 이번 배포 완료 증거가 아니다.

1. 필수 검사와 전체 E2E를 통과하고 독립 검토 뒤 main에 병합한다. 알려진 과거 실패도 이번
   릴리스의 통과로 간주하지 않는다. main staging 배포와 실제 연간 생성·중요 결정 중단·같은
   연도 재개·새로고침, 기존 커리어 호환성을 검증한다.
2. 병합된 정확한 main SHA로 production workflow의 `preflight`를 실행한다. 시작일은
...
**`SERVER_ANNUAL` 저장이 생긴 뒤에는 이전 API/web 바이너리로 되돌리지 않는다.** 신규 저장은
서버 연간 API와 authority 판별을 필요로 하므로, 생성 포인터만 되돌리고 **새 API·web·migration
0014·3.5/0.14 reader/command 경로를 모두 유지**한다. 기존 연간 커리어도 계속 진행되어야 한다.

- artifact `season-rollback.sql`은 정확한 신규 행에 대해서만 버전 두 필드를
  `3.5.0/0.14.0 → 3.4.0/0.13.0`으로 돌리는 역 CAS다. 자동 실행하지 않는다.
  승인 후 실행 결과 `meta.changes=1`과 전체 metadata read-back을 확인한다. 0이면 성공이 아니다.
- migration DROP, DB Time Travel 복원, 사용자 저장 삭제를 rollback으로 사용하지 않는다.
- 다음 deploy 전에 생성 목표 `PRODUCTION_SEASON`과 fallback/seed/smoke를 이전 생성 pair에
  맞추는 후속 PR이 필요하다. 이는 **포인터 설정 수정**이지 과거 애플리케이션 커밋의 재배포가
  아니다. 호환 API/웹을 제거하면 안 되며, 그렇지 않으면 재배포가 신규 생성을 다시 활성화한다.
- 아래 역사적 절차의 “코드 되돌림”은 당시 포인터 설정에 관한 기록이다. 이번 연간 저장에
  이전 바이너리 복원 지시로 적용하지 않는다.
...
| 현재 신규 커리어 버전 | ruleset 3.4.0 / content pack 0.13.0 (2026-09-23 공개 current API 확인; 기존 커리어는 생성 당시 버전 유지) |
| 다음 승격 목표 | PR #276: 3.5.0 / 0.14.0, 검사·병합·staging·운영 preflight 후 승인된 배포 필요 |
