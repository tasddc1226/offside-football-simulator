...
초기 PR #117은 검색 제외 상태의 준비 작업이다. 후속 도메인 연결 작업은 아래 기록으로 구분한다.
T-9-001(풀타임 마이그레이션) 이후 게임 진행은 서버가 아닌 브라우저 localStorage에만 저장되며
서비스 시즌 개념은 사라졌다. 로그인·계정 메타데이터는 이 작업 범위 밖이며 변경하지 않는다.

## 출발점과 목표

점검 기준 main: `15c559aa765c093618dd53906eb7bb8489ec1394`.
초기 점검 당시 운영 주소는 `https://offside-web.tasddc1569.workers.dev/`였고 도메인 구매 전이었다.
...
- 가이드: OVR 외 체력·폼·능력치 성장의 의미, 프리시즌·전반기·후반기 시즌 진행, 계약은 출전 보장이 아님.
- FAQ: 익명 시작과 Google 연결(로그인 상태만, 게임 데이터는 서버로 전송되지 않음), 브라우저
  localStorage 저장, 과거 버전 저장 자동 변환.
- 현재 구현을 넘는 리그 라이선스·공식 제휴·성공 확률·평균 소요 시간은 주장하지 않는다.
- 문의 주소는 공개 약관·개인정보처리방침(`apps/web/scripts/seo.mjs`)에 둔 운영자 연락처 하나만 쓴다. 복구 코드·토큰·개인 선수 기록을 공개 페이지에 넣지 않는다.

랜딩에서 플레이 화면 이미지를 제거했다(2026-09-13, 사용자 지시).
...
T-9-001(풀타임 마이그레이션) 이후 `apps/web`은 React가 아닌 바닐라 TypeScript(`vite build`)다.
정적 소개는 `apps/web/index.html`, 가이드/FAQ 본문·메타·사이트맵 생성은 전부
`apps/web/scripts/seo.mjs`(빌드 후 `closeBundle`에서 `/guide/`·`/faq/`·`/legal/privacy/`·
`/legal/terms/`의 정적 HTML을 만든다)에 있다. 문구는 이 파일 한 곳에만 있다.
