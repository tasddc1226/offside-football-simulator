...
공통 네비게이션을 스크롤 위치와 무관하게 뷰포트 상단에 고정한다. 기존 데스크톱 프레임에서는
페이지 맨 위의 헤더가 약 24px 아래에 있다가 스크롤 시 위에 붙는 동작이 있었다.
앱 콘텐츠 폭과 중앙 정렬, 상단 안전 영역, 본문 시작 공간을 유지하고 팝업은 헤더 위에 표시한다.
텍스트 확대와 설정 내부 앵커 이동에서도 제목·조작부가 고정 헤더에 가리지 않아야 한다.

공통 PageShell의 고정 헤더 높이를 측정해 본문 여백에 반영하며, ResizeObserver 미지원 시 resize 폴백을 사용한다.
헤더 없는 셸은 추가 높이 0으로 처리한다. 본문 앵커에도 헤더 높이만큼 스크롤 여백을 둔다.
로컬 검증에서 360/390px 및 데스크톱의 스크롤 전후 헤더 top=0을 확인했고 가로 넘침은 없었다.
150% 글자 크기에서 설정 앵커 제목은 헤더 아래에 표시됐으며, 팝업 overlay/dialog(z40/50)는 고정 헤더(z30) 위에 표시된다.
관련 web app-routes 20개 및 공통 UI 82개 검사와 타입 검사를 통과했다.
