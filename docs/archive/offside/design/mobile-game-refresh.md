...
| 모션 | 화면·안내·대시보드 탭에 220ms 방향 전환, 공통 모달에 블러와 열림/닫힘. 키보드·모션 감소 시 생략. [상세 계약](app-like-motion.md) |
...
| SCR-017 계약 상태·제안 비교(T-4-007 재통합) | ScreenIntro 도입부, 현재 계약 요약·시장 제안 CompareCards 세로 카드, 제안 상세는 공개 조건·협상 카드와 하단 sticky 뒤로·거절·수락 | PRE_NEGOTIATION 협상 1회·제안 거절/수락·시장 제안 3종 이상 비교·재계약 사전 협상·revision 검증 | [offers](../../apps/web/src/routes/career.$careerId.offers.tsx), [contract](../../apps/web/src/routes/career.$careerId.contract.tsx) |
| SCR-019 이적 루머·관심(T-4-007 재통합) | 공통 이벤트 화면(SCR-013)의 ScreenIntro·상황 카드·선택 카드를 그대로 재사용(전용 변형 없음) | `presentation:'RUMOUR'` 이벤트는 0.2.0 콘텐츠 팩 미활성으로 현재 도달하지 않음(별도 결정 게이트, decision-log 참고) — 도달성이 열리면 `EVENT_SCREEN_OVERRIDES` 확장이 필요 | [career-route](../../apps/web/src/shared/career-route.ts), [event-screen](../../apps/web/src/shared/event-screen.tsx) |
| SCR-020 이적·임대 결과(T-4-007 재통합) | ScreenIntro 도입부, 소속 변화·새 계약·결과 사유 os-panel 카드, 하단 sticky 행동; 임대 복귀 결정은 별도 ScreenIntro·매입 옵션 카드·행동(옵션 1~2개에 맞춰 늘어나는 dock) | 원자 저장, 뒤로 가기로 재협상 불가, LOAN_RETURN RETURN/PERMANENT 분기, base OVR 불변 검증 | [transfer-result](../../apps/web/src/routes/career.$careerId.transfer-result.tsx) |
| 국가대표 소집·데뷔(T-4-007 재통합) | 소집/거절은 공통 이벤트 화면(SCR-013)을 그대로 재사용; 데뷔 챕터는 기존 챕터 화면에서 경기 맥락 캡션·스코어보드·풀타임 결과 섹션을 생략한 축소 구성 | NATIONAL_TEAM pending 라우팅(EVENT/INJURY와 동일 화면으로), 데뷔 챕터 컨텍스트 분기(`isNationalTeam`), 대시보드 타임라인의 소집·거절 기록 | [career-route](../../apps/web/src/shared/career-route.ts), [chapter](../../apps/web/src/routes/career.$careerId.chapter.tsx), [chapter-state](../../apps/web/src/shared/chapter-state.ts) |
...
### 통합 QA 결과 — 최초 화면 개편 `35ee004`, 2026-09-05

이 표는 최초 화면 개편 검증 기록이다. 이후 추가한 앱형 전환·제스처·모달과 최신 재검증 결과는 [앱형 모션 계약](app-like-motion.md)에 별도 기록한다.
