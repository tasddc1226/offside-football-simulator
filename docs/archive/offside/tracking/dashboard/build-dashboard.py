...
ROOT = os.environ.get("DASHBOARD_ROOT") or (
    os.path.abspath(os.path.join(_HERE, "..", "..", "..")) if os.path.basename(_HERE) == "dashboard" else os.path.expanduser("~/orca/workspaces/offside-football-simulator/main-2")
)
...
ETA = [
    ("2026-09-16 12:30", "열린 이슈 27건 분담 합의(Codex·Claude, 사용자 지시): A 출전 밸런스·B 첫 플레이·공통 파일 통합은 Codex/Sol, C 장기 콘텐츠 기획·검증·D 백로그·보드는 Claude(구현은 Sol). C 4건(#243·#246·#247·#169) 1.7.2/0.6.6 재판정·수용기준 이슈 코멘트 등록. 보드 '열린 이슈 분담' 표 신설. #197 진단 코멘트(NATIONAL_TEAM 항목 단독 63.6s)."),
    ("2026-09-16 01:09", "1.7.2/0.6.6 운영 승격 deploy 완료(dev 체크아웃 Claude 세션, D-82): #238 첫 계약 흐름 단축 dffe5b1(01:01 머지)·#240 승격 준비 1f18d06(01:04 머지) → preflight 34992863238·deploy 34993007861 01:09 완료. 플레이 리뷰 3세션·원작 조사로 이슈 #241~#259 19건 등록."),
    ("2026-09-15 18:58", "1.7.1/0.6.5 운영 승격 deploy 완료(Codex): #236 은퇴 아카이브 롤백 결함(RelationshipLogEntry step 0 거부) 430b129(18:47 머지)·#237 FAST 사건 노출 회귀 73f82c2(18:51 머지) → deploy 34955129986 18:58 완료. Codex 운영 17시즌 플레이로 은퇴·새 사건 인수."),
    ("2026-09-15 17:12", "0.6.4 운영 활성화 deploy 완료(Codex): T-7-034 사건 아크 #233 5b5ec03(16:28) → T-7-035 사건 모달 #234 e71c71c(16:31) → T-7-036 활성화 #235 eb95aef(17:05 머지), deploy 34945528602 17:12 완료. 신규 커리어만 1.7.0/0.6.4, 기존 커리어 보존."),
    ("2026-09-15 13:27", "1.7.0/0.6.3 운영 승격 deploy 완료(Codex, 사용자 승인): T-7-033 #232 533c3d4(13:21 머지) → CI 34928535257·preflight 34928569134·deploy 34928791797. Legacy 상위 밴드 22.7%(목표 32%) 수치를 알고 재튜닝 없이 승인 — 활성화 전 재보정 게이트 해소."),
    ("2026-09-15 11:54", "사용자 PR #224(T-7-025 현재 리그 맥락) 머지(9762b73) — Claude 리뷰·전체 체인 PASS + Sol e2e + 브라우저 인수. 이로써 사용자 세션 개선 묶음 #219·#221·#220·#222·#224가 전부 main에 들어갔습니다. 경계: 룰셋 1.7.0·팩 0.6.3은 등록만(운영 ACTIVE는 1.5.0/0.6.0 유지), Legacy 상위 밴드 22.7%(목표 32%)라 활성화 전 재보정 게이트, live API·Worker 미검증. 다음: 2라운드 밸런스 항목·1.7.0 활성화 여부는 사용자 결정"),
    ("2026-09-15 11:40", "사용자 PR #222(T-7-022 리그 원장, 룰셋 1.7.0·팩 0.6.3) 머지(80a7868) — Claude 독립 리뷰·회귀(1.5.0/1.6.1 seed 조인 전부 일치, 1.7.0 결정론 1000/1000)·전체 체인 PASS. 1.7.0은 등록만이고 ACTIVE 승격 없음: 상위 Legacy 밴드 22.7%(목표 32%)라 활성화 전 재보정 검토 게이트. 남은 사용자 PR은 #224(현재 리그 맥락, 리뷰 PASS·Sol 최종 통합 중)"),
    ("2026-09-15 11:27", "사용자 PR #220(T-7-024 커리어 피날레·은퇴 화면) 머지(3e0643e) — Claude 통합 리뷰·전체 체인 PASS + Sol scoped e2e PASS. #222(룰셋 1.7.0·팩 0.6.3 리그 원장)는 회귀 PASS이나 1.7.0 상위 밴드 22.7%(목표 32% 미달)는 활성화 전 Legacy 재보정 게이트로 남김, 최종 동기화 SHA 대기"),
    ("2026-09-15 11:17", "사용자 PR #221(T-7-023 선택 후속 영수증) 머지(60aed6a) — Claude 읽기전용 리뷰·전체 체인(e2e 제외) PASS 뒤 Codex가 브라우저 인수 합산해 머지. #222(룰셋 1.7.0·팩 0.6.3 리그 원장) 리뷰 PASS·대량 회귀 진행 중, #220 최종 통합 대기"),
    ("2026-09-15 10:25", "Codex(sol 통합)·Claude(밸런스/버전 독립 리뷰·러너 진단·tracking) 역할 분담 승인 — 1.7.0은 1.6.1 밸런스 계승, 팩 0.6.3 신규(0.6.2 불변) 결정(d)"),
    ("2026-09-14 20:58", "T-7-031 은퇴 심사 룰셋 필드 머지(PR #226 eb91347) — 1만 회 심사 은퇴 100%·p50 37세, D-80 1라운드 ② 완료"),
    ("2026-09-14 21:48", "T-7-032 Legacy 정책 1.2.0 머지(PR #227 cbd21fe) — 1만 회 상위 밴드 31.9%(포지션 편차 27pp는 2라운드), D-80 1라운드 ①②③ 완료 → 10만 회 최종 실행 중"),
    ("2026-09-15 01:06", "D-80 1라운드 최종 10만 회 완료(1.6.1/0.6.2, 실패 0) — 정점 29.3세·심사 은퇴 100%·상위 밴드 32.4%, 1만 회 판정과 일치. 2라운드 항목 사용자 결정 대기"),
    ("2026-09-14 19:48", "T-7-030 정점 연령 룰셋 1.6.1 머지(PR #225 fe2b3bf) — 1만 회 정점 29.3세, D-80 1라운드 ① 완료"),
    ...
    ("9/7 20:33", "PR #186 머지(241d275, 서명 프리필·결정 기한 셀). 1차 웨이브 T-7-001~010 전부 머지. 이슈 #151·#152 닫힘, 잔여 UI 2건은 #188. 남은 P1: T-7-011 → 운영 배포(사용자 지시)"),
    ("9/14 17:56", "T-7-021 대규모 시뮬 1차 완료·PR #223 891307a 머지: 1.5.0·1.6.0 각 1만 커리어 + 정책 random·first 2천(실패 0). 1.5.0 1만 회: 정점 34.1세, 상한 도달 91%·심사 은퇴 9%, 잠재 도달 89.7%, 1부 29.7%(5시즌 내 9%), 부상 1.24/시즌, RESERVE 41%, LEGEND+ICON 80%(DF 97·GK 92·FW 75·MF 56 포지션 편향). 1.6.0 차이 없음(면담 로직은 명령 없이는 비활성). 정책: random 1부 85%·RESERVE 50%, first 1부 0%. 보고 evidence/sim-baseline-10k-2026-09-14/. 조정 목표 제안표(Legacy 밴드·은퇴 심사·정점 연령 우선) 사용자 답 대기 → 룰셋 1.6.1."),
    ("9/14 17:44", "사용자 PR #213(e2e 안정화) cb513e6 머지 — 1.5.0 seed 드리프트로 깨진 SCR-012·SCR-015를 Sonnet 워커가 수정(d22f6bd). 오픈 PR 3건(#214·#217·#213) 모두 main 반영. 워크트리 정리(/clean-merged-worktrees) 29개 제거."),
    ("9/14 15:09", "T-7-020 career-sim 가속 PR #218 69de23b 머지: 도메인 SHA-256 공급자 주입(값 동일)·Node crypto 주입으로 커리어당 3.22초→1.11초(2.89배). 이어 T-7-021 1만 회 기준선 투입 — 워커는 10분 상한이라 nohup 드라이버 + 오케스트레이터 대기로 전환."),
    ("9/14 13:58", "T-7-019(PR 표기 T-7-017; 사용자 #213과 ID 충돌로 재부여) career-sim 머지(PR #216 c78b6f1): 워커 1차 + 후속 2회(은퇴 심사 강제 처리·retireReason·CSV 결정론, 병렬 합치기 타입 수정). 1.5.0 기준선 200커리어: OVR 정점 33~36세, 은퇴 심사 14/200(나머지 49세까지 현역), 1부 도달 33%, 부상 1.24/시즌, 구단 이동 3.0회, 시즌 40% RESERVE·출전 점유 42%, LEGEND·ICON 155/200. 조정 목표는 사용자 확인 대기(룰셋 1.5.1)."),
    ("9/14 13:11", "사용자 지시(12:58) \"UI 없이 시뮬레이션을 많이 돌려 최적값 세팅\" → D-79 헤드리스 커리어 일괄 시뮬레이션 CLI 브리프 T-7-017 작성·Sonnet 워커 투입(tooling/scripts/career-sim.ts, 도메인 simulate() 직접 호출, 정책 3종·병렬·CSV·결정론 검사). 타당성 실험: engine-client 경로 25시즌 142초(structuredClone 48%). 수치 조정은 결과 뒤 룰셋 1.5.1로 별도. 로컬라이제이션 기획 대기."),
    ("9/14 12:43", "운영 배포 완료 run 34803426999(643cd2a): 실시간 플레이 중 배지(D-78)·K리그식 룰셋 1.5.0/0.6.0 승격(plan=activate)·시즌=연도. 배포 후 검증·QA 플레이(QA배포1244)·배지 표시 통과."),
    ("9/14 12:35", "사용자 지시로 열린 PR 순차 머지: #208 K리그 구조 ceff29c → #215(#210 재적용, 1.5.0/0.6.0 승격 준비) 4b29c4d → #209 시즌=연도 139d1ca → #212 T-7-015 배지 c77375f. 합본 전체 체인 통과. main 4b29c4d로 preflight → DEPLOY_PRODUCTION 진행(시즌 1 manifest 1.5.0/0.6.0 승격)."),
    ("9/14 11:57", "T-7-014 PR #211 b05867b 머지(체인 통과, e2e 새 실패 0 — main 기존 16건은 단독 실행에서도 실패, #207 기록) → T-7-015(네비바 배지) Sonnet 워커 투입. 셀프호스트 러너 오프라인(잠든 맥북)이라 이 맥에 임시 러너 offside-mac-arm64-tmp 등록."),
    ("9/14 11:35", "D-78 실시간 플레이 중 인원 배지: 브리프 T-7-014(api·contracts)·T-7-015(web·ui css) 작성, 사용자 승인 → T-7-014 Sonnet 워커 투입. 9/8~9/13 다른 세션: 룰셋 1.4.0/팩 0.5.1 운영 승격(D-75), Wave C·D·E, UX-012·013, 시뮬레이션 모드 제거(D-77), 운영 배포 5회"),
    ("9/7 21:50", "운영 배포 완료: preflight 34123657025 → deploy run 34123837271(308f9bc) 성공. API 6b78751f…·web a37ab471…, manifest noop(1.3.0/0.5.0 유지). 21:51 health·current·CORS·웹·360px 플레이(QA배포2151) 검증 통과. 오늘 머지 13건 운영 반영"),
    ("9/7 21:44", "PR #189 머지(45ff7b5, T-7-011 e2e 정합). 오케스트레이터 e2e 100/3/7·새 실패 0(main 60 실패 → 3). 이슈 #179 닫힘, 후속 #190(허용 필터 제거·SCR-012·부하 안정화)·#180(axe 잔여 2종). 오늘 워커 13건 전부 머지 → 운영 preflight·deploy로"),
    ("9/7 20:41", "운영 preflight: 1차(run 34117318240, 5086ad0)는 직후 문서 푸시로 main이 움직여 SHA 검사에서 중단(자업자득), 2차(run 34117660472, 1e9ed8f) 성공 — 릴리스 입력·API 호환 테스트·운영 web 빌드·D1 bookmark·시즌 메타데이터·집계 읽기 모두 통과. 스테이징에서 배포 후 검증 스크립트(온보딩→생성→저장→새로고침→허브) 예행 OK. T-7-011 워커는 sync.spec 5건 통과 뒤 a11y.spec 재검증 중"),
...
DECISIONS = [
    ("D-79 밸런스 조정용 헤드리스 커리어 일괄 시뮬레이션 CLI(9/14 12:58, 사용자 요청)", "도구 먼저(T-7-019 career-sim, T-7-020 가속, T-7-021 1만 회 기준선·sim:report — 모두 머지). 1.5.0 1만 회로 조정 후보 확정(Legacy 밴드 포지션 편향·은퇴 미강제·늦은 정점·높은 잠재 도달·RESERVE 정체·부상). 조정은 1.6.0 위 새 룰셋 1.6.1로, A/B는 같은 seed 접두어, 1만 회씩 확인 뒤 10만 회 최종.  도메인 simulate() 직접 호출(engine-client 경로는 복제·해시 비용으로 25시즌 142초), legacy-population 선택 헬퍼 재사용, 정책 first·random·opportunity, --jobs 병렬, careers/seasons CSV·summary.json·failures.csv, --verify 결정론. 룰셋 수치는 건드리지 않고 조정은 새 버전 1.5.1로 별도(지표·목표값은 사용자 확인)."),
    ("D-78 상단 네비바 실시간 \"N명 플레이 중\" 배지(9/14 11:20, 사용자 요청)", "새 바인딩 없이 D1 sessions.last_seen_at + 인덱스 1개. 하트비트는 탭이 보일 때만 60초(서버 30초 스로틀), GET /v1/presence는 공개·30초 캐시·5분 창 COUNT(DISTINCT profile_id). 배지는 0·미수신이면 숨김, ui는 CSS만. T-7-014(api) PR #211 b05867b 11:56 머지 → T-7-015(web) 11:57 투입 → 머지 뒤 운영 배포. 대규모(1만 명 동시)가 오면 Durable Object로 이전."),
    ("D-71 P1 마무리 뒤 운영 배포, 코드만(9/7 20:30, 사용자 지시)", "남은 P1 #179(T-7-011) 머지·게이트 복원 뒤 최종 main SHA로 preflight → DEPLOY_PRODUCTION(입력은 9/7 01:52 배포와 동일: 시즌 시작 2026-09-05T15:00:00Z, 종료 비움, cs_season_1). 릴리스 워크플로가 시즌 manifest 1.3.0/0.5.0을 하드코딩 검사하므로 코드만 올리고 운영 룰셋은 1.3.0 유지 — 1.4.0 승격은 별도 작업·사용자 승인. 20:34 main 5086ad0 preflight 실행. → 21:50 deploy run 34123837271(308f9bc) 성공, 배포 후 검증 통과(D-71 완료)."),
