...
| T-7-003 | api | INSERT-only 로컬 QA 서비스 시즌 seed `svc_recovery_rules_qa`(1.4.0/0.5.0, isTest). 운영 포인터·`season-version-compatibility.ts`는 사용자 결정 뒤 별도 | in-review | [브리프](briefs/T-7-003.md) 이슈 #140. T-7-001 뒤, T-7-002와 병행. port 5263 → 17:21 T-7-002와 함께 투입. 17:26 브랜치 `T-7-003-recovery-qa-season-seed` push 확인 → 17:58 워커 완료(`201f57e`, 로컬 D1 적용·중복 없음 확인), PR #182(REST 개설). 체인 대기 → 18:37 체인은 check:contrast까지 녹색(api 217·domain 925 등 전부 통과)이나 e2e가 webServer 30s 기동 타임아웃(load 200+)으로 시작 못 함 → e2e만 재실행 큐(run8, #184·#183 뒤) |
...
| T-7-007 | web | 서명 이름 프리필(placeholder 제거), 제안 비교 "결정 기한" 셀 짧은 값 + 상단 캡션, 카드 높이 정렬 | in-review | [브리프](briefs/T-7-007.md) 이슈 #151·#152. T-7-004 머지 뒤(`offers.tsx`). port 5267 → 18:29 #174 머지 직후 Sonnet 5 워커 투입(worktree 격리, 포트 5267, 워커는 lint·typecheck·web 단위·transfer.spec 1개·360px 카드 높이 측정까지) → 20:09 워커 완료(`abb41be`, 브랜치 `…-w`: 서명 입력 `signerName` 프리필+보조 문구 `aria-describedby`, 결정 기한 셀 `결정 N번 안`/`제한 없음` + 목록 상단 캡션, 새 헬퍼 `offerDecisionDeadlineLabel`), PR #186(REST 개설). 워커 측정: 360px 카드 높이 464/440/440 — 잔여 24px는 "제안" 행 2줄 줄바꿈(범위 밖, 후속 후보). 체인 큐 run11(포트 5267) |
...
| T-7-011 | web(e2e) | 전체 e2e 스위트를 9/6 UX 개편 이후 앱과 정합(main 60/110 실패 핫픽스): 랜딩·인트로·배경 단계·프리시즌 진입·진행 루프·앱 셸 폭. 테스트 완화 없음 | in-progress | [브리프](briefs/T-7-011.md) 이슈 #179, D-70. port 5272. 17:20 투입 (Workflow Sonnet 5, worktree 격리) |
| T-7-012 | ui | `.os-eyebrow` 색 대비 axe 위반(12화면) 수정 + `check:contrast` 쌍 추가 | in-progress | [브리프](briefs/T-7-012.md) 이슈 #180. port 5273. 17:20 투입 (Workflow Sonnet 5) → 18:10 워커가 PR #183 개설(훅 통과). 검증은 워크플로 완료 보고 뒤 → 20:08 오케스트레이터 360px 측정: /settings eyebrow 라이트 4.40:1(`--os-line` 초록) → 10.66:1(`--os-accent` 남색), 다크 8.86 → 9.08. 캡처 shots183/. 체인 실행 중(run10) |
| T-7-013 | web | SCR-009 제안 비교 `<dl>` 구조 axe 위반(`definition-list`·`only-dlitems`) 수정 | in-review | [브리프](briefs/T-7-013.md) 이슈 #180. port 5274. 17:20 투입 (Workflow Sonnet 5) → 19:5x 워커 완료(`5ec4a86`, `<dd>` 안으로 delta 캡션 이동 + 기존 테스트 파일에 dl 구조 케이스 1개), PR #185(워커가 직접 개설). 워커 보고: definition-list·only-dlitems 위반 0, e2e 60/43/7로 baseline과 동일. 체인 큐 run9(#185, 포트 5274) |
