...
| T-7-007 | web | 서명 이름 프리필(placeholder 제거), 제안 비교 "결정 기한" 셀 짧은 값 + 상단 캡션, 카드 높이 정렬 | in-review | [브리프](briefs/T-7-007.md) 이슈 #151·#152. T-7-004 머지 뒤(`offers.tsx`). port 5267 → 18:29 #174 머지 직후 Sonnet 5 워커 투입(worktree 격리, 포트 5267, 워커는 lint·typecheck·web 단위·transfer.spec 1개·360px 카드 높이 측정까지) → 20:09 워커 완료(`abb41be`, 브랜치 `…-w`: 서명 입력 `signerName` 프리필+보조 문구 `aria-describedby`, 결정 기한 셀 `결정 N번 안`/`제한 없음` + 목록 상단 캡션, 새 헬퍼 `offerDecisionDeadlineLabel`), PR #186(REST 개설). 워커 측정: 360px 카드 높이 464/440/440 — 잔여 24px는 "제안" 행 2줄 줄바꿈(범위 밖, 후속 후보). 체인 큐 run11(포트 5267) → 20:13 리뷰 후속(만료 상태가 "결정 0번 안"으로 표시) 수정 워커 `a5c500b`(`만료` 표시 + 테스트 3건) |
