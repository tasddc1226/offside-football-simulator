...
| 이슈 #105 | web | 핵심 경기 제목에 내부 TAG(프로_데뷔) 노출 (P2) | todo | PR #103 후속 → T-7-005 브리프에 포함 |
...
| 6케이스 리뷰 | qa | 운영 6케이스 플레이 리뷰(GK·CB·WG·CM·ST·DM × 성별·국적·출발 카드·첫 갈림길·전략 직교, Sonnet 서브에이전트 6명 + ego-browser 생성 잠금 직렬화, 57시즌·자연 은퇴 1건). 무출전 고착 5/6, 협상 버튼 무결과 4/6, enum 노출 4/6, #105 3/6(재사용 챕터 한정), 결산 표 모순·승격 표기 불변은 신규. 노쇠·은퇴·Legacy 정상 | completed | 2026-09-07 13:47 [기록](../qa/2026-09-07-production-six-cases/README.md) |
| T-7-001 | content | 룰셋 1.4.0(1.3.0 복사 + 5곳): 회복 `zeroMinutesConsecutiveSeasons` 2→1, `managerTrustPromiseBreach` -8→0, 선택 키 `roleProposal.acceptedRoleUpdatesPromise: true`·`declineDowngradeTrustDelta: 0`, 로더·팩 0.5.0 호환·`retirement-artifacts` 짝 | completed | [브리프](briefs/T-7-001.md) 이슈 #140, D-67. 1.3.0 바이트 불변. port 5261. 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #173(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 → 17:20 검증 완료: 체인 녹색, e2e 실패 집합 baseline과 동일 → PR #173 squash 머지(`ab8a88b`). 이슈 #140은 T-7-002·003 뒤 재확인 |
| T-7-002 | domain | 1.4.0 키를 읽는 도메인: 역할 제안 수락 시 `contract.rolePromise`·출전 약속 갱신(현재는 한 경기 뒤 소멸), 하향 제안 거절 무벌점, 미이행 벌점 0에서도 로그·태그·카운터 유지, 0분 1시즌 회복 제안 테스트 고정 | completed | [브리프](briefs/T-7-002.md) 이슈 #140. T-7-001 머지 뒤. 1.3.0 골든·해시 기대값 불변. port 5262 → 17:21 #173 머지 직후 Sonnet 5 워커 투입(worktree 격리, 워커는 lint·typecheck·패키지 테스트까지만, 전체 체인은 오케스트레이터 순차) → 17:58 워커 완료(`b0bcdb0`, domain 933 테스트·골든 불변), PR #181(REST 개설). 체인 대기 → 18:31 체인 녹색(domain 933·web 553 등 전부 통과), e2e 새 실패 0 → 18:32 PR #181 squash 머지(`630637b`). 이슈 #140은 #182 머지·로컬 QA 시즌 플레이 확인 뒤 닫는다 |
| T-7-003 | api | INSERT-only 로컬 QA 서비스 시즌 seed `svc_recovery_rules_qa`(1.4.0/0.5.0, isTest). 운영 포인터·`season-version-compatibility.ts`는 사용자 결정 뒤 별도 | completed | [브리프](briefs/T-7-003.md) 이슈 #140. T-7-001 뒤, T-7-002와 병행. port 5263 → 17:21 T-7-002와 함께 투입. 17:26 브랜치 `T-7-003-recovery-qa-season-seed` push 확인 → 17:58 워커 완료(`201f57e`, 로컬 D1 적용·중복 없음 확인), PR #182(REST 개설). 체인 대기 → 18:37 체인은 check:contrast까지 녹색(api 217·domain 925 등 전부 통과)이나 e2e가 webServer 30s 기동 타임아웃(load 200+)으로 시작 못 함 → e2e만 재실행 큐(run8, #184·#183 뒤) → 20:17 e2e 재실행 새 실패 0 → 20:18 PR #182 squash 머지(`323adad`). 이슈 #140은 로컬 QA 시즌 플레이 확인 뒤 닫는다 |
| T-7-004 | web | 협상 결과 배너 항상 표시(`contract.tsx` `pending===null` 이른 반환 수정), `negotiationDisabledReason` 사유 캡션·`aria-describedby`, 결과 패널 포커스, 회복 안내 문구 정책값 기반 | completed | [브리프](briefs/T-7-004.md) 이슈 #141, D-69. port 5264 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #174(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 — 리뷰 후속 `isOfferNonNegotiable` 정확 조건 `ce0b897` 반영, 전체 체인 대기(#176→#178→#175→#174 순) — 17:55 체인: web 단위 테스트 2건 실패(주석 "이슈 #141"이 hex 리터럴 검사에 걸림 + 부하 타임아웃) → 수정 워커 `c04c925` push, 체인 재실행 큐. 360px 시즌 완주 후 이적시장 잔류 제안: 협상 3버튼 비활성 + 사유 캡션 3줄 + 요약 캡션, `aria-describedby` 정합 확인 → 18:15 재실행 체인이 단위 테스트 부하 플레이크(대표팀 시드 탐색 120s 타임아웃, load 90+)로 중단 → 기존 워크트리에서 남은 단계만 재개(18:17) → 18:24 재개 체인 녹색(플레이크 파일 단독 7/7). e2e 61 실패 중 baseline 대비 새 실패 2건(`a11y:78` /legal/privacy scrollable-region-focusable·/settings eyebrow 대비)은 현재 main 워크트리에서도 동일 재현(각 3회) → main 자체 실패로 판정(이슈 #180에 기록) → 18:28 PR #174 squash 머지(`b76104a`), 이슈 #141 닫힘 |
| T-7-005 | web | 표시 라벨 정합: `relationshipReasonLabel`(PROMISE_BREACH 등 16키+LEGACY_+폴백), 챕터 TAG 제목(`프로_데뷔`→'데뷔 이후의 무대', 폴백 '핵심 경기'), 결산 표 "선발 등급 vs 계약 역할 약속" 분리 + 캡션 | completed | [브리프](briefs/T-7-005.md) 이슈 #142·#143·#105. port 5265 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #175(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 — 리뷰 후속 경쟁 순위 `(경쟁 순위 N위)` 복원 `48db2d4` 반영, 전체 체인 대기 — 17:50 체인 녹색, e2e 새 실패 0(baseline 1건 통과로). 360px 시즌 완주: 결산 표 4행(선발 등급 → 종료 (경쟁 순위 N위)·계약 역할 약속·실제 출전 등급·약속 이행) DOM 확인, 선수·기록 탭 내부 키 노출 없음, 펼친 화면 캡처 진행 중 → 18:03 PR #175 squash 머지(`a194efa`), 이슈 #105·#142·#143 닫힘. T-7-010 투입 |
| T-7-006 | ui | 액션 독 어포던스: 상단 페이드·그림자, `.os-shell-main` `scroll-padding-bottom`, 불투명 배경 (CSS만) | completed | [브리프](briefs/T-7-006.md) 이슈 #146. port 5266 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #176(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 — 17:24 전체 체인 실행 중(오케스트레이터, origin/main `ab8a88b` 머지 워크트리) → 17:35 검증 완료: 체인 녹색, e2e 새 실패 0(baseline 실패 1건 통과로), 360px before/after 관찰(불투명 배경·페이드·그림자, 마지막 카드 포커스 하단 739→644px) → PR #176 squash 머지(`1da6a50`). 후속 후보: 360px 독 높이 113px라 96px 패딩으로는 17px 겹침 |
| T-7-007 | web | 서명 이름 프리필(placeholder 제거), 제안 비교 "결정 기한" 셀 짧은 값 + 상단 캡션, 카드 높이 정렬 | completed | [브리프](briefs/T-7-007.md) 이슈 #151·#152. T-7-004 머지 뒤(`offers.tsx`). port 5267 → 18:29 #174 머지 직후 Sonnet 5 워커 투입(worktree 격리, 포트 5267, 워커는 lint·typecheck·web 단위·transfer.spec 1개·360px 카드 높이 측정까지) → 20:09 워커 완료(`abb41be`, 브랜치 `…-w`: 서명 입력 `signerName` 프리필+보조 문구 `aria-describedby`, 결정 기한 셀 `결정 N번 안`/`제한 없음` + 목록 상단 캡션, 새 헬퍼 `offerDecisionDeadlineLabel`), PR #186(REST 개설). 워커 측정: 360px 카드 높이 464/440/440 — 잔여 24px는 "제안" 행 2줄 줄바꿈(범위 밖, 후속 후보). 체인 큐 run11(포트 5267) → 20:13 리뷰 후속(만료 상태가 "결정 0번 안"으로 표시) 수정 워커 `a5c500b`(`만료` 표시 + 테스트 3건) → 20:31 체인 녹색, e2e 새 실패 0, 360px 완주(캡션·기한 셀·프리필 확인) → 20:33 PR #186 squash 머지(`241d275`), 이슈 #151·#152 닫힘, 잔여 UI 2건 → 이슈 #188 |
| T-7-008 | ui | RadioGroup Enter 키 선택(Radix 기본은 Space만) | completed | [브리프](briefs/T-7-008.md) 이슈 #149. port 5268 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #177(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 → 17:32 검증 완료: 체인 녹색, e2e 새 실패 1건(service-season:97 생성 흐름 URL 대기 타임아웃, load 111)은 키보드 미사용 흐름이라 부하 플레이크 판정(D-70), 실제 브라우저에서 Enter·Space·화살표 선택 확인 → PR #177 squash 머지(`58ab25a`), 이슈 #149 닫힘. 조용한 머신에서 spec 단독 재실행 큐 |
| T-7-009 | web | 대표팀 챕터 결과 요약 섹션(판단 결과·효과·태그·데뷔, 스코어 꾸미지 않음) | completed | [브리프](briefs/T-7-009.md) 이슈 #150. port 5269 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #178(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 — 체인 대기(#176 뒤) — 17:38 체인: web 단위 테스트 1건(phase4-seed-reachability NATIONAL_TEAM 120s, load 180) 타임아웃으로 중단 → 재실행 큐(큐 스크립트 env 누락으로 18:00 재기동). jsdom 독립 렌더: `<dl>` div/dt/dd 구조, 소집·판단 결과·데뷔 행, 스코어·평점 문구 없음 확인 → 18:11 재실행 체인 녹색(web 544/544), e2e 새 실패 0 → PR #178 squash 머지(`3101d6c`), 이슈 #150 닫힘 |
| T-7-010 | web | "승격"→"승격권" 문구 정합 + 결산 캡션(리그 이동 없음). 실제 리그 이동은 Phase 8 후보로 보류 | completed | [브리프](briefs/T-7-010.md) 이슈 #144, D-68. T-7-005 머지 뒤(`season-result.tsx`). port 5270 → 18:04 #175 머지 직후 Sonnet 5 워커 투입(worktree 격리) → 18:13 완료(`dc45ea3`, 문구 8곳 + `careerTagLabel` 웹 오버라이드 + 결산 캡션, team-flavor 2곳 포함), PR #184(REST 개설). 체인 대기(#174→#181→#182 뒤) → 20:06 체인 녹색(부하 타임아웃 2파일 단독 재실행 통과), e2e 새 실패 0 → 20:07 PR #184 squash 머지(`16e0a98`), 이슈 #144 닫힘(리그 이동은 D-68 보류) |
| T-7-011 | web(e2e) | 전체 e2e 스위트를 9/6 UX 개편 이후 앱과 정합(main 60/110 실패 핫픽스): 랜딩·인트로·배경 단계·프리시즌 진입·진행 루프·앱 셸 폭. 테스트 완화 없음 | completed | [브리프](briefs/T-7-011.md) 이슈 #179, D-70. port 5272. 17:20 투입 (Workflow Sonnet 5, worktree 격리) 21:44 PR #189 머지 `45ff7b5`(오케스트레이터 e2e 100/3/7, 새 실패 0, 부하 27~44). 이슈 #179 닫힘, 후속 #190·#180 |
...
진행 중 워커 없음 (2026-09-07 오후 — 이 세션은 문서·QA 전담. 12:48 운영 3관점 플레이 리뷰(Sonnet 서브에이전트 3명, ego-browser) 완료, 13:47 운영 6케이스 플레이 리뷰(Sonnet 6명, 57시즌·자연 은퇴 1건) 완료, 14:10 이슈 #140~#172 등록, 14:50 1차 웨이브 브리프 T-7-001~010 작성, 14:51 사용자 승인으로 T-7-001·004·005·006·008·009 Sonnet 5 워커 6명 투입(worktree 격리, e2e 5261~5269). T-7-002·003은 T-7-001 머지 뒤, T-7-007은 T-7-004 뒤, T-7-010은 T-7-005 뒤. 17:05 워커 6명 완료 → PR #173~#178(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 17:11 origin/main e2e baseline 60/110 실패 확인(이슈 #179·#180, D-70) → 17:20 핫픽스 T-7-011·012·013 투입, PR #173·#177·#176·#178 체인은 baseline 대비 판정으로 순차 검증 중, #174·#175는 리뷰 후속 워커 진행 중. 17:20 PR #173 머지(`ab8a88b`, e2e 집합 baseline 동일) → 17:21 T-7-002·003 투입(1차 웨이브 b, 워커는 단위 검증까지만·체인은 오케스트레이터 순차). 17:32 PR #177 머지(`58ab25a`, e2e 새 실패 1건 부하 플레이크 판정 + 브라우저 Enter 확인). 17:35 PR #176 머지(`1da6a50`, e2e 새 실패 0 + 360px 관찰). 17:38 #178 체인은 부하 타임아웃 1건으로 중단 → 재실행. 17:50 #175 체인 녹색(e2e 새 실패 0). 17:55 #174 체인은 주석 "이슈 #141" hex 검사 위반으로 실패 → 수정 워커 `c04c925`. 17:58 1차 웨이브 b 완료 → PR #181(T-7-002)·#182(T-7-003). 18:00 체인 순서 #178→#174→#181→#182. 18:03 PR #175 머지(`a194efa`, 360px 결산 표·라커룸 확인) → T-7-010 투입. 18:11 PR #178 머지(`3101d6c`, 재실행 체인 녹색·e2e 새 실패 0·jsdom 독립 렌더). 18:28 PR #174 머지(`b76104a`) → T-7-007 투입. 18:32 PR #181 머지(`630637b`). 19:56 머신 load 200+(다른 세션 다수)로 #184 체인이 80분째 단위 테스트 단계 — 큐는 #184→#183→#182(e2e만)→#185 순으로 직렬 유지. T-7-013 PR #185 개설. 20:07 PR #184 머지(`16e0a98`). 20:09 T-7-007 완료 → PR #186(리뷰 후속 1건: 기한 셀 만료 문구, 수정 워커 투입). 20:12 PR #183 머지(`5e5e2c4`). 20:18 PR #182 머지(`323adad`). 20:26 PR #185 머지(`2074303`). 20:33 PR #186 머지(`241d275`) — 1차 웨이브 T-7-001~010 전부 머지. 남은 것: T-7-011(워커 진행 중), 운영 배포(사용자 지시 20:30: P1 마무리 뒤 운영 배포). 핫픽스 T-7-011·013 워커 진행 중. main 자체 a11y 정적 화면 실패 2건(/legal/privacy·/settings) 발견 → 이슈 #180. #174는 360px 임대 제안에서 역할 협상 1회 → 결과 패널(협상 항목·변경 전/후·상태)·포커스 이동·"더 이상 열려 있지 않음" 확인. 9/5 밤~9/7 새벽 코드 작업은 사용자 세션(Codex·Sol·Luna)이 PR #91~#139로 직접 머지·배포. 남은 항목은 Phase 7 표와 사용자 액션 U-014·U-004·U-005) 21:44 T-7-011(PR #189, `45ff7b5`)까지 머지돼 오늘 워커 작업 13건(1차 웨이브 10 + 핫픽스 3)이 전부 main에 들어갔다(이슈 #179 닫힘, 잔여는 #190·#180). 이어서 사용자 지시(D-71)에 따라 이 문서 커밋을 최종 main SHA로 운영 preflight → DEPLOY_PRODUCTION을 실행한다. 21:50 **운영 배포 완료**(deploy run 34123837271, `308f9bc`) — 오늘 머지분 13건이 offside-lab.com에 반영됐고(시즌 룰셋은 1.3.0 유지, manifest noop) 배포 후 health·current·CORS·웹·플레이 검증을 통과했다. 남은 것: axe 잔여 #180, e2e 정리 #190, 잔여 UI #188, #104, 로컬 QA 시즌에서 1.4.0 회복 규칙 플레이 확인(#140 후속) 뒤 운영 1.4.0 승격은 사용자 결정, 2차 웨이브 #145·#147·#148.
...
| Phase 7 1차 웨이브·핫픽스 (이 세션, Sonnet 워커 13명) | PR #173~#178, #181~#186, #189 | 2026-09-07 17:20 ~ 21:50 | 룰셋 1.4.0 회복 규칙(데이터·도메인·QA seed), 운영 QA 개선 7건, e2e 핫픽스 3건. 운영 재배포 `308f9bc`(run 34123837271, 시즌 룰셋 1.3.0 유지) |
