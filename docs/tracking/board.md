...
| T-0-010 | GitHub Actions CI, 웹 Static Assets·API Workers preview 배포, staging migration | ADR-007 | in-progress | PR #46 `abf9bfa` 머지(2026-09-04 10:07, 사용자 승인). main 푸시 staging 자동 배포·smoke 확인 뒤 completed. [브리프](briefs/T-0-010.md) |
...
| T-2-012 | api + web + platform | LINE TEST 준비: 서비스 시즌 포인터·`svc_line_test` 테스트 보관함·분석 이벤트 수집·스테이징 시드 | T-2-011, T-0-010 | 4 | in-progress | `T-2-012-line-test`, [브리프](briefs/T-2-012.md)(2026-09-04 10:07 투입, D-54·D-55, E2E_PORT 5189) |
...
|---|---|---|---|---|---|---|
| T-3-001 | A | domain + contracts + content | 계약·제안 v2 타입, clubHistory, 제안 상태기계, 타임라인 kind 예약(양 트랙), DSL contract.*, CON payload 스키마 | U-012 | in-progress | `T-3-001-contract-types`, [브리프](briefs/T-3-001.md)(2026-09-04 10:07 투입, U-012 승인 뒤, E2E_PORT 5188) |
...
| T-3-006 | A | content | 루머·잔류·에이전트 이벤트, 협상·이적 문구, 팀 풀 확장(열린 질문) | T-3-001 | todo | U-013 가상 구단 12개로 확장(U-013 확정) |
...
| ID | 워커 | 시작 | 상태 |
|---|---|---|---|
| T-2-012 | Sonnet 5, Orca 워크트리 `T-2-012-line-test` | 2026-09-04 | LINE TEST 준비(api·web·platform) — D-54 서비스 시즌 포인터·D-55 분석 이벤트 |
| T-3-001 | Sonnet 5, Orca 워크트리 `T-3-001-contract-types` | 2026-09-04 | Phase 3·4 타입 슬라이스(domain·contracts·content 스키마) — 머지 뒤 T-4-001·T-3-002·T-3-006 |
