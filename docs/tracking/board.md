...
| T-7-002 | domain | 1.4.0 키를 읽는 도메인: 역할 제안 수락 시 `contract.rolePromise`·출전 약속 갱신(현재는 한 경기 뒤 소멸), 하향 제안 거절 무벌점, 미이행 벌점 0에서도 로그·태그·카운터 유지, 0분 1시즌 회복 제안 테스트 고정 | in-progress | [브리프](briefs/T-7-002.md) 이슈 #140. T-7-001 머지 뒤. 1.3.0 골든·해시 기대값 불변. port 5262 → 17:21 #173 머지 직후 Sonnet 5 워커 투입(worktree 격리, 워커는 lint·typecheck·패키지 테스트까지만, 전체 체인은 오케스트레이터 순차) |
| T-7-003 | api | INSERT-only 로컬 QA 서비스 시즌 seed `svc_recovery_rules_qa`(1.4.0/0.5.0, isTest). 운영 포인터·`season-version-compatibility.ts`는 사용자 결정 뒤 별도 | in-progress | [브리프](briefs/T-7-003.md) 이슈 #140. T-7-001 뒤, T-7-002와 병행. port 5263 → 17:21 T-7-002와 함께 투입. 17:26 브랜치 `T-7-003-recovery-qa-season-seed` push 확인 |
| T-7-004 | web | 협상 결과 배너 항상 표시(`contract.tsx` `pending===null` 이른 반환 수정), `negotiationDisabledReason` 사유 캡션·`aria-describedby`, 결과 패널 포커스, 회복 안내 문구 정책값 기반 | review | [브리프](briefs/T-7-004.md) 이슈 #141, D-69. port 5264 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #174(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 — 리뷰 후속 `isOfferNonNegotiable` 정확 조건 `ce0b897` 반영, 전체 체인 대기(#176→#178→#175→#174 순) |
...
| T-7-009 | web | 대표팀 챕터 결과 요약 섹션(판단 결과·효과·태그·데뷔, 스코어 꾸미지 않음) | review | [브리프](briefs/T-7-009.md) 이슈 #150. port 5269 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #178(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 — 체인 대기(#176 뒤) |
