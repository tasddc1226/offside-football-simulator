...
| T-7-005 | web | 표시 라벨 정합: `relationshipReasonLabel`(PROMISE_BREACH 등 16키+LEGACY_+폴백), 챕터 TAG 제목(`프로_데뷔`→'데뷔 이후의 무대', 폴백 '핵심 경기'), 결산 표 "선발 등급 vs 계약 역할 약속" 분리 + 캡션 | review | [브리프](briefs/T-7-005.md) 이슈 #142·#143·#105. port 5265 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #175(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 — 리뷰 후속 경쟁 순위 `(경쟁 순위 N위)` 복원 `48db2d4` 반영, 전체 체인 대기 — 17:50 체인 녹색, e2e 새 실패 0(baseline 1건 통과로). 360px 시즌 완주: 결산 표 4행(선발 등급 → 종료 (경쟁 순위 N위)·계약 역할 약속·실제 출전 등급·약속 이행) DOM 확인, 선수·기록 탭 내부 키 노출 없음, 펼친 화면 캡처 진행 중 |
