...
| T-7-001 | content | 룰셋 1.4.0(1.3.0 복사 + 5곳): 회복 `zeroMinutesConsecutiveSeasons` 2→1, `managerTrustPromiseBreach` -8→0, 선택 키 `roleProposal.acceptedRoleUpdatesPromise: true`·`declineDowngradeTrustDelta: 0`, 로더·팩 0.5.0 호환·`retirement-artifacts` 짝 | review | [브리프](briefs/T-7-001.md) 이슈 #140, D-67. 1.3.0 바이트 불변. port 5261. 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #173(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 |
| T-7-002 | domain | 1.4.0 키를 읽는 도메인: 역할 제안 수락 시 `contract.rolePromise`·출전 약속 갱신(현재는 한 경기 뒤 소멸), 하향 제안 거절 무벌점, 미이행 벌점 0에서도 로그·태그·카운터 유지, 0분 1시즌 회복 제안 테스트 고정 | todo | [브리프](briefs/T-7-002.md) 이슈 #140. T-7-001 머지 뒤. 1.3.0 골든·해시 기대값 불변. port 5262 |
| T-7-003 | api | INSERT-only 로컬 QA 서비스 시즌 seed `svc_recovery_rules_qa`(1.4.0/0.5.0, isTest). 운영 포인터·`season-version-compatibility.ts`는 사용자 결정 뒤 별도 | todo | [브리프](briefs/T-7-003.md) 이슈 #140. T-7-001 뒤, T-7-002와 병행. port 5263 |
| T-7-004 | web | 협상 결과 배너 항상 표시(`contract.tsx` `pending===null` 이른 반환 수정), `negotiationDisabledReason` 사유 캡션·`aria-describedby`, 결과 패널 포커스, 회복 안내 문구 정책값 기반 | review | [브리프](briefs/T-7-004.md) 이슈 #141, D-69. port 5264 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #174(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 |
| T-7-005 | web | 표시 라벨 정합: `relationshipReasonLabel`(PROMISE_BREACH 등 16키+LEGACY_+폴백), 챕터 TAG 제목(`프로_데뷔`→'데뷔 이후의 무대', 폴백 '핵심 경기'), 결산 표 "선발 등급 vs 계약 역할 약속" 분리 + 캡션 | review | [브리프](briefs/T-7-005.md) 이슈 #142·#143·#105. port 5265 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #175(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 |
| T-7-006 | ui | 액션 독 어포던스: 상단 페이드·그림자, `.os-shell-main` `scroll-padding-bottom`, 불투명 배경 (CSS만) | review | [브리프](briefs/T-7-006.md) 이슈 #146. port 5266 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #176(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 |
...
| T-7-008 | ui | RadioGroup Enter 키 선택(Radix 기본은 Space만) | review | [브리프](briefs/T-7-008.md) 이슈 #149. port 5268 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #177(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 |
| T-7-009 | web | 대표팀 챕터 결과 요약 섹션(판단 결과·효과·태그·데뷔, 스코어 꾸미지 않음) | review | [브리프](briefs/T-7-009.md) 이슈 #150. port 5269 2026-09-07 14:51 사용자 승인 → Workflow Sonnet 5 투입(worktree 격리) → 17:05 워커 완료, PR #178(훅이 `gh pr create`를 막아 오케스트레이터가 REST로 개설). 검증 중 |
...
| T-7-011 | web(e2e) | 전체 e2e 스위트를 9/6 UX 개편 이후 앱과 정합(main 60/110 실패 핫픽스): 랜딩·인트로·배경 단계·프리시즌 진입·진행 루프·앱 셸 폭. 테스트 완화 없음 | todo | [브리프](briefs/T-7-011.md) 이슈 #179, D-70. port 5272. 17:20 투입 |
| T-7-012 | ui | `.os-eyebrow` 색 대비 axe 위반(12화면) 수정 + `check:contrast` 쌍 추가 | todo | [브리프](briefs/T-7-012.md) 이슈 #180. port 5273. 17:20 투입 |
| T-7-013 | web | SCR-009 제안 비교 `<dl>` 구조 axe 위반(`definition-list`·`only-dlitems`) 수정 | todo | [브리프](briefs/T-7-013.md) 이슈 #180. port 5274. 17:20 투입 |
