...
| T-7-007 | web | 서명 이름 프리필(placeholder 제거), 제안 비교 "결정 기한" 셀 짧은 값 + 상단 캡션, 카드 높이 정렬 | in-progress | [브리프](briefs/T-7-007.md) 이슈 #151·#152. T-7-004 머지 뒤(`offers.tsx`). port 5267 → 18:32 #174 머지 직후 Sonnet 5 워커 투입(worktree 격리, 포트 5267, 워커는 lint·typecheck·web 단위·transfer.spec 1개·360px 카드 높이 측정까지) |
