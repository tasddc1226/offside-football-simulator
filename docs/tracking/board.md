...
| T-7-001 | content | 룰셋 1.4.0(1.3.0 복사 + 5곳): 회복 `zeroMinutesConsecutiveSeasons` 2→1, `managerTrustPromiseBreach` -8→0, 선택 키 `roleProposal.acceptedRoleUpdatesPromise: true`·`declineDowngradeTrustDelta: 0`, 로더·팩 0.5.0 호환·`retirement-artifacts` 짝 | todo | [브리프](briefs/T-7-001.md) 이슈 #140, D-67. 1.3.0 바이트 불변. port 5261. **투입 대기(사용자 승인)** |
...
| T-7-004 | web | 협상 결과 배너 항상 표시(`contract.tsx` `pending===null` 이른 반환 수정), `negotiationDisabledReason` 사유 캡션·`aria-describedby`, 결과 패널 포커스, 회복 안내 문구 정책값 기반 | todo | [브리프](briefs/T-7-004.md) 이슈 #141, D-69. port 5264 |
| T-7-005 | web | 표시 라벨 정합: `relationshipReasonLabel`(PROMISE_BREACH 등 16키+LEGACY_+폴백), 챕터 TAG 제목(`프로_데뷔`→'데뷔 이후의 무대', 폴백 '핵심 경기'), 결산 표 "선발 등급 vs 계약 역할 약속" 분리 + 캡션 | todo | [브리프](briefs/T-7-005.md) 이슈 #142·#143·#105. port 5265 |
| T-7-006 | ui | 액션 독 어포던스: 상단 페이드·그림자, `.os-shell-main` `scroll-padding-bottom`, 불투명 배경 (CSS만) | todo | [브리프](briefs/T-7-006.md) 이슈 #146. port 5266 |
...
| T-7-008 | ui | RadioGroup Enter 키 선택(Radix 기본은 Space만) | todo | [브리프](briefs/T-7-008.md) 이슈 #149. port 5268 |
| T-7-009 | web | 대표팀 챕터 결과 요약 섹션(판단 결과·효과·태그·데뷔, 스코어 꾸미지 않음) | todo | [브리프](briefs/T-7-009.md) 이슈 #150. port 5269 |
...
진행 중 워커 없음 (2026-09-07 오후 — 이 세션은 문서·QA 전담. 12:48 운영 3관점 플레이 리뷰(Sonnet 서브에이전트 3명, ego-browser) 완료, 13:47 운영 6케이스 플레이 리뷰(Sonnet 6명, 57시즌·자연 은퇴 1건) 완료, 14:10 이슈 #140~#172 등록, 14:50 1차 웨이브 브리프 T-7-001~010 작성(투입은 사용자 승인 뒤). 9/5 밤~9/7 새벽 코드 작업은 사용자 세션(Codex·Sol·Luna)이 PR #91~#139로 직접 머지·배포. 남은 항목은 Phase 7 표와 사용자 액션 U-014·U-004·U-005)
