...
| T-7-002 | domain | 1.4.0 키를 읽는 도메인: 역할 제안 수락 시 `contract.rolePromise`·출전 약속 갱신(현재는 한 경기 뒤 소멸), 하향 제안 거절 무벌점, 미이행 벌점 0에서도 로그·태그·카운터 유지, 0분 1시즌 회복 제안 테스트 고정 | in-review | [브리프](briefs/T-7-002.md) 이슈 #140. T-7-001 머지 뒤. 1.3.0 골든·해시 기대값 불변. port 5262 → 17:21 #173 머지 직후 Sonnet 5 워커 투입(worktree 격리, 워커는 lint·typecheck·패키지 테스트까지만, 전체 체인은 오케스트레이터 순차) → 17:58 워커 완료(`b0bcdb0`, domain 933 테스트·골든 불변), PR #181(REST 개설). 체인 대기 |
