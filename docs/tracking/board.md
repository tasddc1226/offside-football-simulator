...
| 리뷰 후속 | domain·web | P1: 원소속 잔류 중 무출전 고착 탈출 수단(약속 위반 누적 시 면담·이의제기·이적 요청), 역할 협상 결과 배너·비활성 사유. P2: 관계 태그 enum 노출(+#105 재사용 챕터), 결산 표 역할·약속 모순, 승격 기록 vs 리그 표기, 미이행 실시간 알림, 하단 고정 CTA 여백(5화면), 재계약 사전 협상 타이밍, 임대 성과 미반영, 커스텀 라디오 키보드 선택, 대표팀 경기 결과 공백, 제안 비교 셀 정렬, 서명 placeholder | todo | 이슈 등록·배정은 사용자 결정. 묶음 제안은 6케이스 README "다음 조치" |
