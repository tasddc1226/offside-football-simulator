...
| T-0-011 | Node(Vitest)·Cloudflare Workers(Miniflare) 동일 fixture state hash·SHA-256 경계·canonicalize 일치 테스트 | ADR-003, 08 | todo | [브리프](briefs/T-0-011.md). T-0-007 머지 후. 브라우저 Web Worker 검증은 Playwright 도입 시 |
...
| T-0-013 | Pretendard self-host 폰트를 dynamic subset(unicode-range 분할)으로 바꿔 초기 폰트 전송량 축소, 허브 LCP 2.5초 예산 측정 | 13 구현 체크리스트, 01 성능 예산 | todo | [브리프](briefs/T-0-013.md). T-0-009 후속. 현재 woff2 단일 파일 2MB |
