...
## 2026-09-25 (T-10-017 A매치 집계 정정)

- **아시안게임·올림픽(U-23) 경기는 A매치 출전·골·도움으로 세지 않는다.** 대회 기록(tours)에만
  남긴다. 그동안은 U-23 경기로 `nat.caps`가 올라 커리어 여정의 'A매치 데뷔'가 실제 첫 A대표
  발탁보다 앞선 해로 찍혔다. A대표 선발 규칙은 그대로다.
- **밸런스 기준선과의 차이는 집계 방식이 바뀌어서 생긴 것으로 받아들인다(사용자 결정).** SEED=42,
  3천 커리어 비교: random 정책 `cappedShare` 67.7% → 55.7%(v4 기준선 67.7%, 허용 오차 ±2 밖),
  smart 정책 A매치 100경기 이상 21.6% → 16.7%. 원작 v4도 U-23 경기를 A매치로 센 것으로 보고,
  선발 문턱(`NT_THRESHOLD`)을 낮춰 맞추지는 않았다. full-validation의 `cappedShare` FAIL은 이
  변경 때문이다. `plShare` FAIL(31% vs 35%)은 이 변경 전부터 있었다.
- 기존 세이브의 이미 부풀려진 A매치 수는 보정하지 않는다.
...
- Phase 9(T-9-001) 착수. 서브트랙: a(api) done, b(web) done, c(CI/운영) 진행 중,
  d(문서, 이 작업) 진행 중. 전체 배경·대체된 ADR 목록은
  [ADR-013](../adr/ADR-013-fulltime-replacement.md).
- T-9-001d 범위: 원작 기획·화면·콘텐츠·QA·디자인·트래킹 문서를
  `docs/archive/offside/`로 이동(마지막 main 커밋 `68865f9`, `offside-final` 태그는
  오너 제안만 하고 직접 만들지 않음), ADR-013 추가, 영향받는 ADR 9건 상태 갱신,
  `board.md`에 Phase 9 절 추가(과거 보드는 아카이브로 이동), 이 항목들을 결정
  로그에 기록, 루트 `README.md`·`CLAUDE.md`·`AGENTS.md` 재작성, 변경한 문서
  대상 상대 링크 점검.

## 2026-09-23/24 (D-85 — 풀타임 전면 교체 오너 결정 5건)

1. **전면 교체.** T-8 WORLD STAGE는 취소·무효(WORLD STAGE라는 명칭 자체가
   더 이상 쓰이지 않는다). 원작 OFFSIDE(React 19 + TanStack, Web Worker 결정론
   시뮬레이터, 버전 고정 JSON 콘텐츠 팩·ruleset, Dexie IndexedDB, 서버 체크포인트
   동기화·라커룸·경쟁·발행물·서버 연간 커리어)를 새 게임 **풀타임**(확률/이벤트
   기반 커리어 시뮬레이터)으로 완전히 대체한다. 실사용자 데이터가 운영에 없어
   보존 제약 없이 교체 가능했다. 풀타임이 흡수해야 할 것만 남긴다.
2. **브랜드는 OFFSIDE 유지, 부제에 풀타임 포함.** 부제
   `풀타임: 휘슬이 울릴 때까지`, 페이지 타이틀
   `오프사이드 — 풀타임 축구 커리어`. 도메인 offside-lab.com도 그대로 쓴다
   (ADR-006 재확인).
3. **서버는 로그인·프로필만 유지.** `apps/api`는 health, profile(익명 프로필·
   설정·복구 코드·복구·삭제), Google 로그인(시작·콜백·연결 해제·로그아웃)만
   남긴다. 계정 병합 플로우와 `/v1/auth/merge`는 제거한다. D1 마이그레이션
   0015가 게임 테이블을 전부 드롭하고 `profiles`·`sessions`·`auth_attempts`·
   `audit_log`·`idempotency`만 남긴다.
4. **구현 위임 대상 변경.** 구현은 Claude Sonnet 5 서브에이전트에게 위임하고,
...
- 플레이 리뷰(사용자 지시 "직접 여러 번 플레이해 부족한 점을 이슈로"): 원작 조사(최근 8일 업데이트 — 일일 신인왕 랭킹·미니게임·부정 선수 검증·PvP 베타·서버 이관 예고·로드맵) + Sonnet 3세션(게임성 8시즌, UX/온보딩, 장기 14시즌). 게임 오류 0건. 공백·어색함을 P1 3·P2 11·P3 5로 정리해 이슈 #241~#259 등록(중복 제외: 의도된 "VAR" 브랜드 문구, 기존 #169·#161, 계획된 Phase 8 해외 진출·ADR-011 리그 실시뮬). 좋았던 점: 선택 근거 투명(확률·수치), 출전 약속이 실제 출전·성장과 연동, 은퇴 회고(Legacy) 완성도, 모바일 일관성.
...
- 19:48 T-7-030 PR #225 squash 머지 `fe2b3bf`(main에 룰셋 1.6.1·팩 0.6.2 등록, ACTIVE 불변). 이어 T-7-031 워커 투입.
- 20:16 T-7-031 1차 워커 BLOCKED: 룰셋 `retirementRules` 필드·도메인 인자화는 됐지만 정책을 인자 없이 부르는 곳(career-sim.ts 로컬 판정 2곳, domain career-event.ts 1곳)이 남아 새 정책과 경합 → 1천 커리어 999건 FAILED. 판단: 브리프 범위를 넓혀 career-sim.ts 수정을 허용하고 career-event.ts·simulate.ts·화면까지 같은 정책을 넘기게 한다(도메인 안에서 한 상태에 두 정책이 섞이면 안 됨). 2차 워커 같은 브랜치에서 계속.
- 20:44 T-7-031 1만 회 판정 **통과**(REVIEW 100%, 심사 은퇴 나이 p50 37·평균 36.6·p10 34·p90 38, 정점 29.3세 유지). 1.5.0 200커리어 stateHash 200/200 일치. Legacy 1.1.0 LEGEND+ICON 50.5%로 이미 30pp 하락 — T-7-032 출발점. 관찰: 34세 이전 은퇴 0건(부상/시장 요인이 나이 압력에 묻힘) — 2라운드 검토 항목.
- 20:58 T-7-031 PR #226 squash 머지 `eb91347`. 이어 T-7-032(Legacy 1.2.0) 워커 투입.
- 21:20 T-7-032 워커 PARTIAL: Legacy 1.2.0으로 전체 LEGEND+ICON 32.6%(목표 30~40) 충족, 포지션 편차 31.2pp는 미충족. 진단: DF 편향은 contribution(per90 기대치 900→1300에도 DF 50%대 불변)이 아니라 시즌 시뮬 자체의 포지션 편향(DF: 이동 적음·출전 많음·평점 높음·대표팀 많음)이 longevity·merit·achievement로 전이된 것. 판단: 1라운드 ③은 전체 비율 목표만 채우고 머지, **포지션 편차는 2라운드 신규 항목(포지션별 평점/선발/출전 모델)으로 사용자에게 제안**. 부수 발견: TAG-UNCROWNED 파생은 버전을 안 타서 1.2.0에서도 빈도 불변.
- 21:20 T-7-032 /review:pr Critical 2(engine-client 아카이브 검증·api 동기화가 Legacy 1.2.0을 모름 → 1.2.0 결과 재읽기 INVALID_BINDING, 재동기화 REFERENCE_CONFLICT, fail-closed 검사 구멍). 후속 워커가 21:33 해소(bd588de): 도메인 `LEGACY_VERSIONS` 단일 출처, 게이트를 "1.0.0이 아니면 가용 버전과 동일"로 일반화. 교훈: Legacy 버전 추가 시 domain 밖 소비자(engine-client·api sync·contracts)를 함께 확장해야 한다 — 다음 버전부터 브리프 체크리스트에 포함.
- 21:35 T-7-032 1만 회 판정: LEGEND+ICON 31.9%(목표 30~40) 충족, 포지션 편차 27.0pp(목표 ≤10pp) 미충족. 1.5.0 회귀 200/200. 결정대로 머지 진행.
- 21:48 T-7-032 PR #227 squash 머지 `cbd21fe`. **1라운드 ①②③ 모두 main 반영**(룰셋 1.6.1/팩 0.6.2): 정점 29.3세, 심사 은퇴 100%·p50 37세, 상위 밴드 31.9%. 21:50 10만 회 최종(1.6.1/0.6.2, base-150, opportunity) 착수. 2라운드 제안(사용자 결정 대기): (a) 포지션 편차 — 시즌 시뮬의 포지션별 선발·출전·평점 모델(Legacy 정책 밖), (b) 34세 이전 은퇴 0건 — 부상/시장 요인 하한, (c) 기존 항목 잠재 도달 70.7→55~65%·RESERVE·부상·5시즌 내 1부, (d) 사용자 룰셋 1.7.0(1.6.0 기반)에 1.6.1 값 반영 여부.
- 2026-09-15 01:06 10만 회 최종(1.6.1/0.6.2, base-150, opportunity) 완료, 실패 0, 3.3시간. 1만 회 판정과 일치: 정점 29.31세(p90 31), 심사 은퇴 100%·p50 37세, LEGEND+ICON 32.4%(LEGEND 3.8), 포지션 편차 25.6pp(DF 47.2·GK 34.4·MF 26.5·FW 21.6). **D-80 1라운드 종결** — ①②③ 목표 충족, 포지션 편차만 2라운드 이월. 2라운드 출발점: 잠재 도달 71.1%, 5시즌 내 1부 9.4%, 부상 1.28/시즌, 34세 이전 은퇴 0건. 사용자 결정 대기(2라운드 항목 a~d).
...
- 11:06 #221 최종 head 6998890(main 1d8f8ee 머지) push됨 → c050957과 apps/packages/tooling/.github/lock diff 없음(docs 7파일만) 독립 확인. 11:06 c050957 전체 체인(e2e 제외) CHAIN EXIT 0: install·lint·lint:deps·typecheck·content:validate·test(10 tasks, Test Files 282 passed, 실패 0; content·domain·fixtures·scripts 4개는 turbo 캐시 hit)·build·web check:bundle·ui check:contrast 전부 통과. Codex에 보고하고 CPU 슬롯 반환. #221 Claude 몫(읽기전용 리뷰·전체체인·main 제품 tree 불변) 완료, 머지는 Codex가 worker 스코프 e2e·브라우저 인수 결과와 합산해 결정.
- 11:20 #222 통합 SHA a765da8(merge-base 85b2b3f, 84파일) 읽기전용 리뷰 PASS, 소스 변경 요구 없음: 1.7.0 = 1.6.1 + leagueLedgerRules(growth·retirement deep-equal, 테스트 동치 검사), 팩 0.6.3 = 0.6.1 바이트 복사(manifest version·compat [1.7.0]만, checksum 동일, golden 항목 추가, 0.6.0~0.6.2 불변), Legacy 게이트 (1.7.0,0.6.3)→1.2.0 + 테스트, 스키마 retirementRules 가드 12건 유지 + LeagueLedgerRulesSchema·1.7.0 필수 refine, career-sim 정책 전달 3곳 보존(추가는 measureStorage 계측), 도메인·contracts·engine-client·web 소스는 fc00e50 이후 변경 없음, APPROVED_PRODUCTION_MANIFESTS 불변. 비차단 메모: contracts·engine-client가 rulesetVersion==='1.7.0' 리터럴로 원장 불변식을 켬(1.8.0 때 갱신). 대량 회귀는 CPU 슬롯 배정 대기.
- 11:17 사용자 PR #221(T-7-023 기존 선택 후속 결과 영수증) Codex가 squash 머지 `60aed6a`. 머지 후 main 제품 tree가 제가 체인 돌린 6998890과 diff 0(검증된 tree 그대로). 11:12 #222 a765da8 대량 회귀 시작(nohup, 1.5.0 200 조인·1.6.1 1000 조인·1.7.0/0.6.3 1000×2 결정론+분포 리포트; 동일성 강제는 결정론에만). 다음: #220 Sol이 origin/main(60aed6a) 최종 통합 → exact SHA 재확인(#221이 손댄 index.tsx·offers.tsx와 retirement-screen.tsx 3-way).
...
- 11:54 사용자 PR #224(T-7-025 현재 리그 맥락) Codex가 squash 머지 `9762b73`(exact ce92dfe = 7cee56a + docs; 전체 체인 PASS + Sol E2E2 + root 360px 키보드·CTA·reload·동점 UI 인수 PASS). 머지 후 main 제품 tree = 체인 돌린 7cee56a. **이로써 사용자 세션 개선 묶음 #219·#221·#220·#222·#224 전부 main 머지 완료.** 유지 경계: 룰셋 1.7.0·팩 0.6.3은 등록만(ACTIVE 미승격), Legacy 상위 밴드 22.7%(목표 32%) 활성화 전 재보정 게이트 미통과, live API·Worker 미검증. 추가 코드·대량 검증 불필요(Codex). Claude 검증 산출물: 체인 로그 4건(c050957·b950c13·f229f0f·7cee56a), 회귀 1건(a765da8), 리뷰 8건.
...
- 조사: 네비는 `apps/web/src/routes/__root.tsx` `GameNavigation` 한 곳(전 라우트 공통, `.os-app-nav` 좌 브랜드·우 컨트롤). 세션 미들웨어는 쓰기 없음, `sessions.last_seen_at`은 생성 때만 기록, `profiles.last_seen_at`은 1시간 스로틀. `wrangler.jsonc`는 D1뿐(KV·DO·Cron 없음, Analytics Engine 미사용 결정). 웹에는 폴링(`refetchInterval`)이 아직 한 곳도 없다. 접속자·DAU 개념은 문서에 없다.
- 후보: (A) 세션 하트비트로 `sessions.last_seen_at` 갱신 + 5분 창 `COUNT(DISTINCT profile_id)` / (B) `analytics_events` 최근 5분 `client_id` 집계(쓰기 0이지만 인덱스·보존 정책 없고 허브 대기 중엔 이벤트가 끊겨 값이 흔들림, 유실 허용 테이블에 기능을 얹음) / (C) Durable Object·KV 카운터(정확·실시간이지만 새 바인딩 3환경 + 운영 비용, ADR-007 최소 인프라와 어긋남).
- 결정: **A.** 새 바인딩·테이블 없이 기존 열 + 인덱스 1개. 창 5분, 하트비트는 탭이 보일 때만 60초, 서버 30초 스로틀, `GET`은 공개·30초 캐시. 값은 "활동 중인 프로필 수"(익명 프로필은 기기마다 달라 사실상 활성 기기 수에 가깝다). 배지는 0·미수신이면 숨겨 헤더가 흔들리지 않게 하고, `packages/ui`는 CSS만(ADR-005). 개인정보 문구는 "세션의 마지막 활동 시각 — 집계 전용" 1줄 추가(`privacy.tsx`, 09 문서는 머지 뒤 오케스트레이터가 정합).
- 작업: [T-7-014](../archive/offside/tracking/briefs/T-7-014.md)(api·contracts, API-PRES-001·002) → 머지 뒤 [T-7-015](../archive/offside/tracking/briefs/T-7-015.md)(web·ui css). 사용자 승인 뒤 투입.
- 비용 상한 메모: 하트비트 1분 간격이면 동시 100명 ≈ 하루 D1 쓰기 14만, 1만 명이면 1,400만 — 그 규모가 오면 C(DO)로 옮긴다.
- 적용(9/14 11:56): T-7-014 PR #211 `b05867b` squash 머지. 워커는 브리프대로 구현(`routes/presence.ts`, `repos/sessions.ts` 쿼리 2개, `contracts/presence.ts`, 마이그레이션 `0006_flat_nova` = `sessions_last_seen_at_idx`, 테스트 3건). 리뷰: 세션 미들웨어가 쿠키를 발급하지 않아 공개 `Cache-Control`이 안전, 로그에 토큰·프로필 ID 없음. 검증: 1차 체인은 워커가 남긴 `wrangler dev` 2개로 load 95에서 API 테스트 4건(무관 파일) 타임아웃 → 프로세스 정리 뒤 재개해 단위 10/10 완주(취소 0)·빌드·번들·대비 통과. e2e 19건 실패 → `--workers=1` 단독 재실행 16건 잔존 → **origin/main 230a9e0에서 같은 16건 단독 실행도 동일 실패**(새 실패 0, #207 코멘트로 기록 — 부하가 아니라 9/13 개편 뒤 스펙 불일치로 보임).
- 운영 메모(9/14 11:54): 셀프호스트 러너 `offside-mac-arm64`가 오프라인(사용자 맥북 잠자기, 깨울 수 없음)이라 CI 3건이 큐에 걸림. 사용자 선택으로 이 맥(DDaos-MacBook-Pro)에 임시 러너 `offside-mac-arm64-tmp`(라벨 self-hosted,macOS,ARM64,offside, `~/offside-runner-tmp`, `run.sh` nohup)를 등록. 이번 작업(T-7-015 머지·운영 배포)까지 쓰고 제거한다.
- 다음: T-7-015(web·ui css) 워커 11:57 투입 → 머지 → 운영 배포(인덱스 마이그레이션 포함).
- 적용(9/14 12:32): T-7-015 PR #212 `c77375f` squash 머지(워커 로컬 관찰: 배지 3.2초 내 표시, 하트비트 60초·탭 숨김 시 중단·복귀 시 즉시 1회, API 다운 시 무배지, 360px·150% 허브·커리어 한 줄 유지). 사고: 워커의 `pkill -f "vite"`가 오케스트레이터의 `vitest` 체인을 죽여 템플릿에 "PID로만 종료" 규칙 추가.
- 사용자 지시(9/14 12:20): "오픈된 PR을 순차적으로 운영환경까지 배포." 최종 합본(main+#208+#210+#209+#212) 전체 체인 12:18~12:28 통과(단위 10/10·빌드·번들·대비; e2e는 main 단독 16건 + 부하 3건(단독 통과) + `season-result SCR-015` seed 드리프트 1건 — #210의 활성 룰셋 1.5.0 전환으로 같은 seed에서 시장이 열림, #207 기록). 머지 순서 #208 `ceff29c` → #210 → #209 `139d1ca` → #212 `c77375f`. **실수**: #210이 스택 PR(base `feat-kleague-structure`)인데 #208 머지 뒤 브랜치를 남겨 base가 main으로 전환되지 않았고 스크립트가 그대로 머지해 feature 브랜치에 들어감(57f2be0) → 57f2be0을 main에 cherry-pick한 #215 `4b29c4d`로 재적용(트리는 검증 합본과 동일). 교훈: 스택 PR은 선행 PR 머지 시 브랜치를 지우거나 base를 직접 main으로 바꾼 뒤 머지한다.
- 다음: main `4b29c4d` + 문서 커밋으로 preflight → deploy(1.5.0/0.6.0 승격, `sessions_last_seen_at_idx` 마이그레이션).
- 운영 배포 완료(9/14 12:43): main `643cd2a`로 preflight run 34803042783 → deploy run 34803426999 성공(12:41~12:43). manifest plan=activate로 시즌 1이 **1.5.0/0.6.0**으로 승격, 마이그레이션 0006 적용, API 16808919…·web b69dc26d…. 배포 후 검증(api.offside-lab.com 기준 health·current·presence·CORS·웹 번들 교체·QA 플레이 `QA배포1244` 69255a9e-…·배지 "1명 플레이 중" 허브·커리어 표시) 통과. 기록: `docs/operations/production-release.md` "2026-09-14 12:43 재배포". D-78 종결.
...
- 파이프라인: Workflow `pipeline()`으로 구현 → 읽기 전용 리뷰어 2명(렌즈: 회귀·범위 / 접근성·정확성, 도메인 작업은 결정론·불변 / 게임 규칙·UX) → 차단 결함이 있으면 같은 브랜치에 수정 워커. 리뷰어 판정 기준에 "e2e 단언 완화(삭제·허용 필터)는 차단, 정합 갱신만 허용"을 넣었다. D-70대로 워커는 lint·typecheck·단위·관련 e2e까지, 전체 체인은 오케스트레이터가 직렬 실행.
- 도메인 원칙 재확인: 룰셋 1.0.0~1.3.0 골든·해시 불변 → 상태 전이를 바꾸는 수정은 1.4.0 선택 키 존재로 가드(D-43/D-67 패턴), 표시 전용 계산은 가드 없이. 따라서 D의 결과도 1.4.0 승격(사용자 결정) 전엔 운영 미적용이다.
- 부하 플레이크: PR #193 체인에서 `apps/web` `phase4-seed-reachability.test.ts` NATIONAL_TEAM 120s 타임아웃(워커 5명 동시 실행) → 단독 재실행 7/7 통과, build·bundle 녹색으로 확정 후 머지. D-70의 판정 규칙 그대로.
- 브랜치 정리: 사용자 승인("삭제") 뒤 머지된 PR 헤드 ∩ 원격 브랜치 − 열린 PR 헤드 = 86개 삭제. 첫 시도는 zsh가 unquoted 변수를 단어 분리하지 않아 86개 이름이 한 인자로 넘어가 0건 삭제 — 파일 기반 `while read` 루프로 재실행해 86/86 삭제. 보존: `T-5-003-retirement-decision`(머지 PR 없음).

## 2026-09-12 (D-73 — 설정 화면 원작(SLB) 구조 개편 범위 — 사용자 결정)

- 근거: 사용자가 Orca 내장 브라우저에 원작(slbcareer.com) 환경설정을 로그인 상태로 열어 두었고 오케스트레이터가 `orca snapshot/full-screenshot --page`로 읽었다(탭 전환 없이). 원작 구조: 프리셋 색 배너 헤더(뒤로가기·"내 기기" 배지) / 계정 1카드("<이름>님 · Google 계정으로 로그인했습니다" + 로그아웃) / TEAM SETTINGS 아코디언(팀별 로고 업로드 + 이름 + 플레이버, 이름·로고 기본값) / COLOR PRESET 2×4 타일(실제 구단 테마 이름) / LEGAL 목록 2행 / 제작자·채널·베타 버전 / 회원 탈퇴 빨간 링크.
- 결정(UX-013, 사용자 4옵션 전부 포함): 배너 헤더·계정 1카드(+계정 상세 아코디언)·4열 타일 프리셋 + **가상 구단 12팀 컬러 프리셋**(UX-008 `--os-team-<id>` 재사용) + **프리셋이 `--os-hero` 계열도 재정의**(배너·화면 전환·트레이딩 카드·은퇴 화면이 함께 물듦, 18종×2테마 hero/on-hero·hero/hero-muted 대비 자동 검증) + **구단 로고 업로드**(png/jpeg/webp ≤2MB → 캔버스 128px → webp/png data URL ≤40KB, `ui-store.teamLogos` 로컬 전용 — toss 채널이 문자열 KV라 Blob 대신 문자열) + 서비스 정책 목록 + 테마·모션·텍스트·플레이 모드 접힌 아코디언 + 하단 위험 텍스트 링크. 빠른 이동 네비 제거.
- 투입 조건: Wave C-B(settings.tsx·SettingsFooter·__root)·C-A(TeamBadge·PageShell) 머지 뒤 새 main에서 분기. 2단계(기반: 토큰·프리셋 그리드·로고 파이프라인·ClubBadge 래퍼 → settings.tsx 개편) + 회귀 / 원작 충실도·UX 2렌즈 리뷰.

## 2026-09-08 (D-72 — 전역 화면 전환 3초 고정·프로그레스 연출 — 사용자 지시)

- 사용자 지시: "화면 전환 이벤트를 전역에서 재사용 가능한 코드로, 3초로 고정, progress bar 느낌의 극적 효과". 배경: 허브 "커리어 시작"은 로컬 DRAFT 생성 뒤 0.1초 하드 컷, KICKOFF 세리머니는 650ms + 키보드 입력이면 0ms 스킵이라 "전환이 너무 빠르다"는 불만.
- 결정: `packages/ui` `ScreenTransition` — `SCREEN_TRANSITION_MS = 3000` 단일 상수(호출처에서 시간 변경 prop 없음, 스킵 prop 없음), 진행 바 0→96% 선형(rAF/인터벌, aria-valuenow 실시간) + 1/3 지점 단계 문구, `waitFor`는 max(3초, 작업 완료)에 완료·미완이면 96%에서 대기·reject는 `onError`, 모션 감소만 즉시 완료(키보드 모달리티 스킵 제거). 기존 `GameCompletionTransition`은 얇은 래퍼로 남기고 사용처 3곳 마이그레이션 + 허브 커리어 시작·계약 완료 신규 적용.
- 검증에서 잡은 것: 1차 산출물은 바가 0.4초에 88%(강한 ease-out)·오버레이가 셸 절반(중간 래퍼 3겹이 display:block auto-height라 퍼센트 무효) → 수정 워커 2회(선형 진행, `.os-shell-main` closest 실측 min-height). e2e는 프로덕션 우회 코드 없이 스펙 쪽 `reducedMotion: 'reduce'` 에뮬레이션으로 대응. PR #191 `c46b740`, 운영 배포 run 34136583584(preflight 34136583584 이전 성공). 9/12 #193에서 이 실측 로직을 `useShellMainHeight` 훅으로 추출하며 조건부 렌더 ref 미추적 버그를 함께 수정.

## 2026-09-07 (20:30, D-71 — P1 마무리 뒤 운영 배포, 코드만·시즌 룰셋 1.3.0 유지 — 사용자 지시)

- 사용자 지시: "P1 작업 모두 마무리되면 메인 프로덕션까지 배포하고 작업 마무리하자." 남은 P1은 #179(T-7-011 e2e 정합)뿐이고, 1차 웨이브 T-7-001~010은 20:33 PR #186까지 전부 main에 머지됐다(오늘 머지 12건: #173·#177·#176·#175·#178·#174·#181·#184·#183·#182·#185·#186).
...
- 발견: 1차 웨이브 워커 6명(17:05 완료, PR #173~#178)이 모두 "전체 e2e가 origin/main에서도 같은 60건 실패"를 보고했고, 오케스트레이터가 조용한 머신에서 새 워크트리로 재현했다(17:11, `35c9d8a`, 60 failed/43 passed/7 skipped, 결정적). 원인은 9/6~9/7 사용자 세션의 앱 변경(#103·#117·#120·#131·#134·#135·#139)에 `apps/web/e2e` 헬퍼·기대가 따라가지 못한 것 — 첫 방문 흐름(공개 랜딩·시네마틱 인트로·'새 인생 시작'·배경 단계) ~25건, 프리시즌 진입 h1 17건, 시즌 진행 루프 4건, 앱 셸 폭 2건, 그리고 **진짜 결함**인 axe 위반 12건(`.os-eyebrow` 색 대비, SCR-009 `<dl>` 구조). main CI는 스테이징 스모크만 돌려 잡히지 않았다. 이슈 #179(드리프트)·#180(a11y).
- 결정: (1) 핫픽스 3건을 즉시 투입 — [T-7-011](../archive/offside/tracking/briefs/T-7-011.md) e2e 정합(`apps/web/e2e/**`만, 테스트 완화 금지), [T-7-012](../archive/offside/tracking/briefs/T-7-012.md) eyebrow 대비(ui), [T-7-013](../archive/offside/tracking/briefs/T-7-013.md) 제안 비교 dl(web). (2) 과도기 판정 규칙: 핫픽스가 머지될 때까지 PR 체인의 e2e는 **baseline(60건) 대비 새 실패 없음** + 단위 테스트 그린 + 오케스트레이터 화면 확인(ego-browser)으로 판정하고, 머지 뒤 전체 스위트 그린을 게이트로 복원한다. (3) CI에 전체 e2e(또는 축약 세트)를 되돌릴지는 사용자 결정(#179에 기록).
...
- Workflow `offside-t7-wave1`(run `wf_aefc582c-e12`)로 Sonnet 5 워커 6명을 병렬 투입: T-7-001(content)·T-7-004·T-7-005·T-7-009(web, 서로 다른 파일)·T-7-006·T-7-008(ui, 서로 다른 파일). 각자 `origin/main`(`1640fef`)에서 브랜치, worktree 격리, xhigh, 브리프 포트로 e2e. 워커는 `gh pr create`가 막히면 push + 본문 반환으로 보고하고 오케스트레이터가 PR을 연다. 의존 작업(T-7-002·003 ← T-7-001, T-7-007 ← T-7-004, T-7-010 ← T-7-005)은 선행 PR 머지 뒤 2차 Workflow로.
- 검증·머지 규칙은 이전과 같다: 오케스트레이터가 PR별로 전체 체인(고유 포트)과 리뷰, 필요 시 ego-browser 화면 확인을 한 뒤 사용자 승인 없이 squash 머지. 연속 머지 시 뒤 PR의 체인은 앞 PR 머지 뒤 시작한 것만 유효(9/5 교훈).
...
- 방법: 사용자 지시("권장 6회로 돌려보자. ego-browser에서 겹치지 않게 주의해서 sonnet 5 에이전트에게 돌려줘")대로 Sonnet 서브에이전트 6명이 각자 ego-browser 작업공간에서 offside-lab.com을 12:52~13:47 병렬 플레이했다. 케이스는 포지션(GK·CB·WG·CM·ST·DM)·성별·국적·출발 카드·첫 갈림길·전략을 직교로 배치했고, 여섯 작업공간이 같은 브라우저 프로필을 공유하므로 선수 생성 구간만 스크래치패드 `mkdir` 잠금으로 직렬화했다(초안 혼선 0건). 안전 규칙은 3관점과 동일. 결과는 [docs/qa/2026-09-07-production-six-cases](../archive/offside/qa/2026-09-07-production-six-cases/README.md)(케이스별 문서 6건, 캡처 19장).
- 결론: 핵심 루프는 6/6 완주, DM 케이스는 17시즌·36세 자연 은퇴·Legacy 78·보관함까지 정합. 무출전 고착은 57시즌 중 20시즌(5/6 케이스)으로 프로필·전략과 무관하게 재현됐고, "원소속에 머무는 한 어떤 선택도 출전을 만들지 못하며 임대·이적으로 소속을 바꿔야 회복"으로 조건이 좁혀졌다(9/6 ②·9/7 P1-1 세 번째 확인). 역할 협상 버튼 무결과 4/6(자유계약에서는 정상), 관계 태그 enum 노출 4/6, #105 챕터 제목 3/6(진짜 데뷔전은 정상, 재사용 챕터만 키 노출). 신규: 시즌 결산 표의 역할·출전 약속 모순(2/6), 리그 1위·승격전 성공·"승격전문가" 태그에도 소속 리그 표기 불변(3/6). 부상 재발 루프·#104 GK 문구·임대 0회는 이번 표본에서 재현되지 않았다. 종합 P1 2·P2 8·P3 10.
- 판정 조정: 케이스 5의 "강동강동 로버스 팀명 중복"은 캡처상 배지 이니셜+팀명이 정상 분리된 `innerText` 아티팩트(접근성 메모만), 케이스 1의 "계약 기간 불일치 P1"은 계약 마지막 시즌에 갱신을 미리 협상하는 설계로 판정(문구만 P3), 케이스 2·3의 enum P1은 진행을 막지 않아 P2 유지, 케이스 6의 "다른 기기와 어긋남"은 프로필 공유 환경 특성. 케이스 4의 시즌 1 FAST·재계약 오거절은 자동화 오류로 명시.
- 운영에 QA 커리어 6건(`QA2*0907`)이 추가로 남았다(오전 3건·9/6 3건 포함 총 12건 통계 제외). 이슈 등록·브리프·수정 배정은 사용자 결정으로 넘기며, README "다음 조치"에 P1 설계 과제·문구 매핑 티켓·승격 표기 확인·P3 묶음·추가 은퇴 표본 순서를 제안했다.
- 후속(14시, 사용자 지시 "개선 포인트들을 모두 깃헙 이슈로 등록"): 3관점 16건 + 6케이스 20건 + 문서 본문에만 있던 3건(설정 약관 링크, 결산 임대 팀 라벨, 두 탭 동시 생성 초안 충돌)을 같은 원인끼리 합쳐 **이슈 #140~#172(33건)** 로 등록했다. 제목은 기존 관례 `[Pn][영역]`, 라벨은 기존 `bug`·`enhancement`·`accessibility`만 사용, 본문에 환경·재현·근거(레포 내 QA 문서·캡처 링크)·권장 범위를 적고 관련 이슈끼리 교차 링크했다. #105에는 "재사용 챕터에서만 키 노출" 단서, #104에는 GK 8시즌 미재현을 코멘트로 남겼다. 9/6 결론은 PR #115·#104·#105로 이미 반영돼 새 이슈를 만들지 않았다. 워커 배정은 사용자 결정.

## 2026-09-07 (오후, 운영 3관점 플레이 리뷰 — 사용자 요청)

- 방법: 사용자 지시("sonnet 서브에이전트 3개, 디자인·게임성·기능 관점, 각자 ego lite 브라우저 세션으로 운영 게임 플레이")대로 Sonnet 서브에이전트 3명을 병렬로 띄워 각자 ego-browser 격리 작업공간에서 offside-lab.com을 플레이했다(디자인 `QA디자인0907` 1시즌+제안 비교·설정·가이드, 게임성 `QA게임성0907` 6시즌, 기능 `QA기능0907` 2시즌). 기존 기록 미접촉·QA 커리어 1개씩·복구 코드 미출력·피드백 미전송·상태 주입 없음. 결과는 [docs/qa/2026-09-07-production-three-perspectives](../archive/offside/qa/2026-09-07-production-three-perspectives/README.md).
- 결론: 핵심 루프는 운영에서 끝까지 돌고 콘솔 오류 0. PR #115의 성인 진로 전환은 개선됐으나 "무출전에서 벗어날 수단 없음"(9/6 결론 ②)은 그대로 — P1 2건(탈출 경로, 협상 결과 피드백). P2 7건(하단 고정 CTA 가림 4화면, 라디오 키보드 무반응, 대표팀 결과 공백, 관계 태그 enum 노출, #105 챕터 제목 재확인, 제안 비교 셀 정렬, 서명 placeholder), P3 7건.
- 판정 조정: 디자인 문서의 P1 "하단 고정 CTA 가림"은 스크롤로 도달 가능해 P2로, "다른 세션 선수 데이터 노출"은 세 작업공간이 같은 브라우저 프로필(IndexedDB)을 공유해 생긴 테스트 환경 충돌로 분류(같은 브라우저 두 탭 동시 생성 시 초안 충돌 가능성만 메모). 오케스트레이터가 캡처를 직접 열어 CTA 가림·enum 노출·다른 선수 카드 노출을 확인했다.
- 운영에 QA 커리어 3건(`QA*0907`)이 남았다. 통계·랭킹 분석 시 제외. 이슈 등록·브리프·수정 배정은 사용자 결정으로 넘긴다.
...
- **Phase 3·4 출시 게이트 종결(D-65 범위).** (1) T-4-017 해시 probe career-12·13 — PR #90 `84709ec`, Node·workerd·golden 일치. (2) 실사용자 플레이 시간 측정 준비 — PR #94 `232bc83` `elapsedSec` + [play-time-measurement.md](../archive/offside/qa/play-time-measurement.md); 실제 측정은 staging 배포(self-hosted runner, U-017)와 LINE TEST(U-015) 뒤 §6 절차. (3) SCR-032 대표팀 자연 플레이 캡처 — PR #100, 인수표 P4-7 문구 게이트는 D-66·T-4-028 PR #98 `452d32d`로 처리. 기본 팩 0.1.0 유지(D-56·D-65), Phase 5 PR #77 보류(D-65). 기록: [phase34-completion.md](../archive/offside/qa/phase34-completion.md) "출시 게이트 종결 기록", [phase-3-4-plan.md](../archive/offside/tracking/phase-3-4-plan.md) §4.1.
...
- T-4-023 워커의 발견: SCR-032 미리보기 라벨은 콘텐츠 팩 `EVT-NAT-001.json`의 `previewEffects[].label`("협회 관계: +4")이고 SCR-014는 `labels.ts`의 공용 `agent` 라벨("에이전트 관계 +4")이라 두 화면이 같은 효과를 다른 이름으로 부른다. D-66은 결과 화면에 "에이전트 관계는 협회 관계의 대리값입니다"를 붙여 잇는 최소 조치이며, 팩 라벨·공용 라벨 통일은 콘텐츠 승격(PROTOTYPE→SHIPPABLE) 때 다룬다.

## 2026-09-05 (21:36, D-66 대표팀 `agent` 축 문구 — T-4-028, 원래 T-4-025에서 재번호)

- **D-66.** T-4-023 캡처로 P4-7 문구 게이트 확인: SCR-032는 "팬·협회 관계", 소집 수락 결과 SCR-014는 전역 라벨 "에이전트 관계 +4". 전역 라벨(`labels.ts` `agent`)은 윤리·관계 화면이 공유하므로 유지하고, 대표팀 두 화면의 문구만 "에이전트 관계(협회 관계의 대리값)"로 맞춘다(문구 2줄, [T-4-028](../archive/offside/tracking/briefs/T-4-028.md), port 5251). 21:40 사용자 세션의 PR #91 브랜치가 이미 `T-4-025-staging-ui`를 쓰고 #92·#95가 T-4-026·T-4-027을 쓰는 것을 발견해 이 작업을 T-4-028로 재번호(브랜치명 `T-4-025-national-team-wording`은 유지, 머지 제목은 T-4-028). 사용자 PR #91·#92·#95·#77은 이 세션이 건드리지 않는다. 도메인의 `agent` 축 의미 변경·새 축 추가는 하지 않는다(D-62).
...
- **측정은 staging 배포가 전제.** U-017(Actions 결제) 해결 → main 재실행 → staging 배포 뒤 [play-time-measurement.md](../archive/offside/qa/play-time-measurement.md) §6 절차. 남은 출시 게이트: T-4-023(SCR-032 캡처)뿐.
...
- **T-4-017(api 해시 probe) 20:08, T-4-023(SCR-032 대표팀 자연 플레이 캡처)·T-4-024(실사용자 플레이 시간 측정 준비) 20:15 Sonnet 5 워크플로 투입.** 브리프 [T-4-023](../archive/offside/tracking/briefs/T-4-023.md)·[T-4-024](../archive/offside/tracking/briefs/T-4-024.md). 파일 소유권: T-4-017 `apps/api/src/test/*`·`packages/fixtures/src/index.ts`, T-4-023 `apps/web/e2e/helpers/*`·`phase4-seed-reachability.test.ts`·`docs/qa/phase34/national-team*.png`, T-4-024 `apps/web/src/engine/funnel.ts`·`packages/contracts/src/analytics.ts`(props가 `strictObject` 화이트리스트라 스키마 갱신 필수). 근거: D-65 사용자 결정, D-62 최소 검증(새 테스트 파일 없이 기존 구조만 갱신, 룰셋 조정 금지).
...
사용자 답: (1) 출시 게이트(T-4-017 해시 probe·SCR-032 대표팀 캡처·실사용자 플레이 측정 준비)는 이 세션이 진행, (2) 기본 팩 0.1.0 유지, (3) Phase 5 PR #77 계속 보류. → [D-65](../archive/offside/tracking/phase-3-4-plan.md). T-4-017 브리프를 D-62 기준으로 갱신하고 T-4-023(SCR-032 캡처)·T-4-024(플레이 시간 측정 준비) 브리프를 새로 쓴다.

## 2026-09-05 (19:50, 오케스트레이터 일시 중지 — T-4-022 통합 반영·워커 정리)

사용자 지시 "작업 일시 중지하고 최신 main 상태 체크"에 따라 머지·투입을 멈추고 main을 확인했다. **19:01 PR #85(T-4-022, Codex 인계·D-62~64)가 `a2354a5`로 머지**되어 이 세션이 열어 둔 PR #78·#79·#82·#83·#84가 통합 브랜치 경유로 모두 MERGED 처리됐고, T-4-012·T-4-018·T-4-005a/b/c는 Codex가 구현해 함께 반영됐다. main CI 성공, staging 200. 보드 상단의 "현재 인계 현황 — T-4-022" 표와 D-62(최소 검증이 D-61을 대체)가 이 로그의 이전 워커별 기록보다 우선한다.

정리: 검증 큐(#83 실행 중, #84·#78·#82·#79 대기)는 대상이 모두 머지돼 종료. T-4-012 Claude 워커는 옛 base(`518db98`)에서 같은 범위를 진행 중이라 19:50 중단(커밋 3개는 워크트리에 참조용 보존). 머지된 워커 워크트리·로컬 브랜치 정리. 현황판을 이 상태로 재게시.

남은 항목(D-62 기준 출시 게이트, 사용자 결정 필요): 기본 팩 0.1.0 유지 vs 0.2.0/0.3.0 활성화(D-56), T-4-017 해시 probe, 실사용자 플레이 시간 측정, SCR-032 대표팀 자연 플레이 캡처, Phase 5 PR #77 재개. 재개 지시 전까지 코드 작업·투입 없음.
...
## 2026-09-05 (17:28, PR #80 머지 — injury.spec 레이스 해소)

**PR #80(T-4-020 핫픽스) `01be661` squash 머지.** 오케스트레이터 체인 `CHAIN EXIT 0`(e2e 98 passed, load 62~128에서도 injury.spec 통과). 이로써 오늘 "부하 플레이크"로 분류했던 injury.spec 60초 타임아웃은 원인이 제거됐다 — 앞으로 같은 실패가 나오면 부하로 넘기지 말고 새 원인으로 본다. 검증 큐는 #78(T-4-014) → #79(T-4-016)로 자동 진행.
...
**T-4-005 분할(D-59)**: [T-4-005a](../archive/offside/tracking/briefs/T-4-005a.md)가 화면 분기(`career-route.ts`)·결정 화면 디스패처(`event.tsx`)·`routes/-phase4/bodies.ts` 레지스트리·SCR-022·SCR-032를 만들고, [T-4-005b](../archive/offside/tracking/briefs/T-4-005b.md)는 (a) 머지 뒤 레지스트리에 SCR-021·018·016·024 본문 4개만 추가하며, [T-4-005c](../archive/offside/tracking/briefs/T-4-005c.md)는 대시보드(`index.tsx`)·결과 카드(`event_.result.tsx`)·챕터 헤더(`chapter.tsx`)·`labels.ts` 추가만 맡아 (a)와 나란히 간다(T-4-014 머지 뒤). 세 묶음 모두 D-61로 새 테스트 없이 기존 체인 통과 + 360px 스크린샷(일회성 스크립트)을 증거로 한다.

**PR #72 검증**: 1차 체인은 부하(load 170)로 domain·web 시간 예산 테스트가 타임아웃, 격리 재실행에서도 career-01 1,000회(10초 예산 → 23초)·career-02 1,000회(90초 → 157초)가 부하로 실패해 두 파일만 다시 단독 재실행한다(GitHub CI Quality·Browser gates는 둘 다 녹색). PR #73도 같은 절차.

## 2026-09-05 (16:02, 사용자 지시 — 테스트 코드 신규 작성 보류 D-61)

사용자: "테스트 코드는 앞으로 작성하지마, 나중에 추가하면되니까." → [D-61](../archive/offside/tracking/phase-3-4-plan.md). 이후 브리프에서 새 테스트 요구를 뺐다(T-4-012 §4 보류·회귀 테스트 문구 무시, T-4-015 C14 보류, T-4-016 테스트 추가 삭제, T-4-017 전체 보류, T-4-006 §5 e2e 보류). 워커 규칙([README](README.md)·[브리프 양식](../archive/offside/tracking/worker-brief-template.md))에 반영. 진행 중인 워커(T-4-008·009·010·011·014)는 지시 이전에 투입된 브리프대로 끝내고, PR #72(테스트 전용)는 그대로 머지한다.

## 2026-09-05 (오후 2, 감사 재검증 종결·D-60 정정, 후속 브리프 T-4-012·015·016·017, PR #72·#73 검증 시작)

**감사 재검증 종결(`wf_28e6ca87-b91`, 11건 × 3렌즈)**: 확인 10건 — content-dsl: F1 `recentFormAvg`가 평점 0건이면 0을 내 출전 0경기 선수에게 EVT-SLUMP-010이 열림, F2 `season.stats.*` 8필드가 상수 0이고 EVT-CON-010의 phase `TRANSFER_WINDOW`는 캘린더 phase 매핑과 어긋나 영원히 거짓, F4 INJURY previewEffects 검증기 부재(P4-3), F5 `{manager}` 서사 토큰이 팩 사전 이름; effects-ownership: F6(P1) EXPIRED 시장 전부 거절이 안전 잔류 계약을 맺지 않음(C4·C10과 동일 결함, 심각도 P1로 상향), F7 step 7 재계약 수락이 시즌 중 `state.contract`를 즉시 교체해 D-47 약속 판정이 새 약속 기준으로 바뀜, F8 재계약 뒤 결산이 INTEREST 시장을 열어 D-43 (a) "시장 건너뛰고 잔류 확정" 위반, F10 임대 복귀 시 원소속 감독 이력 소실(경질된 `mgr-1` 복귀), F9=C1, F11=C7 중복. 반박 1건 — F3(FAIL outcome 회복 경로 검증기)은 T-4-003 outcome 검증기가 이미 요구를 만족. **최종 집계: 감사 finding 확인 22건(고유 20건), 반박 9건.** 이 로그 상단의 "확인 14건"은 1차 집계이며 이 문단이 단일 최종 집계다.
...
**결정 D-56~D-58**(phase-3-4-plan.md): D-56 활성 팩 0.1.0 유지 + DEV 전용 팩 오버라이드(`offside:e2e-content-pack`)로 Phase 4 화면 도달성 확보(0.2.0 활성화는 LINE TEST 뒤 별도 결정). D-57 대시보드 관계·평판 점진 공개 3단계(계약 전 / 첫 결산 전 / 첫 결산 후, 상태 파생·새 필드 없음). D-58 디자인 PR #66은 T-4-007로 같은 브랜치에 main merge(사용자 커밋 보존) → 충돌 해결 → 새 화면 디자인 정합 → 머지, 순서 T-4-007 ‖ T-4-006(domain) → T-4-005 → T-4-006(e2e).

**브리프**: [T-4-007](../archive/offside/tracking/briefs/T-4-007.md)(PR #66 재통합, 충돌 분석 `merge-tree` 3파일 + #68 추가 예상), [T-4-005](../archive/offside/tracking/briefs/T-4-005.md)(Phase 4 화면 7종·점진 공개·SCR-023 변형·TEST-E2E-004, 도달성 감사·정지 규칙 포함), [T-4-006](../archive/offside/tracking/briefs/T-4-006.md)(3시즌 fixture `career-13-integration`, 불변 property 7종, RULE-TIME-004 측정, Snapshot 크기, 세션 길이 재측정, e2e 3회, 완료 조건 표 증거 — 테스트 전용, PR 분할 허용). 세 브리프는 Codex가 쓴 T-4-004 양식(목표/참조·선행/번호 절/테스트·인수/범위·규칙)을 따른다.

**정리**: T-3-003 legacy Sonnet 터미널·원격 브랜치·`~/.offside-orch/active.txt`를 정리했다(아래 결과 참조).
...
**다음 Wave 브리프**: [T-3-004](../archive/offside/tracking/briefs/T-3-004.md)·[T-4-002](../archive/offside/tracking/briefs/T-4-002.md)·[T-4-003](../archive/offside/tracking/briefs/T-4-003.md)를 병렬 슬롯 3개로 확정했다. T-3-004는 Phase 3 상태를 바꾸지 않고 strict/runtime/sync 증거만 만든다. T-4-002는 부상 발생 뒤 강제 pending을 일반 슬롯보다 우선하고 재발 창을 additive episode counter로 추적한다. T-4-003은 시즌 사이 감독 예약(`nextManager`)·주장단 상태·실패 누계를 additive 상태로 두며, D-52 원문에 맞춰 전용 생성기 대상(INJURY/NATIONAL_TEAM/RUMOUR)만 일반 이벤트 후보에서 제외한다. 두 B 트랙 PR은 먼저 머지된 쪽의 공용 타입을 다른 쪽이 main merge로 보존한다.

**후속 브리프 선작성(17:11)**: [T-3-005](../archive/offside/tracking/briefs/T-3-005.md)와 [T-4-004](../archive/offside/tracking/briefs/T-4-004.md)를 작성했다. T-3-005 사전 감사에서 현재 main의 step 7은 CONTRACT 슬롯만 갖고 `selectEligibleEvents`는 RUMOUR을 제외하므로, 0.2.0 활성화만으로 `EVT-CON-010`이 도달하지 않는 계획-구현 간격을 확인했다. 웹 워커가 가짜 pending이나 조용한 domain 변경으로 덮지 않고 최소 `RESOLVE_EVENT` 재사용 전이를 결정 게이트로 올리게 했다. T-4-004는 skeleton에 없는 평점 대안 기준(`minRatingTenths:70`)과 이벤트 참조를 룰셋 데이터로 추가하고, 부상 자동 사양·managerTrust 불변·NATIONAL_DEBUT 1회 예약을 검증하도록 고정했다.
...
**투입**: T-2-012(LINE TEST 준비, [브리프](../archive/offside/tracking/briefs/T-2-012.md), 포트 5189)와 T-3-001(타입 슬라이스, [브리프](../archive/offside/tracking/briefs/T-3-001.md), 포트 5188)을 나란히. 파일 경계: T-2-012는 domain·content·contracts career-state/commands/careers를 만지지 않고, T-3-001은 apps·platform·contracts analytics/service-seasons를 만지지 않는다. T-0-010은 main 푸시 staging 자동 배포·smoke 확인 뒤 completed → **10:40 completed**: `abf9bfa` 런은 10분 뒤 문서 푸시(`0ba3ea0`)가 워크플로 concurrency `cancel-in-progress`로 취소했고, `0ba3ea0` 런이 Quality·Browser·Deploy staging 전부 success. staging api health·web root 200 직접 확인. 교훈: 문서만 바뀐 푸시가 진행 중인 main 런을 취소하고 같은 코드를 다시 배포한다 — `paths-ignore: docs/**`(그리고 main 푸시는 cancel-in-progress 대신 대기)가 필요하다. Phase 0 전 항목 종료.
**D-54·D-55**([phase-2-plan.md](../archive/offside/tracking/phase-2-plan.md)): 서비스 시즌 포인터는 API 환경 변수 `ACTIVE_SERVICE_SEASON_ID`(staging `svc_line_test`, production 미설정 → 503), `service_seasons.is_test`로 테스트 보관함 구분, API-SVC-001 공개 라우트, 신규 생성만 PRESEASON·ACTIVE 허용. 분석 이벤트는 platform 큐 → `POST /v1/analytics/events` → D1 `analytics_events`, zod 화이트리스트, 신규 이벤트 4종(`funnel_reached`·`step_passed`·`season_settled`·`choice_selected`)으로 D-31 네 지표를 낸다. Analytics Engine 미사용.
