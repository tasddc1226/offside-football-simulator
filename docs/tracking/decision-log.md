...
- 작업: [T-7-014](briefs/T-7-014.md)(api·contracts, API-PRES-001·002) → 머지 뒤 [T-7-015](briefs/T-7-015.md)(web·ui css). 사용자 승인 뒤 투입.
...
- 결정: (1) 핫픽스 3건을 즉시 투입 — [T-7-011](briefs/T-7-011.md) e2e 정합(`apps/web/e2e/**`만, 테스트 완화 금지), [T-7-012](briefs/T-7-012.md) eyebrow 대비(ui), [T-7-013](briefs/T-7-013.md) 제안 비교 dl(web). (2) 과도기 판정 규칙: 핫픽스가 머지될 때까지 PR 체인의 e2e는 **baseline(60건) 대비 새 실패 없음** + 단위 테스트 그린 + 오케스트레이터 화면 확인(ego-browser)으로 판정하고, 머지 뒤 전체 스위트 그린을 게이트로 복원한다. (3) CI에 전체 e2e(또는 축약 세트)를 되돌릴지는 사용자 결정(#179에 기록).
...
- 방법: 사용자 지시("sonnet 서브에이전트 3개, 디자인·게임성·기능 관점, 각자 ego lite 브라우저 세션으로 운영 게임 플레이")대로 Sonnet 서브에이전트 3명을 병렬로 띄워 각자 ego-browser 격리 작업공간에서 offside-lab.com을 플레이했다(디자인 `QA디자인0907` 1시즌+제안 비교·설정·가이드, 게임성 `QA게임성0907` 6시즌, 기능 `QA기능0907` 2시즌). 기존 기록 미접촉·QA 커리어 1개씩·복구 코드 미출력·피드백 미전송·상태 주입 없음. 결과는 [docs/qa/2026-09-07-production-three-perspectives](../qa/2026-09-07-production-three-perspectives/README.md).
...
- **D-66.** T-4-023 캡처로 P4-7 문구 게이트 확인: SCR-032는 "팬·협회 관계", 소집 수락 결과 SCR-014는 전역 라벨 "에이전트 관계 +4". 전역 라벨(`labels.ts` `agent`)은 윤리·관계 화면이 공유하므로 유지하고, 대표팀 두 화면의 문구만 "에이전트 관계(협회 관계의 대리값)"로 맞춘다(문구 2줄, [T-4-028](briefs/T-4-028.md), port 5251). 21:40 사용자 세션의 PR #91 브랜치가 이미 `T-4-025-staging-ui`를 쓰고 #92·#95가 T-4-026·T-4-027을 쓰는 것을 발견해 이 작업을 T-4-028로 재번호(브랜치명 `T-4-025-national-team-wording`은 유지, 머지 제목은 T-4-028). 사용자 PR #91·#92·#95·#77은 이 세션이 건드리지 않는다. 도메인의 `agent` 축 의미 변경·새 축 추가는 하지 않는다(D-62).
...
- **측정은 staging 배포가 전제.** U-017(Actions 결제) 해결 → main 재실행 → staging 배포 뒤 [play-time-measurement.md](../qa/play-time-measurement.md) §6 절차. 남은 출시 게이트: T-4-023(SCR-032 캡처)뿐.
...
사용자: "테스트 코드는 앞으로 작성하지마, 나중에 추가하면되니까." → [D-61](phase-3-4-plan.md). 이후 브리프에서 새 테스트 요구를 뺐다(T-4-012 §4 보류·회귀 테스트 문구 무시, T-4-015 C14 보류, T-4-016 테스트 추가 삭제, T-4-017 전체 보류, T-4-006 §5 e2e 보류). 워커 규칙([README](README.md)·[브리프 양식](worker-brief-template.md))에 반영. 진행 중인 워커(T-4-008·009·010·011·014)는 지시 이전에 투입된 브리프대로 끝내고, PR #72(테스트 전용)는 그대로 머지한다.
...
**D-60(정정 결정, [phase-3-4-plan.md](phase-3-4-plan.md))**: (1) D-50 감독 교체 roll은 결정 스트림 소비 대신 **독립 파생 시드** `seedRng('manager:…')`로 — 기존 골든 `draws` 유지, 상태 word 복사 방식 금지; (2) T-3-002 시장 경쟁자 요약 시드에서 careerId 제거(T-2-006 fork 불변식을 시장 골든에도 적용); (3) D-43 (a) 구현 규칙: step 7 재계약은 `nextContract`로 보관했다가 결산 약속 판정 뒤 교체, 재계약한 시즌은 시장 생략, EXPIRED 전부 거절 = 안전 잔류 체결, FIRST_CONTRACT 전부 거절 거부, 빈 CONTRACT pending은 `ADVANCE`; (4) D-45·D-46 보강: 임대→FA 경로 유령 stint 금지·팬 이월 1회·`toSeasonIndex >= fromSeasonIndex` 불변식, 임대 복귀 감독 이력 보존.

**후속 브리프**: [T-4-012](briefs/T-4-012.md) domain 묶음(C1~C7·C10·C12·C13·F6~F8·F10·F11, PR #72 머지 직후), [T-4-015](briefs/T-4-015.md) content 묶음(F1·F2·F4·C14, T-4-008 머지 뒤), [T-4-016](briefs/T-4-016.md) web 소수정(C8·F5, T-4-011 머지 뒤), [T-4-017](briefs/T-4-017.md) api 해시 probe(career-12·13, PR #72 워커 질문 1의 답 — `apps/api`는 별도 작업으로 분리).
...
**브리프**: [T-4-007](briefs/T-4-007.md)(PR #66 재통합, 충돌 분석 `merge-tree` 3파일 + #68 추가 예상), [T-4-005](briefs/T-4-005.md)(Phase 4 화면 7종·점진 공개·SCR-023 변형·TEST-E2E-004, 도달성 감사·정지 규칙 포함), [T-4-006](briefs/T-4-006.md)(3시즌 fixture `career-13-integration`, 불변 property 7종, RULE-TIME-004 측정, Snapshot 크기, 세션 길이 재측정, e2e 3회, 완료 조건 표 증거 — 테스트 전용, PR 분할 허용). 세 브리프는 Codex가 쓴 T-4-004 양식(목표/참조·선행/번호 절/테스트·인수/범위·규칙)을 따른다.
