...
| 작업    | 이번 통합에서 닫은 구현                                                        | 현재 남은 인수 증거                                      |
| ------- | ------------------------------------------------------------------------------ | -------------------------------------------------------- |
| T-5-002 | Archive+Legacy 원자 저장, D1/API owner GET, 복구·삭제, 버전/checksum 고정       | 최종 head의 구버전 byte/hash 불변과 신규 1.1 복구 재검증 |
| T-5-003 | 실제 압력 입력, 마지막 선택, RETIRE/terminal 명령 거부, 연령 회귀               | 신규 1.1 실제 커리어의 은퇴 명령 QA                      |
| T-5-004 | 원본→5축 정규화, source 연결, 동일 품질 4포지션 공정성 회귀                     | 최종 40k provenance/count/seed와 고득점 도달 증거         |
| T-5-005 | 14종 eligibility·폴백·최고 순간·미선택 에필로그                                | RAW_EVIDENCE 검증임을 유지; 자연 플레이 14회로 오표현 금지 |
| T-5-006 | FULL TIME→Legacy→source 연대기→최종 프로필 및 보관 UX                           | 최종 head 모바일·접근성·실제 1.1 브라우저 QA             |
| T-5-007 | 4포지션 20시즌·원자성·복구·변조·중복 요청 회귀                                 | 최종 40k와 전체 quality/browser/build 증거                |
| T-5-008 | 한국 복무/휴식/U23/특례/멘토링과 Archive 연결                                  | 최종 head 실제 경로 회귀                                 |

Phase 5 전체 완료는 위 검증과 정본의 미충족 조건을 모두 닫은 뒤에만 선언한다.

후속 승인: [밸런스 재설계](phase-5-balance-redesign.md)를 이번 범위에 포함한다.
사용자 요청으로 상시 테스트는 핵심 동작·정합성 중심으로 축소하며 대규모 분석은 별도 실행한다.
아래 과거 기록의 100회 검증은 당시 실행 이력이지, 현재 CI의 필수 반복 횟수가 아니다.
...
## 2. 구현 순서와 상태 — 초기 계획 기록

아래 표는 독립 코어 착수 당시 계획과 그 시점 상태를 보존한 역사 기록이다. `예정`·`대기`는 현재
상태가 아니며, 최신 상태는 위 표와 [검증 기록](phase-5-verification.md)을 따른다.
