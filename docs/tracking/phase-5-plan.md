...
아래 번호는 이 계획의 작업 제안이며 외부 작업 시스템에 자동 배정한 것이 아니다. 사용자 승인으로 #70·#71은 main에 병합됐고 UI #66도 통합됐다. main `3f39511`에서 T-5-003의 독립 은퇴 판정·마지막 선택 코어에 착수했다. Phase 3·4 공유 파일을 수정하는 실제 엔진·저장 통합은 별도로 남긴다.

| 작업    | 범위                                                                              | 선행 조건                               | 상태                                     |
| ------- | --------------------------------------------------------------------------------- | --------------------------------------- | ---------------------------------------- |
| T-5-001 | 통산 기록 projection, 정규화된 Legacy 가중합·밴드, 이미 충족된 엔딩 우선순위 해소 | 현재 SeasonResult                       | #70 main 병합 완료 (`3a5a79e`)              |
| T-5-002 | 통산·업적 정본, 불변 Archive, Legacy 버전·저장 계약                               | T-5-001, 공유 타입 변경 소유권 합의     | #71 독립 코어 병합 (`3f39511`), 저장 통합 대기 |
| T-5-003 | 다년 성장·노쇠, 은퇴 압력, 마지막 선택과 명령                                     | T-5-002, Phase 3·4 엔진 통합 완료       | 독립 판정·선택 코어 구현, 운영 정책·명령·다년 검증 대기 |
