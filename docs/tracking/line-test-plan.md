...
| 3 | FAST·CHAPTER 각 한 시즌 완주 → `analytics_events`에 `funnel_reached` 5단계·`season_settled`가 쌓인다 | 오케스트레이터 | ✅ 14:56 D1 확인(U-016 로그인 뒤): `funnel_reached` 5단계 각 1건(ONBOARDING_STARTED·PLAYER_CONFIRMED·CONTRACT_SIGNED·SEASON_STARTED·SEASON_SETTLED), `season_settled` FAST 1·CHAPTER 1, 전체 114건. 이력: 12:56 예행 20건 배치 503(D1 변수 상한) → T-2-015(PR #52) → 13:32 재예행 배치 202. 오케스트레이터가 넣은 합성 배치 70건은 삭제함 |
...
| 7 | 감시: 매일 5절 쿼리 + `wrangler tail --env staging --format pretty` 1회 | 오케스트레이터 | ✅ 14:56 U-016 완료, 5절 쿼리 실행 확인(`--json`으로 파싱). 일일 실행은 09-08부터 |
...
| 2 | 오케스트레이터 예행: FAST·CHAPTER 각 1시즌 완주 이벤트가 D1에 있다 | ✅ | 3절 #3 — 14:56 D1에서 funnel 5단계·season_settled FAST/CHAPTER 확인 |
