...
| 기술 책임자·오케스트레이터 | Claude (Claude Code 메인 세션) | 명세·ADR·보드 관리, 작업 브리프 작성, 워커(서브에이전트) 투입, 검증 체인 실행, 리뷰, 머지 판단 | 앱 코드 직접 수정 |
| 구현 워커 | Claude Sonnet 5 서브에이전트(격리 worktree) | 브리프 범위의 코드·문서 구현, 커밋/PR | 명세 변경, 범위 밖 수정 |

구현은 Claude Sonnet 5 서브에이전트에게 위임하고, 오케스트레이션과 리뷰·검증은
Claude 메인 세션이 맡는다(2026-09-24, ADR-013·D-85). 오케스트레이터는 브리프 없이
`apps/`, `packages/`를 직접 편집하지 않는다. 코드 변경이 필요하면 브리프를 써서
워커에게 넘긴다. `docs/`와 CI 설정 리뷰는 오케스트레이터가 직접 할 수 있다.
