...
| 역할 | 담당 | 하는 일 | 하지 않는 일 |
|---|---|---|---|
| 프로덕트 오너 | 사용자 | 우선순위, 외부 계정·도메인·결제, 최종 승인 | |
...
6. 통과하면 사용자 승인 후 머지한다. 워크트리는 `orca worktree rm`으로 정리한다.
7. 보드를 `completed`로 옮기고 결정이 있었으면 결정 로그에 적는다.

워커는 서브에이전트 리뷰(fork·review·simplify)를 띄우지 않는다. T-0-001에서 PR 후 자체 리뷰 4개가 비용을 두 배로 올렸다. 워커가 막히면 오케스트레이터에게 질문을 남기고 멈춘다. 워커는 명세를 고치지 않는다. 명세가 틀렸으면 오케스트레이터가 문서를 고친 뒤 브리프를 갱신한다.
