...
병합을 막는 절대 합격선이 아니다. 다만 저장 호환성, 동일 품질의 포지션 공정성, 플레이로 도달 가능한
고득점 경로는 필수 인수 조건으로 유지한다. 과거 파일럿의 목표 미달 판정은 당시 관측으로 보존한다.
참조집단을 발행할 때 provenance·표본 수·seed 무결성도 필수 gate다.
...
`0da4525`는 1.1 제안에서 계약상 역할·출전 시간 약속과 시즌 시작 시점의 전술 적합도·경쟁 순위
예상을 구분하고, 이후 선수·감독 조건이 바뀌면 예상도 달라질 수 있음을 명시했다. 첫 계약과 안전
잔류의 예상이 최초 `START_SEASON` 계산과 일치하는 회귀도 추가했다. START 직후 KEEP 수락으로
감독 신뢰가 바뀐 뒤 재산출되는 순위는 그 이후 상태이며 제안 시점 예상과 같은 값이라고 주장하지 않는다.
...
아래 내용은 이 QA를 실행한 시점의 역사 기록이다. 당시 `loadRetirementArtifacts`는 candidate Legacy
...
minutes 노출 수정 뒤 새 800 evidence `84799b6`도 관찰 목표와 차이가 있다.
...
pack 0.1.0과 0.3.0 모두 YOUTH에서 `고집` 태그 또는 season step 4만 확인하고 포지션 조건이 없다.
situation에는 NPC `이도현`을 하드코딩하고 0.3.0의 rival token에도 같은 이름이 있어, 선수 이름이
이도현이면 문맥상 두 인물이 구분되지 않는다. 이는 동결된 content pack을 이번 통합에서 즉석 수정하거나
