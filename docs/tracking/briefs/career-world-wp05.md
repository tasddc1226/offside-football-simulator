...
- 다음 결정 카드와 이번 시즌 구단 면담 목표 사이, 계약 탭의 기존 계약 정보 앞에 같은 compact summary를 장착하고 기존 일정 탭 순위표로 연결한다.
...
- PR #221의 `recentChronicleItems` 아래와 계약 요약 뒤 후속 영수증 삽입 지점을 피한다. 부모의 final table tuple adapter는 이 작업에서 수정하지 않는다.

## 검증

- D-61: 새 테스트 파일이나 독립 `it`/`test`/golden을 추가하지 않는다. 기존 대시보드 케이스에 1.7 실제 명령 진행, 무확정 상태, 동률/선두/승점 차/BYE 표, 손상·identity mismatch, 홈/계약 mount와 순위표 링크 assertion을 합친다.
- 통과: `pnpm --filter @offside/web exec vitest run 'src/routes/career.$careerId.index.test.tsx'` — 27/27, 기준과 테스트 수 동일.
- 통과: `pnpm --filter @offside/web lint`.
- 통과: `pnpm --filter @offside/web typecheck`.
- 통과: `git diff --check`.
