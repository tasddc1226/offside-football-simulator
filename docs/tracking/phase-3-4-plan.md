...
### D-62 Codex 인계·최소 검증으로 통합 마감 (2026-09-05)

- Claude 세션 한도로 중단된 Phase 3·4를 Codex가 인계한다. 원격에 올라온 PR #78·#79·#82·#83·#84의 커밋을 통합 브랜치에서 보존하며, 미완료 domain 감사(T-4-012)와 화면(T-4-005)을 마무리한다. Phase 5 PR #77 및 밸런스 재설계는 계속 보류한다.
- 현재 사용자 지시인 **“최소한의 필요한 검증”**이 D-61의 일괄 신규 테스트 금지를 대체한다. 기존 체인을 재사용하고, 이번 변경에서 계약 보존·클램프 후 실제 결과·팩 버전·진행 경로를 확인하는 소수 회귀 검증만 보강한다. 대규모 신규 fixture나 테스트 조합 확장은 요구하지 않는다.
- 기존 전체 E2E 1회와 실패 원인 수정 후 해당 흐름 재검증을 병합 게이트로 삼는다. 3회 반복 의무·전용 추가 해시 probe·실사용자의 플레이 시간 측정은 출시 전 검증 백로그로 분리하며, 자동화 실행 시간을 실제 플레이 시간으로 보고하지 않는다.
- 사용자가 **로컬 검증 및 PR CI 통과 후 main 병합**을 승인했다. 기존 플레이 기록 삭제, 원격 DB 초기화, 기본 콘텐츠 팩 변경은 승인 범위에 포함하지 않는다.
- 현재 증거와 남은 출시 게이트는 [통합 완료 기록](../qa/phase34-completion.md), [완료 조건별 검토](../qa/phase34-acceptance.md)를 우선한다. 과거 워커별 작업 범위·모델·테스트 금지 메모는 당시 이력이다.

### D-63 이적 루머의 실제 진입 경로

- `RUMOUR`는 일반 경기 EVENT 슬롯에서 추첨하지 않는다. 시즌 이적 창(step 7)의 **제안 없는 CONTRACT 체크포인트**에서 다음 `ADVANCE`가 현재 상태로 평가한 루머 후보를 받아 EVENT를 연다. 해당 결정 슬롯을 재사용하므로 경기 재실행이나 별도 달력 step은 추가하지 않는다.
- 후보 DTO에 선택 필드 `slot: 'TRANSFER_WINDOW'`를 추가한다. 기존 필드만 있는 명령은 기존 일반 이벤트 경로를 유지하고, 창 전용 후보는 일반 슬롯에서 제외한다. 전용 후보가 없으면 기존처럼 다음 결정으로 진행한다.
- 시즌 중 재계약 제안이 있는 경우에는 계약 결정을 우선한다. 제안을 거절해 비운 뒤 루머를 강제로 추가하지 않는다. 루머 선택은 기존 `RESOLVE_EVENT`와 SCR-019 이벤트 변형을 재사용한다.

### D-64 결과 화면은 실제 적용량과 당시 기록을 표시

- SCR-014는 해당 결과 revision의 직전·직후 Snapshot 차이를 보여준다. 98→100이면 요청 Effect +5가 아니라 실제 적용 +2이며, 인기 centi는 표시 단위로 환산한다.
- 부상 결과는 해당 revision의 재활 기록이 가리키는 episode를 표시한다. 과거 결과에 현재 진행 상태를 섞거나 RNG를 다시 실행하지 않는다.
- 과거 가져오기 등으로 전후 Snapshot이 없으면 정의된 효과임을 명시한 대체 표시를 쓴다. 원인 문구는 사용자용 결과 설명을 사용하고 기술용 sourceId는 저장 기록에 남긴다.

### D-65 재개 범위·기본 팩·Phase 5 (2026-09-05 20:00, 사용자 결정)

- T-4-022(PR #85) 통합 뒤 남은 D-62 출시 게이트는 **이 오케스트레이터 세션이 진행**한다: T-4-017 Node/workerd 해시 probe(D-62 최소 검증 기준으로 재개), SCR-032 대표팀 자연 플레이 캡처(T-4-023), 실사용자 플레이 시간 측정 준비(T-4-024). 브리프 단위로 Sonnet 5 워커에 투입하고 기존 체인으로 검증·머지한다.
- **기본 콘텐츠 팩은 0.1.0 유지(D-56)**. 0.2.0/0.3.0 기본 활성화는 LINE TEST 기준선·콘텐츠 승인 뒤 별도 결정.
- **Phase 5 PR #77은 계속 보류.** 이 세션은 손대지 않는다.
