...
| # | 조건 | 측정값 | 통과 | 근거 | 비고 |
|---|------|--------|------|------|------|
| 1 | 360px 모바일과 키보드로 첫 계약까지 완료한다. | 뷰포트 360×780(playwright.config.ts 전역), 키보드 전용 완주 1건 통과(3회 연속) | ✅ | `e2e/keyboard.spec.ts` | (이 PR) SCR-002 포지션 구분 탭(`TabsList`)이 `<RadioGroup>`에 중첩돼 Radix의 두 roving-tabindex 관리자가 충돌하던 버그를 고쳤다 — `Tabs`를 `RadioGroup`의 형제로 분리(`career.$careerId.create.tsx`). `e2e/keyboard.spec.ts`도 골키퍼 그룹 우회 없이 "공격수 탭 → 윙어" 실제 흐름으로 갱신했다. |
...
| 5 | 포지션 전환 뒤에도 최초 선호 포지션과 현재 주포지션을 구분해 표시한다. | 대시보드(SCR-029)·이벤트 화면 `PlayerHeader`가 두 값을 구분 표시, 웹 단위 테스트 2건("같음"/"다름") 통과 | ✅ | `apps/web/src/shared/labels.ts`(`positionHeaderField`), `career.$careerId.index.tsx`, `event-screen.tsx`, `career.$careerId.index.test.tsx` | (이 PR) `PlayerHeader`에 `caption` 필드를 추가해, 주포지션과 선호 포지션이 같으면 "선호 포지션과 같음"을, 다르면 "선호 {포지션}"으로 구분해 보여준다. 포지션 전환 명령이 아직 없어 "다름" 테스트는 TanStack Query 캐시를 직접 조작해 상태를 구성했다. |
...
1. **(고침 — 이 PR) SCR-002 포지션 구분 탭 키보드 도달 불가(가장 심각, 출시 차단 기준 해당 가능)**: `TabsList`(골키퍼/수비수/미드필더/공격수)가 `<RadioGroup>` 안에 중첩돼(`career.$careerId.create.tsx:330-360`) Radix의 두 roving-tabindex 관리자가 충돌한다. 트리거 4개 전부 `tabindex="-1"`(선택된 것 포함)이라 Tab으로 절대 도달할 수 없다 — 직접 확인: Tab이 "왼발" 라디오에서 곧장 (탭이 아니라) 포지션 카드로 건너뛴다. 대시보드(SCR-029)의 동일 `Tabs` 컴포넌트는 `RadioGroup`에 중첩되지 않아 정상 작동한다(직접 확인, Tab으로 진입). 08 "출시 차단 기준: 키보드로 P0 흐름 완료 불가"에 해당할 수 있다. → `Tabs`를 `RadioGroup`의 형제로 분리해 고쳤다. `e2e/keyboard.spec.ts`도 우회 없이 실제 탭 전환 흐름으로 갱신했다.
2. **(고침 — 이 PR) COMMITTING 중 뒤로 가기 경고 대화상자 없음**: 06 문서가 요구하는 "명령 응답 대기 중 뒤로 가기 → 경고 대화상자"가 실제로는 구현돼 있지 않다(`platform.lifecycle.onBackPressed`가 어디서도 호출되지 않는 죽은 코드). `resilience.spec.ts`(d)는 실제 동작(경고 없이 재진입 시 확정 결과가 보임)을 그대로 특성화했다. → `useCommittingExitGuard`(`beforeunload`)를 SCR-004 확정·이벤트 확정 COMMITTING 구간에 연결했다. `platform.lifecycle.onBackPressed`(popstate) 기반 경고 대화상자는 시도하지 않았다 — TanStack Router의 `@tanstack/history`가 각 히스토리 항목의 내부 키(`state.__TSR_index`)로 popstate 방향을 계산하는데, 이 경고를 위해 직접 넣는 더미 히스토리 항목엔 이 키가 없어 라우터의 내부 인덱스 추적이 깨짐을 소스 코드로 확인했다(`use-committing-exit-guard.ts` 주석). 브리프가 허용한 최소 구현(대화상자 없이 beforeunload만)으로 남겼다.
3. **(고침 — 이 PR) 결과 확정 시 aria-live 영역 없음**: 08 접근성 체크리스트가 요구하는 "결과 변화가 aria-live로 한 번만 낭독"이 실제로는 없다. 이벤트 확정→결과 화면 전환 경로 어디에도 `aria-live`/`role=status`가 없다(`ScreenStateView`의 announcer는 어디에도 마운트되지 않는 죽은 코드). `a11y.spec.ts`가 `MutationObserver`로 실제 개수(0건)를 기록했다. → SCR-014 결과 화면에 전용 `aria-live="polite"` 영역을 추가했다(빈 문자열로 마운트 뒤 결과가 정해지면 한 번만 텍스트 갱신). `a11y.spec.ts`는 "정확히 1회 갱신"을 기대하도록 바꿨다.
4. **(고침 — 이 PR) 선호 포지션(`preferredPosition`)이 화면에 전혀 노출되지 않음**: 완료 조건 표 #5. 계약(스키마)엔 있지만 대시보드·이벤트 화면 모두 `primaryPosition`만 읽는다. → `PlayerHeader`에 보조 문구(`caption`)를 추가해 두 값이 같으면 "선호 포지션과 같음", 다르면 "선호 {포지션}"으로 구분해 보여준다.
5. **(고침 — 이 PR) ADR-008 국외 이전 항목 미기재**: 완료 조건 표 #14. 개인정보 처리방침에 "처리 위탁"(Cloudflare) 언급은 있으나 이전받는 자·국가·항목·목적·보유기간을 명시한 절은 없다. → "개인정보의 국외 이전" 절과 표(Cloudflare, Inc./Google LLC)를 추가했다.
6. **(고침 — 이 PR) `screen_viewed` 분석 이벤트의 `careerPhase`가 SCR-002/003/004에서 전부 `'YOUTH'`로 고정**: `career.$careerId.create.tsx:115`·`style.tsx:68`·`confirm.tsx:52`. 다만 세 화면 모두 동일하게 고정돼 있고 Phase 1엔 YOUTH 외 다른 생성 단계가 없어, 의도된 단순화인지 방치된 자리표시자인지 애매하다 — 확실한 버그로 단정하지 않는다. → 의도된 단순화로 확정하고, 세 화면이 공유하는 상수 `PLAYER_CREATION_CAREER_PHASE`(`shared/player-draft.ts`)로 통일했다(이유를 주석에 남김).
