...
후속 구현: 이 표는 **통합 전 감사**다. 이후 실제 RETIRE 명령·로컬 Archive 원자 저장과
4포지션 20시즌 회귀를 추가했다. 최신 충족/미충족 구분은
[통합 브리프](briefs/T-5-003-local-integration.md)를 참조한다. 서버 저장·LegacyResult·화면·
전체 인수 조건은 여전히 미완료이며 아래 최초 지적을 모두 해결한 것으로 해석하지 않는다.
