...
# --- 손으로 유지하는 데이터 ---
SCREENS = [
    ("SCR-034", "온보딩", "예정", "T-1-007"),
    ("SCR-001", "허브(홈)", "부분", "빈 상태만. 커리어 카드·이어하기·삭제는 T-1-007"),
    ...
    ("SCR-010", "계약 확정", "예정", "T-1-009"),
    ("SCR-029", "대시보드", "예정", "T-1-009"),
    ("SCR-030", "설정·데이터(복구·삭제)", "예정", "T-1-012"),
...
LAYERS = [
    ...
    ("engine-client", "브라우저 실행기", ["명령 실행기·멱등성·Snapshot 복구", "Web Worker 시뮬레이터", "동기화 클라이언트(재시도·409 처리)", "룰셋 배선 (T-1-001)"], ["화면 배선·충돌 화면 (T-1-007·011)"]),
    ...
    ("web · ui", "React 화면", ["라우터·디자인 토큰·허브 빈 상태", "Pretendard 동적 서브셋(2109KB→269KB)", "Button·Card 등 기본 부품", "Radix RadioGroup·Dialog·Tabs, ChoiceCard·CompareCards 등 부품 10종", "Playwright + axe E2E, 브라우저 Worker 해시 검증 (T-1-010)"], ["화면 12개 (T-1-007~012)", "E2E 완료 조건 표·5분 세션 측정 (T-1-014)"]),
...
ETA = [
    ...
    ("오늘 22시 전후", "T-1-007 머지 → 선수 만들기(T-1-008)·진로~계약·대시보드(T-1-009) 동시 투입"),
    ("오늘 자정 전후", "Wave 2 머지. 허브에서 커리어 생성까지 로컬 동작"),
