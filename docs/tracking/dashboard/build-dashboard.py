...
NOW = ("Phase 0·1은 끝났고 Phase 2는 코드가 끝나 사용자 게이트(플레이테스트·LINE TEST)만 남았습니다. "
       "LINE TEST 준비 코드(T-2-012)가 정오에 머지됐고 운영 계획(T-2-013)도 썼습니다. staging 예행은 통과했지만 분석 이벤트 20건 배치가 D1 상한에 걸려 유실되는 결함을 찾아 T-2-015로 고칩니다(첫 워커 종료 뒤 투입). 공개까지 남은 사용자 일은 U-014·U-015·U-016입니다. "
       "오늘 오전 Phase 3(계약·이적)과 Phase 4(부상·관계·평판)를 병렬로 열었고, 워커 3명(T-4-001·T-3-002·T-3-006)이 동시에 돌고 있습니다. "
...
ETA = [
    ...
    ("9/4~", "T-2-015 최우선 투입(첫 워커 종료 뒤) → 머지 뒤 재예행. T-3-002 뒤 T-3-003(명령·결산 배선), T-4-001 뒤 T-4-002·T-4-003. U-005 플레이테스트 뒤 T-2-010. Orca PR 게이트 유지 여부는 사용자 결정"),
]
DECISIONS = [
