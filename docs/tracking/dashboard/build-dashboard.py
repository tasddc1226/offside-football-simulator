...
NOW = ("Phase 0·1·2 코드는 끝났고 Phase 2는 사용자 게이트(플레이테스트 U-005·LINE TEST 9/8 시작 예정, U-014·U-015)만 남았습니다. "
       ...
       "12:30 세션 한도로 워커 4개가 중단됐다가 15:20 WIP를 이어받아 재개했고, 그 사이 사용자가 디자인 PR #66(T-4-007 완료)과 Phase 5 PR #70·#71을 머지했습니다. 코드 감사는 확인 22건(고유 20건)·반박 9건으로 종결했고 정정 결정 D-60(감독 교체 roll 파생 시드, 시장 시드 careerId 제거, step 7 재계약 교체 시점)을 남겼습니다. 수정은 소유권별 묶음 T-4-012(domain)·T-4-013(api)·T-4-014(web)·T-4-015(content)·T-4-016(web 문구)·T-4-017(api 해시 probe)로 나눠 투입합니다. 16:02 사용자 지시(D-61)로 새 테스트 코드 작성은 보류하고 기존 테스트·골든 갱신만 합니다. 16:30~16:38 PR #72(T-4-006 domain)·#73(T-4-013)을 머지하고 T-4-012(감사 domain 수정)를 투입했으며, 16:45 PR #75(T-4-008 팩 0.3.0)를 머지하고 T-4-015(content 감사 수정)를 투입했으며, 16:48~16:50 PR #74(T-4-010 e2e 안정화)·#76(T-4-011 SCR-020 잔류 결과)도 머지해 오후 머지 5건, T-4-016(web 문구)을 투입했습니다. 16:56 PR #78 검증에서 main 통합 회귀(#74 e2e 헬퍼 × #76 STAY URL)를 발견해 핫픽스 T-4-019(PR #81)를 17:18 머지해 해소했고, injury.spec 반복 타임아웃의 실제 원인(테스트 헬퍼 레이스)도 T-4-020(PR #80, 17:28 머지)으로 고쳤습니다. #78 2차 체인에서 같은 원인의 잔여(season.spec:17 랜덤 seed)가 하나 더 나와 T-4-021(PR #83)로 고쳤고, T-4-015(content 감사)도 PR #82로 완료됐습니다. 검증 큐는 앞 PR 머지를 기다리는 게이트를 달아 #83→#82→#78(T-4-014)→#79(T-4-016) 순으로 돌고 있으며 staging 배포는 복구됐습니다. 남은 워커는 T-4-009(PR 개설 중)·T-4-012(체인 실행 중)이고, T-4-009 머지 뒤 T-4-005a·c(Phase 4 화면)를 투입합니다. "
...
LAYERS = [
    ...
    ("content", "이벤트·룰셋 데이터",
     ["프로토타입 팩 0.1.0(이벤트 10개)",
      ...
      "콘텐츠 팩 0.3.0: Phase 4 이벤트 12종·포지션 챕터 3종·500 seed 도달성 (T-4-008, PR #75)"],
     ["감사 content 묶음: season.stats 파생·슬럼프 게이트·EVT-CON-010 phase·INJURY previewEffects 검증기 (T-4-015, PR #82 검증 대기)"]),
    ...
    ("api", "Cloudflare Workers + D1",
     ["D1 스키마·migration",
      ...
      "api 100회 병렬 테스트 타임아웃 60초 (T-4-013, PR #73)"],
    ...
    ("web · ui", "React 화면",
     ["라우터·디자인 토큰·허브 빈 상태",
      ...
      "e2e 간헐 실패 안정화·CI 실패 아티팩트·INTEREST 시드 케이스 (T-4-010, PR #74)",
      "SCR-020 잔류 결과 STAY 카드·loader redirect 튕김 제거 (T-4-011, PR #76)",
      "[핫픽스] e2e 헬퍼 `&interested=` 허용·season.spec 프리시즌 단언 — main 회귀 해소 (T-4-019, PR #81)",
      "[핫픽스] injury.spec poll textContent 타임아웃 — 반복 플레이크 원인 제거 (T-4-020, PR #80)"],
     ["Phase 4 화면 SCR-016·018·021·022·024·032, 관계 수치 점진 공개 (T-4-005)",
      ...
      "감사 web 수정: step 7 전부 거절 문구·휴대폰 탭 시장 사유 (T-4-014, PR #78 검증 중)",
      "SCR-020 RETURN 관계 문구·{manager} 서사 토큰 (T-4-016, PR #79 대기)",
      "offers 화면 eyebrow 시장 사유별 (T-4-018, PR #78 머지 뒤)",
      "[핫픽스] season.spec:17 STAY→프리시즌 도착 허용 (T-4-021, PR #83 검증 중)"]),
