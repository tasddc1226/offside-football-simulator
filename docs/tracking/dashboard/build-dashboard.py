...
NOW = ("Phase 0·1은 끝났고 Phase 2는 코드가 끝나 사용자 게이트(플레이테스트·LINE TEST)만 남았습니다. "
       "LINE TEST 준비 코드(T-2-012)가 정오에 머지됐고 운영 계획(T-2-013)도 썼습니다. staging 예행에서 찾은 분석 이벤트 유실·오류 문구·온보딩 안내 결함 3건은 T-2-015(PR #52)로 13:21에 머지됐고 staging 재예행까지 통과했습니다. 오후에 팩 0.2.0·팀 12(T-3-006)가 머지됐고 이적시장 생성기(T-3-002)는 리뷰 수정 1건 뒤 13:33에 머지됐고, 이어서 명령 처리기·결산 배선(T-3-003)을 투입했습니다. 공개까지 남은 사용자 일은 U-014·U-015·U-016입니다. "
       "오늘 오전 Phase 3(계약·이적)과 Phase 4(부상·관계·평판)를 병렬로 열었고, 워커 2명(T-4-001·T-3-003)이 돌고 있습니다. "
...
ETA = [
    ...
    ("9/4~", "T-3-003 뒤 T-3-004(계약·api·동기화)·T-3-005(화면)(명령·결산 배선), T-4-001 뒤 T-4-002·T-4-003. U-005 플레이테스트 뒤 T-2-010. Orca PR 게이트 유지 여부는 사용자 결정"),
]
DECISIONS = [
