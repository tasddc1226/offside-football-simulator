...
NOW = ("Phase 0·1은 끝났고 Phase 2는 코드가 끝나 사용자 게이트(플레이테스트·LINE TEST)만 남았습니다. "
       "LINE TEST 준비와 staging 재예행이 끝났고 공개까지 남은 사용자 일은 U-014·U-015입니다. Phase 3에서는 팩 0.2.0·팀 12(T-3-006), 이적시장 생성기(T-3-002), 명령 처리기·결산 배선(T-3-003, PR #54)이 모두 머지됐습니다. 트랙 B 타입 슬라이스(T-4-001)도 머지돼 Phase 4 룰 작업(T-4-002 부상·T-4-003 관계)을 열 수 있습니다. "
       "T-3-003 완료를 경계로 기존 Claude 세션은 정지했고 Codex가 오케스트레이터를 인계했습니다. 이후 구현·테스트 코드는 Orca의 gpt-5.6-luna reasoning max 워커에게만 위임하며 동시 최대 3개를 유지합니다. "
...
ETA = [
    ...
    ("다음", "T-3-004(계약·api·동기화), T-4-002(부상), T-4-003(관계)를 최대 3개 병렬 투입. T-3-005는 T-3-004 뒤, T-4-004는 T-4-002·003 뒤"),
]
DECISIONS = [
