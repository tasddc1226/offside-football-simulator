...
LAYERS = [
    ...
    ("contracts", "API·명령 스키마(Zod)", ["요청·응답 봉투·오류 코드·프로필 설정", "커리어 동기화 GET/PUT·Snapshot 봉투", "Phase 1 명령 payload 판별 유니온·CareerState·PlayerPublic(잠재력 비노출)·복구·삭제 스키마 (T-1-006)", "api 복구·삭제 라우트가 contracts 스키마로 검증 (T-1-012)"], ["auth·병합 스키마 (T-1-013)", "선수 성별·선호 포지션 스키마·migration (T-1-016)"]),
...
ETA = [
    ...
    ("9/3 오전~오후", "성별·선호 포지션(T-1-016) 머지 → E2E 완료 조건(T-1-014) 투입, Google 연결(T-1-013) 리뷰"),
