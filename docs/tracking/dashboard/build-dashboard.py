...
SCREENS = [
    ...
    ("SCR-030", "설정·데이터(복구·삭제)", "부분", "테마·모션·글자 크기·기본 모드·버전 (T-1-007), 동기화 행 (T-1-011), 복구 코드·프로필 복구·삭제·기기 데이터 삭제 (T-1-012). Google 행·로그아웃은 T-1-013 진행 중"),
    ("법적 문서", "개인정보·약관", "구현", "본문 초안 (T-1-012). 운영자명·연락처는 U-010"),
]
LAYERS = [
    ("domain", "결정론 시뮬레이터", ["시드 RNG·canonical hash·golden fixture", "CREATE_CAREER·RESOLVE_EVENT·ADVANCE", "효과 5종·태그·지연 효과", "선수 모델·Base OVR 59·DRAFT→확정·이벤트 제시 (T-1-001)", "제안 생성·계약 확정, golden 첫 계약까지 (T-1-005)", "선수 성별(결과 불변)·선호/현재 포지션 분리·golden 갱신 (T-1-016)"], []),
    ...
    ("contracts", "API·명령 스키마(Zod)", ["요청·응답 봉투·오류 코드·프로필 설정", "커리어 동기화 GET/PUT·Snapshot 봉투", "Phase 1 명령 payload 판별 유니온·CareerState·PlayerPublic(잠재력 비노출)·복구·삭제 스키마 (T-1-006)", "api 복구·삭제 라우트가 contracts 스키마로 검증 (T-1-012)", "선수 성별·선호/현재 포지션 스키마 (T-1-016)"], ["auth·병합 스키마 (T-1-013)"]),
...
ETA = [
    ...
    ("9/3 오후", "Google 연결(T-1-013)·E2E 완료 조건(T-1-014) 리뷰·머지"),
