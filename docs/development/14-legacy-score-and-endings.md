...
이 문서의 모집단/밴드 수치는 계속 관찰하되 작은 테스트 통과로 완료 처리하지 않는다. 2026-09-06
사용자 결정에 따라 합성 grade 비율은 단독 merge hard gate가 아니다. 저장 호환성, 동일 품질 포지션
공정성, Archive 근거의 고득점 도달성, population provenance·count·seed 무결성은 필수다.
...
- 가중치와 정규화 상한표는 독립 `legacyVersion` 정책과 정의 checksum으로 버전 관리한다. 원본 ruleset/content manifest는 수정하지 않고 Archive에 함께 고정한다. 1.0.0의 실행 정본은 `LEGACY_POLICY`이며 참조 모집단은 별도 `packages/content/legacy/1.0.0` 아티팩트다.
...
| 요소        | 입력 정본                                                                       | 정규화 규칙                                                                                                                                                         |
| ----------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 성취 (30)   | 실제 리그/컵 트로피, 개인 시즌 평가, 성공 챕터, 성인 대표팀 출전 근거, U23 메달 | 리그 15/컵 10 × 리그 단계 100/75/50/25%, 출전 비율 40% 미만 절반. 개인 시즌 평가 6(최대 60), 챕터 2(최대 30), 성인 출전 0.5(최대 20), U23 금/은/동 12/8/5. 상한 100 |
| 기여 (25)   | 시즌 출전 시간 비율, 포지션별 90분당 실제 기록, 계약 약속 이행                  | 시즌별 `출전 비율×40 + 포지션 기록 점수×50% + 약속 이행 10`의 평균. 기대 기록과 정규화는 문서 19의 동결표를 따른다. 상한 100                                        |
| 장기성 (15) | 완료한 시즌 수, 총 출전, 중대 부상 에피소드, 컴백 태그                          | 0~20시즌 단조 증가 상한표 + 출전/50(최대 10) − 중대 부상당 3(최대 15) + 컴백 10. 상한 100                                                                           |
| 관계 (15)   | 시즌별 감독/주장/팬/에이전트 관계와 최종 관계                                   | 시즌 평균 80% + 최종 평균 20%. 과거 ledger가 없으면 기록된 감독 신뢰만 사용. 상한 100                                                                               |
| 서사 (15)   | 실제 부여된 CareerTag와 희귀도                                                  | COMMON 5, RARE 12, EPIC 25의 합. 반전 사건은 실제 태그를 얻었을 때만 반영하며 엔딩 자체를 다시 가산하지 않는다. 상한 100                                            |

위 표는 1.0.0의 실제 규칙이다. 초기 기획의 별도 리그 강도 곱, 로그 함수, 역할별 추가 가산,
부정 태그의 별도 관계 감점 등은 구현된 것으로 간주하지 않는다. 해당 사건이 실제 관계 값을
바꾼 경우에만 그 관계 기록이 반영된다. 추후 확장 시 새 Legacy 버전과 회귀 검증이 필요하다.
...
- 백분위와 밴드 밸런스는 ruleset에 묶인 독립 불변 아티팩트의 합성 참조 분포 `referencePopulationId`를 사용한다.
...
- 모집단을 탑재하기 전 생성된 결과는 `referencePopulationId=null`, `percentileHidden=true`로 보존한다. 나중에 모집단이 추가되어도 과거 결과를 같은 키에서 덮어쓰거나 백분위를 소급 삽입하지 않는다. 새로운 결과만 검증된 기준 집단을 사용한다.
- 공개 결과의 checksum은 점수 배열을 포함한 population 자체를 고정한다. 별도 content manifest/CI가 40,000개 원본 evidence, seed 정책, 실제 ruleset/content checksum 및 동결된 실행 bundle을 검증한다. 합성 표본은 실제 이용자 순위가 아니다.
...
참조 분포의 관찰 목표는 BAND-LEGEND 약 2%, BAND-ICON 약 8%, BAND-REMEMBERED 약 30%다. 목표와의 편차는 밸런스 리포트로 확인하되 그 편차만으로 merge pass/fail을 정하지 않는다. 고정 선택 정책의 모집단이 목표를 벗어났다는 이유만으로 점수를 역으로 끼워 맞추지 않는다. 플레이 정책의 대표성을 먼저 점검하고, 상한표 조정이 필요하면 새 버전·전체 재생성으로 검증한다.
...
- 모든 커리어는 엔딩 하나와 최고의 순간 하나를 반드시 가진다. 1.0.0은 실제 성공 챕터 → 트로피 → 첫 시즌 → 은퇴 기록 순으로 유효한 `sourceId`를 선택한다. 아직 기록하지 않은 복귀·계약 순간을 임의로 만들지 않는다.
...
부정 태그(TAG-TRAITOR, TAG-CONTROVERSIAL)는 서사 요소에서 희귀도만큼 가산한다. 관계 감점은 원 사건이 실제로 변경한 관계 기록에서만 반영하며, 1.0.0은 태그 자체로 추가 감점하지 않는다. 부정 태그가 서사를 만드는 것 자체는 유효한 커리어다.
...
type LegacyResult = {
  ...
  referencePopulationId: string | null;
  ...
  computedAt: { retirementRevision: number };
};
...
- 검증된 같은 Legacy/ruleset 참조 분포가 있는 새 결과만 `percentileHidden=false`다. 미탑재 결과는 true이며 `coverage.referencePopulation='UNAVAILABLE'`다. false여도 백분위는 상세 보기에만 노출한다.
- `bestMomentRef`는 항상 존재해야 한다. 앞선 경기·트로피·시즌 기록이 없으면 실제 은퇴 source를 사용한다.
- 위 DTO는 핵심 필드 요약이다. 실제 타입은 `packages/domain/src/legacy/result.ts`의 반환값이며, 공개 통산·타임라인 근거·coverage·무결성 hash도 함께 보존한다. `computedAt`은 벽시계가 아니라 재현 가능한 은퇴 revision이다.
...
| TEST-LEG-006 | 무관 fixture가 총점 80 이상에 도달하고 참조 분포의 관찰 비율을 별도로 보고한다                     |
