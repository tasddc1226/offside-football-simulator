...
밸런스 목표는 참조 분포에서 BAND-LEGEND 약 2%, BAND-ICON 약 8%, BAND-REMEMBERED 약 30%다. 목표와의 편차는 밸런스 리포트로 확인한다. 고정 선택 정책의 모집단이 목표를 벗어났다는 이유만으로 점수를 역으로 끼워 맞추지 않는다. 플레이 정책의 대표성을 먼저 점검하고, 상한표 조정이 필요하면 새 버전·전체 재생성으로 검증한다. 목표 미달은 분포 검증 통과와 별개로 명시한다.
