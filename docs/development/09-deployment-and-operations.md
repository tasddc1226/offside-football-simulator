...
- 세션의 마지막 활동 시각(`sessions.last_seen_at`, D-78 하트비트)은 "플레이 중 인원" 집계(5분 창, 프로필 수)에만 쓰고 개인을 표시하거나 활동 이력을 노출하지 않는다. 세션이 만료·폐기되면 함께 사라진다. 개인정보 화면(`apps/web/src/legal/privacy.tsx`)의 "세션의 마지막 활동 시각" 항목이 이 규칙의 사용자용 문구다.
