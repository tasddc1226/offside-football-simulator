...
상태(2026-09-06): 구현과 필수 로컬/API 인수 근거는 PR #103으로 main
`fb8b78260377c838947ae43caa3290481c3a4999`에 병합됐다. 아래 완료 조건은 충족됐으며 현재 release
main/expanded staging 배포, 구버전 불변, expanded 신규 1.1 커리어의 실제 은퇴·보관·강제
새로고침을 확인해 staging 인수를 완료했다. Production 활성화를 뜻하지 않으며, 합성 모집단을 실제
사용자 분포로 보거나 14개 엔딩을 모두 자연 플레이했다는 뜻도 아니다.
...
- [x] 20년 범위 fixture를 제한 시간 안에 완주한다.
- [x] 포지션별 성장·노쇠 경계가 golden curve와 일치한다.
- [x] 은퇴 후 일반 진행 명령이 거부된다.
- [x] 통산 합계가 FootballSeason 기록 합과 일치한다.
- [x] 같은 Career의 Legacy 결과가 재현된다.
- [x] 세계 최고가 아니어도 의미 있는 엔딩을 최소 12종 제공하고 폴백 엔딩이 있다.
- [x] Archive는 생성 후 변경되지 않는다.
- [x] GK/DF/MF/FW 동일 품질 fixture의 Legacy 총점 차이가 ±5 이내다.
- [x] 저장된 경기 근거로 고득점 Legacy에 도달 가능한 플레이 경로가 있다.
- [x] 한국 국적 fixture에서 병역 경로 선택이 커리어 연대기와 통산 기록에 반영된다.
