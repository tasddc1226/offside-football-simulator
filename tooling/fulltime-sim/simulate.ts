...
function run(N: number, policy: 'random' | 'smart'): { rows: Row[]; agg: Agg } {
  ...
  for (let iter = 0; iter < N; iter++) {
    ...
    let events = 0,
      evOk = 0,
      injuries = 0;
    let proDebutAge: number | null = null,
      euAge: number | null = null,
      retireReason = 'age35';
    ...
    try {
      for (let y = 0; y < 30 && !s.retired; y++) {
        ...
        const rec = res.rec,
          L = LEAGUES.find((l) => l.name === rec.league);
        if (rec.pro) {
        ...
        res.tours.forEach((t) => {
          if (t.inSquad)
            inc(
              A.tours as Record<string, number>,
              t.name.replace(/^\d+ /, '').replace(/ \(.*/, '') + ':' + t.stage,
            );
        });
        s_cur = s;
        let m = market(s),
    ...
    const T = pro.reduce((a, r) => ({ p: a.p + r.apps, g: a.g + r.goals, a: a.a + r.assists }), {
      p: 0,
      g: 0,
      a: 0,
    });
    const maxTier = Math.max(
      0,
      ...s.career.map((r) => (LEAGUES.find((l) => l.name === r.league) || { tier: 0 }).tier),
    );
    ...
    (s.storyLog || []).forEach((x) =>
      inc(A.endings as Record<string, number>, x.name + ':' + x.ending),
    );
    (s.miles || []).forEach((x) =>
      inc(
        A.miles as Record<string, number>,
        x.t
          .replace(/\(.*\)/, '')
          .replace(/ · .*/, '')
          .replace(/^.* (홈구장에서 은퇴 경기|레전드 헌정)$/, '$1'),
      ),
    );
    inc(
      A.mil as Record<string, number>,
      s.mil.exempt
        ? 'exempt:' + s.mil.exempt
        : s.mil.served
          ? 'served:' + s.mil.type
    ...
    inc(
      A.ballonBest as Record<string, number>,
      ballonBest === 99 ? 'none' : ballonBest === 1 ? '1' : ballonBest <= 10 ? '2-10' : '11-30',
    );
    inc(
      A.capsBin as Record<string, number>,
      s.nat.caps === 0
        ? '0'
        : s.nat.caps < 30
          ? '1-29'
    ...
    inc(
      A.moneyBin as Record<string, number>,
      s.money < 1e4 ? '<1억' : s.money < 1e5 ? '1-10억' : s.money < 1e6 ? '10-100억' : '100억+',
    );
    ...
    rows.push({
      ...
      peak: s.peak,
      retireAge: s.age,
      seasons: s.career.length,
      proSeasons: pro.length,
      proDebutAge: proDebutAge ?? '',
      euAge: euAge ?? '',
      firstLeague: first,
      maxTier,
      clubs: clubs.size,
  ...
  function value(x: GameState): number {
    ...
    return (
      ...
      (S.cs || 0) * 0.3 -
      x.injury * 0.8 +
      (x.nat ? x.nat.caps * 0.2 : 0) +
      (x.flags && x.flags.scouted ? 2 : 0) +
      (x.awards ? x.awards.length * 3 : 0)
    );
  }
  ...
  function pickOption(s: GameState, m: { options: MarketOption[] }): MarketOption {
    ...
    if (smart) {
      ...
      return (
        stay || good || offers[0] || m.options.find((o) => o.kind !== 'sangmu') || m.options[0]!
      );
    }
    const best = offers.sort((a, b) => b.str - a.str)[0];
    return best && (!stay || best.str > s.club.str + 2)
      ? best
      : stay || m.options.find((o) => o.kind !== 'sangmu') || m.options[0]!;
  }
  function anyJob(o: MarketOption): boolean {
    return (
      ['offer', 'renew', 'stay', 'uni', 'sangmu', 'army', 'serve'] as MarketOption['kind'][]
    ).includes(o.kind);
  }
  function realJob(o: MarketOption): boolean {
    if (o.kind === 'sangmu' || o.kind === 'army' || o.kind === 'serve') return true;
    const L =
      o.kind === 'offer' ? LEAGUES.find((l) => l.id === o.leagueId) : leagueOf(s_cur!.leagueId);
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(outDir, `careers-${policy}.csv`),
  ...
  console.log(
    `[${tag}] ${rows.length} careers (${policy}) in ${((Date.now() - t0) / 1000).toFixed(1)}s, errors ${agg.errors}`,
  );
}
