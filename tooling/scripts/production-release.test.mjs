...
describe('production release guards', () => {
  it('keeps non-production seeds and staging expectations on the release target', () => {
    const expectedPair = "'3.4.0', '0.13.0'";
  ...
  it('targets the 3.4.0/0.13.0 manifest from the actual 3.3.0/0.12.0 predecessor', () => {
    expect(PRODUCTION_SEASON.rulesetVersion).toBe('3.4.0');
    expect(PRODUCTION_SEASON.contentPackVersion).toBe('0.13.0');
  ...
  it('activates only from the exact previous manifest and emits a guarded inverse', () => {
    ...
    expect(decision.sql).toContain("ruleset_version = '3.3.0'");
    expect(decision.sql).toContain("content_pack_version = '0.12.0'");
    ...
    expect(decision.rollbackSql).toContain("ruleset_version = '3.4.0'");
    expect(decision.rollbackSql).toContain("content_pack_version = '0.13.0'");
