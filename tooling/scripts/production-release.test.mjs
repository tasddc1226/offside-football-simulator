...
describe('production release guards', () => {
  it('keeps non-production seeds and staging expectations on the release target', () => {
    const expectedPair = "'3.2.0', '0.11.0'";
  ...
  it('targets the 3.2.0/0.11.0 manifest from the actual 3.1.0/0.10.0 predecessor', () => {
    expect(PRODUCTION_SEASON.rulesetVersion).toBe('3.2.0');
    expect(PRODUCTION_SEASON.contentPackVersion).toBe('0.11.0');
  ...
  it('activates only from the exact previous manifest and emits a guarded inverse', () => {
    ...
    expect(decision.sql).toContain(
      "SET ruleset_version = '3.2.0', content_pack_version = '0.11.0'",
    );
    ...
    expect(decision.rollbackSql).toContain("ruleset_version = '3.2.0'");
    expect(decision.rollbackSql).toContain("content_pack_version = '0.11.0'");
    ...
    expect(() =>
      decideSeason(
        [{ ...previous, rulesetVersion: '1.5.0', contentPackVersion: '0.11.0' }],
        proposal,
      ),
    ).toThrow('different ACTIVE');
  });
