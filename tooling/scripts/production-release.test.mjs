...
describe('production release guards', () => {
  ...
  it('targets the 3.5.0/0.14.0 manifest from the actual 3.4.0/0.13.0 predecessor', () => {
    expect(PRODUCTION_SEASON.rulesetVersion).toBe('3.5.0');
    expect(PRODUCTION_SEASON.contentPackVersion).toBe('0.14.0');
    ...
    expect(() =>
      decideSeason(
        [{ ...proposal, rulesetVersion: '3.3.0', contentPackVersion: '0.12.0' }],
        proposal,
      ),
  ...
  it('activates only from the exact previous manifest and emits a guarded inverse', () => {
    ...
    expect(decision.sql).toContain("ruleset_version = '3.4.0'");
    expect(decision.sql).toContain("content_pack_version = '0.13.0'");
    expect(decision.sql).toContain(
      "SET ruleset_version = '3.5.0', content_pack_version = '0.14.0'",
    );
    ...
    expect(decision.rollbackSql).toContain(
      "SET ruleset_version = '3.4.0', content_pack_version = '0.13.0'",
    );
    expect(decision.rollbackSql).toContain("ruleset_version = '3.5.0'");
    ...
    expect(() =>
      decideSeason(
        [{ ...previous, rulesetVersion: '1.5.0', contentPackVersion: '0.13.0' }],
        proposal,
      ),
    ).toThrow('different ACTIVE');
  });
