...
describe('production release guards', () => {
  it('keeps non-production seeds and staging expectations on the release target', () => {
    const expectedPair = "'3.0.0', '0.9.0'";
  ...
  it('targets the 3.0.0/0.9.0 manifest from the actual 2.1.0/0.8.0 predecessor', () => {
    expect(PRODUCTION_SEASON.rulesetVersion).toBe('3.0.0');
    expect(PRODUCTION_SEASON.contentPackVersion).toBe('0.9.0');
  ...
  it('activates only from the exact previous manifest and emits a guarded inverse', () => {
    ...
    expect(decision.sql).toContain("ruleset_version = '2.1.0'");
    expect(decision.sql).toContain("content_pack_version = '0.8.0'");
    expect(decision.sql).toContain("SET ruleset_version = '3.0.0', content_pack_version = '0.9.0'");
    ...
    expect(decision.rollbackSql).toContain(
      "SET ruleset_version = '2.1.0', content_pack_version = '0.8.0'",
    );
    expect(decision.rollbackSql).toContain("ruleset_version = '3.0.0'");
    expect(decision.rollbackSql).toContain("content_pack_version = '0.9.0'");
    ...
    expect(() =>
      decideSeason(
        [{ ...previous, rulesetVersion: '1.5.0', contentPackVersion: '0.9.0' }],
        proposal,
      ),
    ).toThrow('different ACTIVE');
  });
