...
// revision·snapshot 없음). T-10-010이 0019에서 club_customs(클럽 커스텀 동기화)를, T-10-011이 0020에서 board_posts·board_comments(게시판)를, T-10-016이 0022에서 balance_versions(밸런스 설정)를, T-10-027이 0024에서 server_firsts(서버 최초 기록)·app_meta를 추가했다. 이 목록 밖의 테이블이 나타나거나 이 목록의 테이블이 없으면 preflight/deploy
